# generated by datamodel-codegen:
#   filename:  openapi.json
#   timestamp: 2026-09-21T15:52:32+00:00

from __future__ import annotations

from datetime import date as Date
from datetime import datetime as DateTime
from enum import Enum
from typing import Any, Dict, List, Optional, Union
from uuid import UUID

from pydantic import (
    BaseModel,
    ConfigDict,
    Field,
    PositiveFloat,
    confloat,
    conint,
    constr,
)


class AccountBalance(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    account_number: str = Field(
        ...,
        description="Ledger account number in the chart of accounts",
        title="Account Number",
    )
    account_name: Optional[str] = Field(
        None, description="Display name of the ledger account", title="Account Name"
    )
    debit: float = Field(
        ...,
        description="Debit at end date. When debit and credit are equal to 0 and balance is different from 0 then the debit and credit cannot be determined, only the balance can be calculated in those cases.",
        title="Debit",
    )
    credit: float = Field(
        ...,
        description="Credit at end date. When debit and credit are equal to 0 and balance is different from 0 then the debit and credit cannot be determined, only the balance can be calculated in those cases.",
        title="Credit",
    )
    balance: float = Field(..., title="Balance")
    period_debit: float = Field(
        ...,
        description="Debit at end date beginning from the given start date. When debit and credit are equal to 0 and balance is different from 0 then the debit and credit cannot be determined, only the balance can be calculated in those cases.",
        title="Period Debit",
    )
    period_credit: float = Field(
        ...,
        description="Credit at end date beginning from the given start date. When debit and credit are equal to 0 and balance is different from 0 then the debit and credit cannot be determined, only the balance can be calculated in those cases.",
        title="Period Credit",
    )
    period_balance: float = Field(
        ...,
        description="Balance at end date beginning from the given start date.",
        title="Period Balance",
    )


class AccountBalanceFilter(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    accounts: List[str] = Field(
        ...,
        description='Array/list containing the account numbers/classes to include in the response. E.g. ["601", "7"] to retrieve all account in the class 7 and accounts with an account number beginning with 601',
        title="Accounts",
    )
    start: Optional[str] = Field(
        None,
        description="Start month (inclusive). Format: YYYYMM (e.g., 202502 for February 2025).",
        title="Start",
    )
    end: constr(min_length=6, max_length=6) = Field(
        ...,
        description="End month (inclusive). Format: YYYYMM (e.g., 202512 for December 2025).",
        title="End",
    )


class AccountItemType(Enum):
    bank = "bank"
    cash = "cash"
    other_financial = "other_financial"
    receivable = "receivable"
    payable = "payable"
    income = "income"
    expense = "expense"
    other_expense = "other_expense"
    vat = "vat"
    other = "other"


class AccountTaxCode(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="The ID of the tax code, as returned by the tax codes endpoint of the same connection.",
        examples=["<accounting-tax-code>"],
        title="Id",
    )
    default: Optional[bool] = Field(
        False,
        description="Indicates whether this tax code is the default tax code of the ledger account. It doesn't mean that the accounting system will apply it by default, this is provided by the value of the 'mode' attribute. In a 'forced' mode the default tax code will be automatically applied by the accounting system when the ledger account is used.",
        title="Default",
    )


class AccountTaxMode(Enum):
    free = "free"
    restricted = "restricted"
    forced = "forced"


class AccountToCreate(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    account_number: constr(min_length=1) = Field(
        ..., examples=["<accounting-account-number>"], title="Account Number"
    )
    account_name: constr(min_length=1) = Field(
        ..., examples=["Consulting services"], title="Account Name"
    )


class AccountTypeFilter(Enum):
    bank = "bank"
    cash = "cash"
    income = "income"
    expense = "expense"
    other_expense = "other_expense"
    vat = "vat"


class AccountingCategoryItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the accounting category",
        examples=["<this-accounting-category-id>"],
        title="Id",
    )
    name: str = Field(
        ...,
        description="Name given to the accounting category",
        examples=["Beverages"],
        title="Name",
    )
    code: Optional[str] = Field(
        None,
        description="Code assigned to the category",
        examples=["123456"],
        title="Code",
    )
    ledger_account_code: Optional[str] = Field(
        None,
        description="Ledger account code assigned to the category",
        examples=["<pos-ledger-account-code>"],
        title="Ledger Account Code",
    )
    posting_account_code: Optional[str] = Field(
        None,
        description="Posting account code assigned to the category",
        examples=["<pos-posting-account-code>"],
        title="Posting Account Code",
    )


class AccountingInfoOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    main_currency: Optional[str] = Field(
        None,
        description="Main currency of the accounting folder. ISO-4217 format.",
        examples=["EUR", "USD", "GBP"],
        title="Main Currency",
    )
    main_currency_total: Optional[float] = Field(
        0,
        description="Total amount of the invoice in the accounting folder's currency.",
        examples=[100.0, 123.4, 1.23],
        title="Main Currency Total",
    )
    account_number: Optional[str] = Field(
        None,
        description="Client/supplier ledger account used to book the invoice total amount. (optional)",
        examples=["411001", "401234", "1600"],
        title="Account Number",
    )


class AccountingInvoicePaymentItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    invoice_id: str = Field(
        ...,
        description="Technical ID of the invoice in the accounting system.",
        examples=["<accounting-invoice-id>"],
        title="Invoice Id",
    )
    amount: PositiveFloat = Field(
        ...,
        description="Paid amount dedicated to the invoice. The amount must be positive and follows the sign of the linked invoice.",
        examples=[242.0],
        title="Amount",
    )


class AccountingPaymentMethod(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical ID of the payment method in the accounting system.",
        examples=["<this-payment-method-id>"],
        title="Id",
    )
    name: Optional[str] = Field(
        None,
        description="Name of the payment method",
        examples=["Bank transfer"],
        title="Name",
    )


class AccountingPaymentTerms(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical ID of the payment term in the accounting system.",
        examples=["<this-payment-term-id>"],
        title="Id",
    )
    name: Optional[str] = Field(
        None, description="Name of the payment term", examples=["30 days"], title="Name"
    )


class AddressItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    address_type: Optional[str] = Field(
        None, description="Type of the address", examples=["home"], title="Address Type"
    )
    name: Optional[str] = Field(
        None,
        description="Name given to the address (e.g. 'home')",
        examples=["Home"],
        title="Name",
    )
    street: Optional[str] = Field(
        None, description="Street name", examples=["Main Street"], title="Street"
    )
    number: Optional[str] = Field(
        None, description="Number of the address", examples=["123"], title="Number"
    )
    box: Optional[str] = Field(
        None, description="Box of the address", examples=["A"], title="Box"
    )
    city: Optional[str] = Field(
        None, description="City name", examples=["Paris"], title="City"
    )
    postal_code: Optional[str] = Field(
        None,
        description="Postal code of the address",
        examples=["75000"],
        title="Postal Code",
    )
    country: Optional[str] = Field(
        None,
        description="Country, format: ISO 3166-1 codes.",
        examples=["FR"],
        title="Country",
    )


class AddressType(Enum):
    main = "main"
    delivery = "delivery"
    invoice = "invoice"


class AddressTypeInvoicing(Enum):
    main = "main"
    delivery = "delivery"
    invoice = "invoice"
    other = "other"


class AnalyticAccountItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    active: Optional[bool] = Field(
        True,
        description="Flag indicating whether the analytic account is active.",
        title="Active",
    )
    code: constr(min_length=1) = Field(
        ...,
        description="Short code for the analytic account. Often used for internal referencing. May be required to be unique",
        examples=["ANLY-01"],
        title="Code",
    )
    name: constr(min_length=1) = Field(
        ...,
        description="Name or label of the analytic account as it appears in the accounting system.",
        examples=["Marketing"],
        title="Name",
    )
    currency: str = Field(
        ...,
        description="Indicates the currency of the analytic account (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )


class AnalyticAccountItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique id of the analytic account in the accounting system.",
        examples=["<this-analytic-account-id>"],
        title="Id",
    )
    active: bool = Field(
        ...,
        description="Flag indicating whether the analytic account is active.",
        title="Active",
    )
    code: Optional[str] = Field(
        None,
        description="Short code for the analytic account. Often used for internal referencing. For some integrations, it may be required to be unique.",
        examples=["ANLY-01"],
        title="Code",
    )
    name: str = Field(
        ...,
        description="Name or label of the analytic account as it appears in the accounting system.",
        examples=["Marketing"],
        title="Name",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the analytic account (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    balance: Optional[float] = Field(0, title="Balance")
    credit: Optional[float] = Field(0, title="Credit")
    debit: Optional[float] = Field(0, title="Debit")


class AnalyticAccountItemOutMultiAnalyticPlans(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique id of the analytic account in the accounting system.",
        examples=["<this-analytic-account-id>"],
        title="Id",
    )
    active: bool = Field(
        ...,
        description="Flag indicating whether the analytic account is active.",
        title="Active",
    )
    code: Optional[str] = Field(
        None,
        description="Short code for the analytic account. Often used for internal referencing. For some integrations, it may be required to be unique.",
        examples=["ANLY-01"],
        title="Code",
    )
    name: str = Field(
        ...,
        description="Name or label of the analytic account as it appears in the accounting system.",
        examples=["Marketing"],
        title="Name",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the analytic account (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    balance: Optional[float] = Field(0, title="Balance")
    credit: Optional[float] = Field(0, title="Credit")
    debit: Optional[float] = Field(0, title="Debit")
    analytic_plan: str = Field(
        ..., examples=["<accounting-analytic-plan-id>"], title="Analytic Plan"
    )


class AnalyticAccountItemUpdate(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    active: Optional[bool] = Field(
        True,
        description="Flag indicating whether the analytic account is active.",
        title="Active",
    )
    code: Optional[str] = Field(
        None,
        description="Short code for the analytic account. Often used for internal referencing. May be required to be unique",
        examples=["ANLY-01"],
        title="Code",
    )
    name: Optional[str] = Field(
        None,
        description="Name or label of the analytic account as it appears in the accounting system.",
        examples=["Marketing"],
        title="Name",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the analytic account (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )


class AnalyticDistributionDetail(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    analytic_account: str = Field(
        ...,
        description="Technical ID of the analytic account.",
        examples=["<accounting-analytic-account-id>"],
        title="Analytic Account",
    )
    percentage: float = Field(
        ...,
        description="Percentage of the untaxed amount attributed to this analytic account. Only whole numbers (no decimals) are allowed. The total across the analytic distribution (all accounts in a given analytic plan) must equal 100%.",
        examples=[100.0],
        title="Percentage",
    )


class AnalyticDistributionDetailInvoicing(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    analytic_account_code: str = Field(
        ...,
        description="Code of the analytic account.",
        examples=["<invoicing-analytic-account-code>"],
        title="Analytic Account Code",
    )
    analytic_account_name: str = Field(
        ...,
        description="Name of the analytic account.",
        examples=["Marketing"],
        title="Analytic Account Name",
    )
    percentage: float = Field(
        ...,
        description="Percentage of the untaxed amount attributed to this analytic account. Only whole numbers (no decimals) are allowed. The total across the analytic distribution (all accounts in a given analytic plan) must equal 100%.",
        examples=[100.0],
        title="Percentage",
    )


class AnalyticDistributionInvoicing(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    analytic_plan_code: str = Field(
        ...,
        description="Code of the analytic plan to which the distribution applies.",
        examples=["<invoicing-analytic-plan-code>"],
        title="Analytic Plan Code",
    )
    analytic_accounts: List[AnalyticDistributionDetailInvoicing] = Field(
        ...,
        description="List of analytic accounts and their respective percentages for the distribution.",
        title="Analytic Accounts",
    )


class AnalyticPlanItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique id of the analytic plan instance in the accounting system.",
        title="Id",
    )
    name: str = Field(
        ...,
        description="Name of the analytic plan as it appears in the accounting system.",
        title="Name",
    )
    active: Optional[bool] = Field(
        True, description="Indicates if the analytic plan is active.", title="Active"
    )
    parent_id: Optional[str] = Field(
        None,
        description="Id of the parent analytic plan, if any. This is used to create a hierarchy of analytic plans when that is supported.",
        title="Parent Id",
    )


class Api(Enum):
    Point_of_Sale = "Point of Sale"
    eCommerce = "eCommerce"
    Accounting = "Accounting"
    Invoicing = "Invoicing"
    Communication = "Communication"
    Banking = "Banking"
    Custom = "Custom"
    Payment = "Payment"
    Property_Management_System = "Property Management System"


class AttachmentItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    base64_string: str = Field(..., title="Base64 String")
    name: Optional[
        constr(
            pattern=r"^[a-zA-Z0-9\-_ ]{1,196}(?:.pdf)?$", min_length=1, max_length=200
        )
    ] = Field(
        None,
        description="A name for the PDF file to be created for accounting software that support it.",
        title="Name",
    )


class AttachmentItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="The ID of the attachment in the accounting system.",
        examples=["<this-attachment-id>"],
        title="Id",
    )
    base64_string: str = Field(
        ...,
        description="The file content encoded as a base64 string. Decode this to reconstruct the original file (PDF, image, etc.).",
        examples=["<base64-encoded-file-content>"],
        title="Base64 String",
    )


class AuthItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    clientId: str = Field(..., title="Clientid")
    clientSecret: str = Field(..., title="Clientsecret")
    accountId: UUID = Field(..., title="Accountid")
    marketplaceId: Optional[UUID] = Field(None, title="Marketplaceid")


class BankAccountItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    code: constr(min_length=1) = Field(
        ...,
        description="A userfriendly code to represent the bank account. This will be used as the code of the journal if we need to create a journal for the bank account.",
        examples=["BANK-001"],
        title="Code",
    )
    currency: constr(min_length=1) = Field(
        ...,
        description="Currency of the bank account (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    account_name: Optional[str] = Field(
        None,
        description="Name of the bank account. Represents this specific bank account in the accounting system.",
        examples=["Main checking account"],
        title="Account Name",
    )
    account_number: Optional[str] = Field(
        None,
        description="Unique number representing the bank account. At least one of the fields 'account_number' or 'IBAN' must be filled in.",
        examples=["0000000000"],
        title="Account Number",
    )
    iban: Optional[str] = Field(
        None,
        description="IBAN of the bank account. At least one of the fields 'account_number' or 'IBAN' must be filled in.",
        examples=["BE00000000000000"],
        title="Iban",
    )
    bank_name: str = Field(
        ...,
        description="Name of the bank linked to the bank account.",
        examples=["Example Bank"],
        title="Bank Name",
    )


class BankAccountItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., examples=["<this-bank-account-id>"], title="Id")
    code: Optional[str] = Field(
        None,
        description="A userfriendly code to represent the bank account. This will be used as the code of the journal if we need to create a journal for the bank account.",
        examples=["BANK-001"],
        title="Code",
    )
    currency: Optional[str] = Field(
        None,
        description="Currency of the bank account (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    account_name: Optional[str] = Field(
        None,
        description="Name of the bank account. Represents this specific bank account in the accounting system.",
        examples=["Main checking account"],
        title="Account Name",
    )
    account_number: Optional[str] = Field(
        None,
        description="Unique number representing the bank account.",
        examples=["0000000000"],
        title="Account Number",
    )
    iban: Optional[str] = Field(
        None,
        description="IBAN of the bank account.",
        examples=["BE00000000000000"],
        title="Iban",
    )
    bank_name: Optional[str] = Field(
        None,
        description="Name of the bank linked to the bank account.",
        examples=["Example Bank"],
        title="Bank Name",
    )
    journal_id: Optional[str] = Field(
        None,
        description="Indicates the journal used in the accounting system to represent the bank account. The journal contains all the transactions related to this specific bank account.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    ledger_account: Optional[str] = Field(
        None,
        description="Indicates the ledger account used in the accounting system to represent the bank account. The ledger account is used in all the transactions related to this specific bank account. This ledger account is used to track the balance of the bank account.",
        examples=["<accounting-account-number>"],
        title="Ledger Account",
    )
    unallocated_account: Optional[str] = Field(
        None,
        description="Unallocated ledger account used to book entries when the final ledger account / client / supplier / employee is not yet known (specific to bank and cash journals)",
        examples=["<accounting-account-number>"],
        title="Unallocated Account",
    )


class BankAccountRoutingType(Enum):
    unknown = "unknown"
    ach = "ach"
    sort_code = "sort_code"
    bsb = "bsb"
    transit = "transit"
    ifsc = "ifsc"
    clabe = "clabe"


class BankTransactionCounterpartType(Enum):
    customer = "customer"
    supplier = "supplier"
    employee = "employee"


class BankTransactionStatus(Enum):
    draft = "draft"
    done = "done"


class BankingAccountItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the account",
        examples=["<this-account-id>"],
        title="Id",
    )
    currency: str = Field(
        ..., description="Currency of the account", examples=["EUR"], title="Currency"
    )
    current_balance: float = Field(
        ...,
        description="Current balance of the account",
        examples=[1000],
        title="Current Balance",
    )
    current_balance_last_update_date: Optional[DateTime] = Field(
        None,
        description="Last update date of the current balance",
        examples=["2025-01-01T00:00:00Z"],
        title="Current Balance Last Update Date",
    )
    available_balance: float = Field(
        ...,
        description="Available balance of the account",
        examples=[1000],
        title="Available Balance",
    )
    available_balance_last_update_date: Optional[DateTime] = Field(
        None,
        description="Last update date of the available balance",
        examples=["2025-01-01T00:00:00Z"],
        title="Available Balance Last Update Date",
    )
    description: str = Field(
        ...,
        description="Description of the account",
        examples=["Main checking account"],
        title="Description",
    )
    reference: str = Field(
        ...,
        description="External reference identifying the account, e.g. its IBAN.",
        examples=["BE00000000000000"],
        title="Reference",
    )
    reference_type: str = Field(
        ...,
        description="Type of the reference",
        examples=["IBAN"],
        title="Reference Type",
    )
    holder_name: Optional[str] = Field(
        None,
        description="Name of the account holder",
        examples=["John Doe"],
        title="Holder Name",
    )
    active: Optional[bool] = Field(
        True,
        description="Indicates if the account is active",
        examples=[True],
        title="Active",
    )


class BankingCounterPartItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: Optional[str] = Field(
        None, description="Name of the counterpart", examples=["John Doe"], title="Name"
    )
    reference: Optional[str] = Field(
        None,
        description="Reference of the counterpart",
        examples=["BE00000000000000"],
        title="Reference",
    )
    details: Optional[str] = Field(
        None,
        description="Details of the counterpart",
        examples=["Card payment"],
        title="Details",
    )


class BankingFinancialInstitutionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the financial institution",
        examples=["<this-financial-institution-id>"],
        title="Id",
    )
    bic: Optional[str] = Field(
        None,
        description="BIC of the financial institution",
        examples=["EXMPBEBBXXX"],
        title="Bic",
    )
    country: Optional[str] = Field(
        None,
        description="Country of the financial institution, format: ISO 3166-1 codes.",
        examples=["FR"],
        title="Country",
    )
    name: str = Field(
        ...,
        description="Name of the financial institution",
        examples=["BNP"],
        title="Name",
    )


class BankingOpeningBalanceItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    account_id: str = Field(
        ...,
        description="Identifier of the banking account",
        examples=["<banking-account-id>"],
        title="Account Id",
    )
    date: Date = Field(
        ...,
        description="Date for which the opening balance is reported",
        examples=["2025-01-15"],
        title="Date",
    )
    opening_balance: Optional[float] = Field(
        None,
        description="Opening balance of the account on the given date",
        examples=[1000.0],
        title="Opening Balance",
    )
    currency: Optional[str] = Field(
        None, description="Currency of the balance", examples=["EUR"], title="Currency"
    )


class BankingTransactionStatus(Enum):
    pending = "pending"
    declined = "declined"
    completed = "completed"


class BookYear(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(
        ...,
        description="Name of the book year, typically in the format 'YYYY'.",
        examples=["2026"],
        title="Name",
    )
    start: Date = Field(
        ...,
        description="Start date of the book year (format: YYYY-MM-DD). This is the date when the book year begins.",
        examples=["2026-01-01"],
        title="Start",
    )
    end: Date = Field(
        ...,
        description="End date of the book year (format: YYYY-MM-DD). This is the date when the book year ends.",
        examples=["2026-12-31"],
        title="End",
    )
    closed: bool = Field(
        ...,
        description="Indicates whether the book year is closed. A closed book year normally cannot be modified or have new entries added to it.",
        title="Closed",
    )


class BoolParam(Enum):
    true = "true"
    false = "false"


class ChainExecutionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., description="Execution id", title="Id")
    parentexecutionid: Optional[str] = Field(
        ...,
        description="Parent execution id. An execution is linked to multiple child executions linked to consumers.",
        title="Parentexecutionid",
    )
    start: DateTime = Field(..., title="Start")
    end: Optional[DateTime] = Field(..., title="End")
    status: str = Field(..., title="Status")


class ChiftError(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    message: str = Field(..., title="Message")
    status: Optional[str] = Field("error", title="Status")
    detail: Optional[str] = Field("", title="Detail")
    error_code: Optional[str] = Field(None, title="Error Code")


class ChiftPageAccountBalance(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[AccountBalance] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageAccountingCategoryItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[AccountingCategoryItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageAccountingPaymentMethod(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[AccountingPaymentMethod] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageAccountingPaymentTerms(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[AccountingPaymentTerms] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageAnalyticAccountItemOutMultiAnalyticPlans(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[AnalyticAccountItemOutMultiAnalyticPlans] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageAnalyticAccountItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[AnalyticAccountItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageAnalyticPlanItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[AnalyticPlanItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageAttachmentItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[AttachmentItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageBankAccountItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[BankAccountItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageBankingAccountItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[BankingAccountItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageBankingCounterPartItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[BankingCounterPartItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageBankingFinancialInstitutionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[BankingFinancialInstitutionItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageBookYear(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[BookYear] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ClosureOperationItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    type: str = Field(
        ...,
        description="Type of the cash movement operation",
        examples=["withdrawal"],
        title="Type",
    )
    total: float = Field(
        ..., description="Total amount of the cash movement operation", title="Total"
    )


class ClosurePaymentItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Unique identifier of the payment method used for the payment",
        examples=["<pos-payment-method-id>"],
        title="Payment Method Id",
    )
    payment_method_name: Optional[str] = Field(
        None,
        description="Chift's name of the payment method used for the payment",
        examples=["Cash"],
        title="Payment Method Name",
    )
    tip: Optional[float] = Field(
        "0",
        description="Total amount of the tips of the payment. Tips are not part of the total of the payment",
        examples=[1],
        title="Tip",
    )
    total: float = Field(
        ..., description="Total amount including tax", examples=[11], title="Total"
    )


class ClosureStates(Enum):
    open = "open"
    closed = "closed"


class CommerceAddressItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    first_name: str = Field(..., examples=["John"], title="First Name")
    last_name: str = Field(..., examples=["Doe"], title="Last Name")
    street: str = Field(..., examples=["Example Street"], title="Street")
    number: str = Field(..., examples=["123"], title="Number")
    box: Optional[str] = Field(None, examples=["A"], title="Box")
    city: str = Field(..., examples=["Brussels"], title="City")
    postal_code: Optional[str] = Field(None, examples=["1000"], title="Postal Code")
    country: str = Field(
        ..., description="Format: ISO 3166-1 codes.", examples=["BE"], title="Country"
    )
    phone: Optional[str] = Field(None, examples=["+32 2 000 00 00"], title="Phone")
    email: Optional[str] = Field(None, examples=["john.doe@example.com"], title="Email")


class CommerceAddressItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    address_type: AddressType
    company_name: Optional[str] = Field(
        None, examples=["Example Company S.A."], title="Company Name"
    )
    first_name: Optional[str] = Field(None, examples=["John"], title="First Name")
    last_name: Optional[str] = Field(None, examples=["Doe"], title="Last Name")
    street: Optional[str] = Field(None, examples=["Example Street"], title="Street")
    number: Optional[str] = Field(None, examples=["123"], title="Number")
    box: Optional[str] = Field(None, examples=["A"], title="Box")
    city: Optional[str] = Field(None, examples=["Brussels"], title="City")
    postal_code: Optional[str] = Field(None, examples=["1000"], title="Postal Code")
    country: Optional[str] = Field(
        None, description="Format: ISO 3166-1 codes.", examples=["BE"], title="Country"
    )
    phone: Optional[str] = Field(None, examples=["+32 2 000 00 00"], title="Phone")
    email: Optional[str] = Field(None, examples=["john.doe@example.com"], title="Email")
    vat: Optional[str] = Field(None, examples=["BE0000000000"], title="Vat")


class CommerceDiscountItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., examples=["Promo code"], title="Name")
    description: str = Field(..., examples=["10% discount"], title="Description")
    amount: float = Field(..., examples=[10.0], title="Amount")


class CommerceLocationItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the location in Chift",
        examples=["<this-location-id>"],
        title="Id",
    )
    name: str = Field(..., examples=["Main warehouse"], title="Name")


class CommerceProductCategoryItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the category in Chift",
        examples=["<this-category-id>"],
        title="Id",
    )
    name: str = Field(..., examples=["Beverages"], title="Name")


class CommerceProductPriceItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    currency: str = Field(..., examples=["EUR"], title="Currency")
    price: Optional[float] = Field(0, examples=[100.0], title="Price")


class CommerceTransactionStatus(Enum):
    failed = "failed"
    pending = "pending"
    success = "success"


class CommonAttributeItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., examples=["Material"], title="Name")
    values: List[str] = Field(..., examples=[["Cotton"]], title="Values")


class ConnectionStatus(Enum):
    active = "active"
    inactive = "inactive"


class ConsumerAccountItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: constr(min_length=1) = Field(
        ...,
        description="Name of the marketplace account linked to the consumer.",
        title="Name",
    )
    email: Optional[str] = Field(
        None,
        description="Email of the user with the marketplace admin role on that account.",
        title="Email",
    )


class ConsumerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    consumerid: UUID = Field(
        ..., description="Unique id of the consumer in Chift", title="Consumerid"
    )
    name: constr(min_length=1) = Field(
        ...,
        description="Name of the consumer as it will appear in your Chift platform. This name may be used for automated communications, such as automatic mailings if you enable this feature",
        title="Name",
    )
    email: Optional[str] = Field(
        None,
        description="Optional - Email address of the consumer. It can be used for sending automated reminders (if the feature is enabled) and deployment by mail of local agent (when on-premise connector).",
        title="Email",
    )
    created_on: Optional[DateTime] = Field(
        None,
        description="Date and time when the consumer was created",
        title="Created On",
    )
    internal_reference: Optional[str] = Field(
        None,
        description="Your internal reference for the client (e.g., customer ID or unique identifier in your system).",
        title="Internal Reference",
    )
    redirect_url: Optional[str] = Field(
        None,
        description="URL to redirect the user after completing the activation via our link. Useful for directing customers to a dynamic or personalized url. You can also set up a default static URL in Chift's back office for all consumers.",
        title="Redirect Url",
    )
    account: Optional[ConsumerAccountItem] = Field(
        None,
        description="Marketplace account of the consumer. Only returned when include_account is true, and only for marketplace consumers.",
    )


class ContactGender(Enum):
    H = "H"
    F = "F"
    N_A = "N/A"


class ContactItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique id of the contact in the accounting system.",
        examples=["<this-contact-id>"],
        title="Id",
    )
    name: str = Field(
        ...,
        description="Full name of the contact.",
        examples=["John Doe"],
        title="Name",
    )
    first_name: Optional[str] = Field(
        None,
        description="First name of the contact.",
        examples=["John"],
        title="First Name",
    )
    last_name: Optional[str] = Field(
        None,
        description="Last name of the contact.",
        examples=["Doe"],
        title="Last Name",
    )
    function: Optional[str] = Field(
        None,
        description="Business role or job title of the contact.",
        examples=["Accountant"],
        title="Function",
    )
    email: Optional[str] = Field(
        None,
        description="Email address of the contact.",
        examples=["john.doe@example.com"],
        title="Email",
    )
    phone: Optional[str] = Field(
        None,
        description="Phone number of the contact.",
        examples=["+32 2 000 00 00"],
        title="Phone",
    )
    mobile: Optional[str] = Field(
        None,
        description="Mobile phone number of the contact.",
        examples=["+32 4 000 00 00"],
        title="Mobile",
    )


class ContactType(Enum):
    prospect = "prospect"
    customer = "customer"
    supplier = "supplier"
    all = "all"


class CountryItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    code: str = Field(
        ..., description="Country ISO Code", examples=["BE"], title="Code"
    )
    name: str = Field(..., examples=["Belgium"], title="Name")


class CoverageStatus(Enum):
    SUPPORTED = "SUPPORTED"
    NOT_IMPLEMENTED = "NOT_IMPLEMENTED"
    NOT_SUPPORTED = "NOT_SUPPORTED"
    UNDER_ANALYSIS = "UNDER_ANALYSIS"


class CreateConsumerSyncItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    syncid: UUID = Field(..., title="Syncid")
    integrationids: Optional[List[str]] = Field(
        [],
        description="[OPTIONAL] Can be used to specify maximum one integrationid for each One API that you want to highlight. If specified, only this connector will be displayed to your clients.",
        title="Integrationids",
    )
    country: Optional[str] = Field(
        None,
        description="ISO 3166-1 alpha-2 country code to filter connectors by country. Ignored if integrationids are provided.",
        title="Country",
    )
    link_metadata: Optional[Dict[str, Any]] = Field(None, title="Link Metadata")


class CredentialItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    key: str = Field(..., title="Key")
    value: str = Field(..., title="Value")


class CubeLeafFilter(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    member: str = Field(..., title="Member")
    operator: str = Field(..., title="Operator")
    values: Optional[List[str]] = Field(None, title="Values")


class Order(Enum):
    asc = "asc"
    desc = "desc"


class Direction(Enum):
    asc = "asc"
    desc = "desc"


class CubeOrderItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: Optional[str] = Field(None, title="Id")
    member: Optional[str] = Field(None, title="Member")
    desc: Optional[bool] = Field(None, title="Desc")
    direction: Optional[Direction] = Field(None, title="Direction")


class CubeTimeDimension(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    dimension: str = Field(..., title="Dimension")
    dateRange: Optional[Union[str, List[str]]] = Field(None, title="Daterange")
    compareDateRange: Optional[List[List[str]]] = Field(None, title="Comparedaterange")
    granularity: Optional[str] = Field(None, title="Granularity")


class DataItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    data: Dict[str, Any] = Field(..., title="Data")


class DataItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    data: Dict[str, Any] = Field(..., title="Data")
    id: str = Field(..., title="Id")
    created_on: DateTime = Field(..., title="Created On")
    modified_on: DateTime = Field(..., title="Modified On")


class DatalayerEnableBody(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    from_date: Optional[Date] = Field(
        None,
        description="Date from which the data is synced. Defaults to one year ago when omitted.",
        title="From Date",
    )
    fiscal_years_back: Optional[conint(ge=0)] = Field(
        None,
        description="Sync from the start of the book year this many fiscal years back (accounting only). 0 syncs from the start of the current fiscal year.",
        title="Fiscal Years Back",
    )
    full_history: Optional[bool] = Field(
        False,
        description="Sync the whole history, without a lower date bound. Mirrors the 'infinite' automatic-activation policy.",
        title="Full History",
    )
    entity_filter: Optional[Dict[str, List[str]]] = Field(
        None,
        description="Restrict the datalayer to specific entities of the connection instead of every one the end-user selected, keyed by entity kind applicable to the connection's vertical (e.g. `folder_ids`, `location_ids`). The ids are validated against the ones available on the connection when the sync runs. Omit to sync every selected entity.",
        title="Entity Filter",
    )


class DatalayerRefreshBody(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    force_from_date: Optional[Date] = Field(
        None,
        description="Force the refresh to (re)sync data from this date, overriding the flow configuration.",
        title="Force From Date",
    )


class DatalayerStatus(Enum):
    not_enabled = "not_enabled"
    syncing = "syncing"
    ready = "ready"


class DatastoreColumn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    title: str = Field(..., title="Title")
    type: str = Field(..., title="Type")
    optional: Optional[bool] = Field(False, title="Optional")


class DatastoreDef(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    columns: List[DatastoreColumn] = Field(..., title="Columns")
    search_column: Optional[str] = Field(
        None,
        description="Column name that will be indexed and used in search if any.",
        title="Search Column",
    )


class DatastoreStatus(Enum):
    active = "active"
    inactive = "inactive"


class DiscountType(Enum):
    OFFERED = "OFFERED"
    UNKNOWN = "UNKNOWN"
    LOSS = "LOSS"


class DocumentType(Enum):
    invoice = "invoice"
    entry = "entry"


class EcommerceFulfillmentType(Enum):
    FBA = "FBA"
    FBM = "FBM"


class EmployeeItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique id of the employee in the accounting system.",
        title="Id",
    )
    name: str = Field(
        ...,
        description="Full name or legal name of the employee as it appears in the accounting system.",
        title="Name",
    )
    first_name: Optional[str] = Field(
        None, description="First name of the employee.", title="First Name"
    )
    last_name: Optional[str] = Field(
        None, description="Last name of the employee.", title="Last Name"
    )
    function: Optional[str] = Field(
        None, description="Job title or function of the employee.", title="Function"
    )
    phone: Optional[str] = Field(
        None, description="Phone number of the employee.", title="Phone"
    )
    mobile: Optional[str] = Field(
        None, description="Mobile phone number of the employee.", title="Mobile"
    )
    email: Optional[str] = Field(
        None, description="Email address of the employee.", title="Email"
    )
    active: Optional[bool] = Field(
        True, description="Indicates if the employee is active.", title="Active"
    )
    reference: Optional[str] = Field(
        None,
        description="External reference or identifier for the employee.",
        title="Reference",
    )
    account_number: Optional[str] = Field(
        None,
        description="Ledger account number assigned to the employee in the accounting system as it will appear in the official accounting export file (FEC, SIE, iXBRL, etc.)",
        title="Account Number",
    )


class EnableFlowConsumer(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    integrationids: Optional[List[str]] = Field(
        [],
        description="List of integrationids of the connections that will be used to enable the flow",
        title="Integrationids",
    )
    triggerid: Optional[str] = Field("trigger-1", title="Triggerid")
    cronschedule: Optional[str] = Field(None, title="Cronschedule")
    data: Optional[Dict[str, Any]] = Field(
        {},
        description="Object containing the configuration of the flow for the consumer",
        title="Data",
    )


class EntryLineType(Enum):
    customer_account = "customer_account"
    supplier_account = "supplier_account"
    employee_account = "employee_account"
    general_account = "general_account"


class ErrorInfo(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    error_code: str = Field(..., title="Error Code")
    status: str = Field(..., title="Status")
    title: str = Field(..., title="Title")
    description: Optional[str] = Field(None, title="Description")


class ExecutionConsumerResult(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    consumerid: str = Field(..., title="Consumerid")
    status: str = Field(..., title="Status")


class ExecutionResult(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    column: str = Field(..., title="Column")
    title: Optional[str] = Field(None, title="Title")
    data: List = Field(..., title="Data")
    type: str = Field(..., title="Type")


class ExecutionType(Enum):
    code = "code"
    module = "module"


class ExecutionWarning(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    code: str = Field(..., title="Code")
    message: str = Field(..., title="Message")
    blocking: Optional[bool] = Field(False, title="Blocking")


class ExpenseLineItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    total: float = Field(
        ...,
        description="Total amount for the line including taxes.",
        examples=[242.0],
        title="Total",
    )
    untaxed_amount: float = Field(
        ...,
        description="Untaxed amount for the line.",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ..., description="Tax amount for the line.", examples=[42.0], title="Tax Amount"
    )
    description: Optional[str] = Field(
        None,
        description="Line description",
        examples=["Office supplies"],
        title="Description",
    )
    account_number: str = Field(
        ...,
        description="General ledger account number to book the expense.",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    tax_rate: float = Field(
        ...,
        description="Tax rate (e.g., 21.0 for 21%).",
        examples=[21.0],
        title="Tax Rate",
    )
    tax_id: str = Field(
        ...,
        description="Tax identifier to validate against the provided tax_rate.",
        examples=["<accounting-tax-code>"],
        title="Tax Id",
    )


class FECItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    JournalCode: str = Field(..., title="Journalcode")
    JournalLib: str = Field(..., title="Journallib")
    EcritureNum: str = Field(..., title="Ecriturenum")
    EcritureDate: Date = Field(..., title="Ecrituredate")
    CompteNum: str = Field(..., title="Comptenum")
    CompteLib: str = Field(..., title="Comptelib")
    CompAuxNum: str = Field(..., title="Compauxnum")
    CompAuxLib: str = Field(..., title="Compauxlib")
    PieceRef: str = Field(..., title="Pieceref")
    PieceDate: Date = Field(..., title="Piecedate")
    Debit: float = Field(..., title="Debit")
    Credit: float = Field(..., title="Credit")
    EcritureLet: str = Field(..., title="Ecriturelet")
    DateLet: Optional[Date] = Field(..., title="Datelet")
    ValidDate: Optional[Date] = Field(..., title="Validdate")
    Montantdevise: float = Field(..., title="Montantdevise")
    Idevise: str = Field(..., title="Idevise")


class FeesType(Enum):
    shipping = "shipping"
    other = "other"


class FieldItemType(Enum):
    fixed = "fixed"
    api = "api"


class FieldItemValue(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., title="Id")
    label: str = Field(..., title="Label")


class FieldRef(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: Optional[str] = Field(
        None,
        description="Technical id in the target software",
        examples=["<target-software-id>"],
        title="Id",
    )
    model: Optional[str] = Field(
        None,
        description="Name of the model/entity in the target software",
        examples=["invoice"],
        title="Model",
    )
    name: Optional[str] = Field(
        None,
        description="Value the field in the target software",
        examples=["Example Company S.A."],
        title="Name",
    )


class FlowDataStoreItemInput(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: Optional[str] = Field(None, title="Id")
    name: str = Field(..., title="Name")
    status: Optional[DatastoreStatus] = "active"
    definition: DatastoreDef


class FlowDataStoreItemOutput(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: Optional[str] = Field(None, title="Id")
    name: str = Field(..., title="Name")
    status: Optional[DatastoreStatus] = "active"
    definition: DatastoreDef


class FlowExecutionChain(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")


class FlowExecutionCode(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    code: str = Field(..., title="Code")


class FolderAddressItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    street: Optional[str] = Field(None, title="Street")
    number: Optional[str] = Field(None, title="Number")
    box: Optional[str] = Field(None, title="Box")
    postal_code: Optional[str] = Field(None, title="Postal Code")
    city: Optional[str] = Field(None, title="City")
    country: Optional[str] = Field(
        None, description="Format: ISO 3166-1 codes.", title="Country"
    )


class FolderItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., title="Id")
    name: str = Field(..., title="Name")
    selected: Optional[bool] = Field(
        True,
        description="Indicates whether the folder was selected during the Chift activation process. Only selected folders can be used in subsequent requests. Requests targeting a folder that was not selected will be rejected. If only one folder is selected, it will be used by default when no folder is specified in the request.",
        title="Selected",
    )
    active: Optional[bool] = Field(
        True,
        description="Indicates whether the folder is currently active in the target accounting software. Requests targeting an inactive folder will fail on the target software side.",
        title="Active",
    )
    vat: Optional[str] = Field(None, title="Vat")
    company_number: Optional[str] = Field(None, title="Company Number")
    main_currency: Optional[str] = Field(
        None, description="Format: ISO 3166-1 codes.", title="Main Currency"
    )
    addresses: Optional[List[FolderAddressItem]] = Field(None, title="Addresses")


class HTTPMethod(Enum):
    GET = "GET"
    POST = "POST"
    PUT = "PUT"
    PATCH = "PATCH"
    DELETE = "DELETE"
    HEAD = "HEAD"
    OPTIONS = "OPTIONS"


class ImageItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the image in the target software",
        examples=["<commerce-image-id>"],
        title="Id",
    )
    main_image: bool = Field(..., title="Main Image")
    url: str = Field(
        ..., examples=["https://example.com/images/product.png"], title="Url"
    )


class ImageType(Enum):
    icon = "icon"
    logo = "logo"


class ImportMode(Enum):
    normal = "normal"
    force_reimport = "force_reimport"
    IMPORT_ONLY_MISSING_DAYS = "IMPORT_ONLY_MISSING_DAYS"
    REIMPORT_ENTIRE_PERIOD = "REIMPORT_ENTIRE_PERIOD"


class IntegationStatus(Enum):
    active = "active"
    inactive = "inactive"


class IntegrationsPostConnectionItemConfig(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    enabled: Optional[bool] = Field(False, title="Enabled")
    allow_multiple: Optional[bool] = Field(False, title="Allow Multiple")


class InventoryDetailsItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    location: CommerceLocationItem
    available_quantity: float = Field(..., examples=[10.0], title="Available Quantity")


class InventoryDetailsUpdate(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    location_id: str = Field(
        ...,
        description="Technical id of the location in Chift",
        examples=["<commerce-location-id>"],
        title="Location Id",
    )
    available_quantity: confloat(ge=0.0) = Field(
        ..., examples=[10.0], title="Available Quantity"
    )


class InvoiceApprovalStatus(Enum):
    unknown = "unknown"
    pending = "pending"
    approved = "approved"
    rejected = "rejected"


class InvoiceApprovalStatusFilter(Enum):
    pending = "pending"
    approved = "approved"
    rejected = "rejected"


class InvoiceCorrection(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    sale_invoice_correction_tax_code: Optional[str] = Field(
        None,
        description="Tax code to apply on the correction line when the invoice is a sale (customer invoice). Required only when invoice_correction is used for a sale invoice or refund.",
        examples=["<accounting-tax-code>"],
        title="Sale Invoice Correction Tax Code",
    )
    purchase_invoice_correction_tax_code: Optional[str] = Field(
        None,
        description="Tax code to apply on the correction line when the invoice is a purchase (supplier invoice). Required only when invoice_correction is used for a purchase invoice or refund.",
        examples=["<accounting-tax-code>"],
        title="Purchase Invoice Correction Tax Code",
    )
    invoice_correction_credit_account_number: Optional[str] = Field(
        None,
        description="Account number to use for the correction line on a sale (customer invoice).",
        examples=["<accounting-account-number>"],
        title="Invoice Correction Credit Account Number",
    )
    invoice_correction_debit_account_number: Optional[str] = Field(
        None,
        description="Account number to use for the correction line on a purchase (supplier invoice).",
        examples=["<accounting-account-number>"],
        title="Invoice Correction Debit Account Number",
    )


class InvoiceItemDueDatesOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    due_date: Date = Field(
        ...,
        description="Due date of the item.",
        examples=["2026-02-14"],
        title="Due Date",
    )
    payment_method: Optional[str] = Field(
        None,
        description="Payment method used to pay the invoice on that due date.",
        examples=["bank_transfer"],
        title="Payment Method",
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical ID of the payment method used to pay the invoice on that due date.",
        examples=["<accounting-payment-method-id>"],
        title="Payment Method Id",
    )
    amount: float = Field(
        ...,
        description="Amount due for the invoice on that due date. A positive amount represents debit on customer invoices and supplier refunds or credit on supplier invoices and customer refunds. A negative amount represents credit on customer invoices and supplier refunds or debit on supplier invoices and customer refunds.",
        examples=[242.0],
        title="Amount",
    )


class InvoiceLineItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    description: Optional[str] = Field(
        None,
        description="Line description",
        examples=["Consulting services"],
        title="Description",
    )
    unit_price: float = Field(
        ..., description="Unit price excl. taxes", examples=[100.0], title="Unit Price"
    )
    quantity: float = Field(
        ..., description="Quantity", examples=[2.0], title="Quantity"
    )
    discount_amount: Optional[float] = Field(
        0.0,
        description="Line discount amount excl. taxes, (unit_price * quantity) - discount_amount = untaxed_amount",
        examples=[0.0],
        title="Discount Amount",
    )
    tax_amount: float = Field(
        ..., description="Line total taxes amount", examples=[42.0], title="Tax Amount"
    )
    untaxed_amount: float = Field(
        ...,
        description="Line total untaxed amount",
        examples=[200.0],
        title="Untaxed Amount",
    )
    total: float = Field(
        ...,
        description="Line total amount incl. taxes, total = tax_amount + untaxed_amount",
        examples=[242.0],
        title="Total",
    )
    tax_rate: Optional[float] = Field(
        None, description="Tax rate (e.g. 21.0)", examples=[21.0], title="Tax Rate"
    )
    account_number: Optional[str] = Field(
        None,
        description="Number of the accounting account used (e.g. 701000)",
        examples=["<invoicing-account-number>"],
        title="Account Number",
    )
    tax_id: Optional[str] = Field(
        None,
        description="Technical id of the tax in Chift",
        examples=["<invoicing-tax-id>"],
        title="Tax Id",
    )
    tax_exemption_reason: Optional[str] = Field(
        None,
        description="Tax exemption reason",
        examples=["Export outside EU"],
        title="Tax Exemption Reason",
    )
    unit_of_measure: Optional[str] = Field(
        None,
        description="Unit of measure matching target sofware name",
        examples=["unit"],
        title="Unit Of Measure",
    )
    product_id: Optional[str] = Field(
        None,
        description="Technical id of the product in Chift",
        examples=["<invoicing-product-id>"],
        title="Product Id",
    )
    product_code: Optional[str] = Field(
        None,
        description="Product reference",
        examples=["SKU-0001"],
        title="Product Code",
    )
    product_name: Optional[str] = Field(
        None,
        description="Product name",
        examples=["Consulting services"],
        title="Product Name",
    )


class InvoiceLineItemInMonoAnalyticPlan(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    line_number: Optional[int] = Field(
        1,
        description="Optional line sequence number used to preserve the order of invoice items.",
        title="Line Number",
    )
    unit_price: float = Field(
        ...,
        description="Unit price of the item, before tax.",
        examples=[100.0],
        title="Unit Price",
    )
    unit_of_measure: Optional[str] = Field(
        None,
        description="A list of default units can be used for which an advanced mapping has been done (units: 'hour', 'day', 'cm', 'm2', 'm3', 'm', 'km', 'mile', 'g', 'kg', 'ton', 'box', 'pallet', 'roll', 'liter', 'unit'). Custom units of measure must have an identical name in the target software.",
        examples=["unit"],
        title="Unit Of Measure",
    )
    quantity: float = Field(
        ...,
        description="Quantity of items. This is the amount of units sold or purchased.",
        examples=[2.0],
        title="Quantity",
    )
    untaxed_amount: float = Field(
        ...,
        description="Amount excluding tax for the line item (i.e., unit_price × quantity).",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_rate: float = Field(
        ...,
        description="Tax rate used for the line (e.g., 21.0).",
        examples=[21.0],
        title="Tax Rate",
    )
    tax_amount: float = Field(
        ...,
        description="Tax amount calculated for the line (i.e. untaxed_amount * tax_rate).",
        examples=[42.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total amount for the item, including tax (untaxed_amount + tax_amount).",
        examples=[242.0],
        title="Total",
    )
    account_number: constr(min_length=1) = Field(
        ...,
        description="Ledger account number (of type general) used for the line (e.g., 701000).",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    tax_code: constr(min_length=1) = Field(
        ...,
        description="Indicates the tax code used for the line. This is the Id of the Tax Code in the accounting software.",
        examples=["<accounting-tax-code>"],
        title="Tax Code",
    )
    description: Optional[str] = Field(
        "",
        description="Text description for this line item.",
        examples=["Consulting services"],
        title="Description",
    )
    analytic_account: Optional[str] = Field(
        None,
        description="Analytic account in the default analytic plan.",
        examples=["<accounting-analytic-account-id>"],
        title="Analytic Account",
    )


class InvoiceLineItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    description: Optional[str] = Field(
        None,
        description="Line description",
        examples=["Consulting services"],
        title="Description",
    )
    unit_price: float = Field(
        ..., description="Unit price excl. taxes", examples=[100.0], title="Unit Price"
    )
    quantity: float = Field(
        ..., description="Quantity", examples=[2.0], title="Quantity"
    )
    discount_amount: Optional[float] = Field(
        0.0,
        description="Line discount amount excl. taxes, (unit_price * quantity) - discount_amount = untaxed_amount",
        examples=[0.0],
        title="Discount Amount",
    )
    tax_amount: float = Field(
        ..., description="Line total taxes amount", examples=[42.0], title="Tax Amount"
    )
    untaxed_amount: float = Field(
        ...,
        description="Line total untaxed amount",
        examples=[200.0],
        title="Untaxed Amount",
    )
    total: float = Field(
        ...,
        description="Line total amount incl. taxes, total = tax_amount + untaxed_amount",
        examples=[242.0],
        title="Total",
    )
    tax_rate: Optional[float] = Field(
        None, description="Tax rate (e.g. 21.0)", examples=[21.0], title="Tax Rate"
    )
    account_number: Optional[str] = Field(
        None,
        description="Number of the accounting account used (e.g. 701000)",
        examples=["<invoicing-account-number>"],
        title="Account Number",
    )
    tax_id: Optional[str] = Field(
        None,
        description="Technical id of the tax in Chift",
        examples=["<invoicing-tax-id>"],
        title="Tax Id",
    )
    tax_exemption_reason: Optional[str] = Field(
        None,
        description="Tax exemption reason",
        examples=["Export outside EU"],
        title="Tax Exemption Reason",
    )
    unit_of_measure: Optional[str] = Field(
        None,
        description="Unit of measure matching target sofware name",
        examples=["unit"],
        title="Unit Of Measure",
    )
    product_id: Optional[str] = Field(
        None,
        description="Technical id of the product in Chift",
        examples=["<invoicing-product-id>"],
        title="Product Id",
    )
    product_code: Optional[str] = Field(
        None,
        description="Product reference",
        examples=["SKU-0001"],
        title="Product Code",
    )
    product_name: Optional[str] = Field(
        None,
        description="Product name",
        examples=["Consulting services"],
        title="Product Name",
    )
    analytic_distribution: Optional[List[AnalyticDistributionInvoicing]] = Field(
        [],
        description="List of analytic distributions for the line item. Each distribution specifies an analytic plan and the associated analytic accounts with their percentages.",
        title="Analytic Distribution",
    )


class InvoiceLineItemOutMonoAnalyticPlan(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    line_number: Optional[int] = Field(
        1,
        description="Optional line sequence number used to preserve the order of invoice items.",
        title="Line Number",
    )
    unit_price: float = Field(
        ...,
        description="Unit price of the item, before tax.",
        examples=[100.0],
        title="Unit Price",
    )
    unit_of_measure: Optional[str] = Field(
        None,
        description="A list of default units can be used for which an advanced mapping has been done (units: 'hour', 'day', 'cm', 'm2', 'm3', 'm', 'km', 'mile', 'g', 'kg', 'ton', 'box', 'pallet', 'roll', 'liter', 'unit'). Custom units of measure must have an identical name in the target software.",
        examples=["unit"],
        title="Unit Of Measure",
    )
    quantity: float = Field(
        ...,
        description="Quantity of items. This is the amount of units sold or purchased.",
        examples=[2.0],
        title="Quantity",
    )
    untaxed_amount: float = Field(
        ...,
        description="Amount excluding tax for the line item (i.e., unit_price × quantity).",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_rate: float = Field(
        ...,
        description="Tax rate used for the line (e.g., 21.0).",
        examples=[21.0],
        title="Tax Rate",
    )
    tax_amount: float = Field(
        ...,
        description="Tax amount calculated for the line (i.e. untaxed_amount * tax_rate).",
        examples=[42.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total amount for the item, including tax (untaxed_amount + tax_amount).",
        examples=[242.0],
        title="Total",
    )
    account_number: Optional[str] = Field(
        None,
        description="Ledger account number (of type general) used for the line (e.g., 701000).",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    tax_code: Optional[str] = Field(
        None,
        description="Indicates the tax code used for the line. This is the Id of the Tax Code in the accounting software.",
        examples=["<accounting-tax-code>"],
        title="Tax Code",
    )
    description: Optional[str] = Field(
        "",
        description="Text description for this line.",
        examples=["Consulting services"],
        title="Description",
    )
    analytic_account: Optional[str] = Field(
        None,
        description="Analytic account in the default analytic plan.",
        examples=["<accounting-analytic-account-id>"],
        title="Analytic Account",
    )


class InvoiceStatus(Enum):
    cancelled = "cancelled"
    draft = "draft"
    posted = "posted"
    paid = "paid"


class InvoiceStatusIn(Enum):
    draft = "draft"
    posted = "posted"


class InvoiceStatusOut(Enum):
    cancelled = "cancelled"
    draft = "draft"
    posted = "posted"
    paid = "paid"


class InvoiceType(Enum):
    customer_invoice = "customer_invoice"
    customer_refund = "customer_refund"
    supplier_invoice = "supplier_invoice"
    supplier_refund = "supplier_refund"


class InvoicingCreateInvoiceType(Enum):
    customer_invoice = "customer_invoice"
    customer_refund = "customer_refund"
    supplier_invoice = "supplier_invoice"
    supplier_refund = "supplier_refund"


class InvoicingDocumentType(Enum):
    customer_document = "customer_document"
    supplier_document = "supplier_document"
    employee_expense = "employee_expense"


class InvoicingInvoiceType(Enum):
    customer_invoice = "customer_invoice"
    customer_refund = "customer_refund"
    supplier_invoice = "supplier_invoice"
    supplier_refund = "supplier_refund"
    all = "all"


class InvoicingPaymentItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    amount: float = Field(
        ..., description="Payment amount", examples=[242.0], title="Amount"
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="The technical ID of the payment method in the invoicing system",
        examples=["<invoicing-payment-method-id>"],
        title="Payment Method Id",
    )
    invoice_id: str = Field(
        ...,
        description="The technical ID of the invoice in the invoicing system to which the payment will be linked",
        examples=["<invoicing-invoice-id>"],
        title="Invoice Id",
    )
    date: Date = Field(
        ...,
        description="The date of the payment",
        examples=["2026-02-14"],
        title="Date",
    )
    description: Optional[str] = Field(
        None,
        description="Description of the payment",
        examples=["Payment for invoice INV-2026-0001"],
        title="Description",
    )
    currency: str = Field(
        ..., description="Currency of the payment", examples=["EUR"], title="Currency"
    )


class InvoicingPaymentItemUpdate(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    amount: Optional[float] = Field(
        None, description="Payment amount", examples=[242.0], title="Amount"
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="The technical ID of the payment method in the invoicing system",
        examples=["<invoicing-payment-method-id>"],
        title="Payment Method Id",
    )
    date: Optional[Date] = Field(
        None,
        description="The date of the payment",
        examples=["2026-02-14"],
        title="Date",
    )
    description: Optional[str] = Field(
        None,
        description="Description of the payment",
        examples=["Payment for invoice INV-2026-0001"],
        title="Description",
    )


class IssueEvent(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., title="Id")
    execution_id: str = Field(..., title="Execution Id")
    created_on: DateTime = Field(..., title="Created On")
    context: Optional[Dict[str, Any]] = Field(None, title="Context")
    message: Optional[str] = Field(None, title="Message")


class IssueLevel(Enum):
    error = "error"
    warning = "warning"


class IssueStatus(Enum):
    open = "open"
    closed = "closed"
    all = "all"


class ItemAttachmentInfoAttachment(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    filename: Optional[str] = Field(
        None,
        description="The name of the attachment.",
        examples=["invoice-example.pdf"],
        title="Filename",
    )
    url: Optional[str] = Field(
        None,
        description="Direct download URL for the attachment file. Only populated when the parent attachments_info.status is 'yes'. When status is 'yes_to_request', this field is null and the file must be retrieved via GET /accounting/attachments with the appropriate type and document_id, which returns the content as a base64-encoded string.",
        examples=["https://example.com/attachments/invoice-example.pdf"],
        title="Url",
    )


class ItemAttachmentInfoStatus(Enum):
    unknown = "unknown"
    yes = "yes"
    yes_to_request = "yes_to_request"
    no = "no"


class JournalInType(Enum):
    customer_invoice = "customer_invoice"
    customer_refund = "customer_refund"
    supplier_invoice = "supplier_invoice"
    supplier_refund = "supplier_refund"
    bank = "bank"
    cash = "cash"
    miscellaneous_operation = "miscellaneous_operation"


class JournalItemDueDatesOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    due_date: Date = Field(
        ...,
        description="Due date of the item.",
        examples=["2026-02-14"],
        title="Due Date",
    )
    payment_method: Optional[str] = Field(
        None,
        description="Payment method used to pay the entry on that due date.",
        examples=["bank_transfer"],
        title="Payment Method",
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical ID of the payment method used to pay the entry on that due date.",
        examples=["<accounting-payment-method-id>"],
        title="Payment Method Id",
    )
    debit: Optional[float] = Field(
        0,
        description="Debit of the item on this due date.",
        examples=[242.0],
        title="Debit",
    )
    credit: Optional[float] = Field(
        0,
        description="Credit of the item on this due date.",
        examples=[0.0],
        title="Credit",
    )


class JournalItemMonoAnalyticPlan(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    account_number: constr(min_length=1) = Field(
        ...,
        description="Depending on account_type, if the account_type is 'general_account', this is the general ledger account number used to book the journal item against. If the account_type is 'customer_account', 'supplier_account' or 'employee_account', this is the thirdparty_id (id of the customer, supplier or employee in the accounting system).",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    partner_id: Optional[str] = Field(
        None,
        description="Id of the thirdparty (customer, supplier or employee) in the accounting system if the journal item relates to a thirdparty (e.g., an invoice). The thirdparty line is typically the invoice's total line, i.e. the accounting line tied to a due date / payment term.",
        examples=["<accounting-client-id>"],
        title="Partner Id",
    )
    description: Optional[str] = Field(
        None,
        description="Text description for this journal item. This is typically used to provide additional context or information.",
        examples=["Consulting services"],
        title="Description",
    )
    debit: float = Field(
        ...,
        description="Amount to debit on the account. Must be 0 if credit field is not 0",
        examples=[242.0],
        title="Debit",
    )
    credit: float = Field(
        ...,
        description="Amount to credit on the account. Must be 0 if debit field is not 0",
        examples=[0.0],
        title="Credit",
    )
    currency: constr(min_length=1) = Field(
        ...,
        description="Currency code (e.g., USD, EUR) used for the journal item amounts.",
        examples=["EUR"],
        title="Currency",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, applicable at the date of the journal item. Required when currency is different from the folder's default currency. Amount in folder currency = amount in foreign currency * currency_exchange_rate.",
        title="Currency Exchange Rate",
    )
    id: str = Field(
        ...,
        description="Unique id of the journal item instance in the accounting system.",
        examples=["<this-journal-item-id>"],
        title="Id",
    )
    partner_name: Optional[str] = Field(
        None,
        description="Name of the thirdparty (customer, supplier or employee) linked to the journal item if journal item relates to a thirdparty (e.g., an invoice).",
        examples=["Example Company S.A."],
        title="Partner Name",
    )
    account_name: str = Field(
        ...,
        description="Name or label of the ledger account used to book the journal item against.",
        examples=["Consulting services"],
        title="Account Name",
    )
    matching_numbers: Optional[List[str]] = Field(
        [],
        description="List of matching numbers used to link this journal item to other entries in the accounting system, for reconciliation/lettering. Lines on the same ledger account, in the same book year, carrying the same matching number are lettered (matched) together; when a partner_id is set it is also part of that link. A thirdparty line with debit or credit and no matching number is an open (outstanding) item, and stays outstanding until the matched set balances (sum of debit == sum of credit) within the same book year.",
        examples=[["<accounting-matching-number>"]],
        title="Matching Numbers",
    )
    analytic_account: Optional[str] = Field(
        None,
        description="Technical ID of the analytic account in the default analytic plan.",
        examples=["<accounting-analytic-account-id>"],
        title="Analytic Account",
    )
    analytic_account_name: Optional[str] = Field(
        None,
        description="Name of the analytic account in the default analytic plan.",
        examples=["Marketing"],
        title="Analytic Account Name",
    )


class JournalType(Enum):
    customer_invoice = "customer_invoice"
    customer_refund = "customer_refund"
    supplier_invoice = "supplier_invoice"
    supplier_refund = "supplier_refund"
    financial_operation = "financial_operation"
    miscellaneous_operation = "miscellaneous_operation"
    unknown = "unknown"


class LedgerAccountItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: constr(min_length=1) = Field(
        ...,
        description="Name or label of the ledger account as it appears in the accounting system.",
        examples=["Consulting services"],
        title="Name",
    )
    number: constr(min_length=3) = Field(
        ...,
        description="The account number in the chart of accounts. Must be unique within the accounting folder. In some countries (e.g., France, Belgium, Spain), the number structure is constrained by local accounting rules and must follow official charts of accounts (e.g., Plan Comptable Général).",
        examples=["700000"],
        title="Number",
    )
    type: Optional[AccountItemType] = "other"


class LinkItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    url: str = Field(..., title="Url")


class LinkSyncItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    url: str = Field(..., title="Url")


class LinkedDocumentType(Enum):
    invoice = "invoice"
    other = "other"


class LocalAgentStatus(Enum):
    up = "up"
    down = "down"


class LogoImage(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    integrationid: int = Field(..., title="Integrationid")
    data: str = Field(..., title="Data")


class MCPAuthItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    clientId: str = Field(..., title="Clientid")
    clientSecret: str = Field(..., title="Clientsecret")
    accountId: UUID = Field(..., title="Accountid")
    envId: Optional[UUID] = Field(None, title="Envid")
    marketplaceId: Optional[UUID] = Field(None, title="Marketplaceid")
    consumerId: UUID = Field(..., title="Consumerid")


class MatchingIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    entries: List[str] = Field(
        ...,
        description="List of entries to match. Must contain at least 2 entries. Entries are typically journal entry id, invoice id or financial entry id.",
        examples=[["<this-journal-entry-id>", "<this-invoice-id>"]],
        min_length=2,
        title="Entries",
    )
    partner_id: str = Field(
        ...,
        description="Id of the thirdparty (customer, supplier or employee) in the accounting system . All entries are must be linked to one same partner.",
        examples=["<accounting-client-id>"],
        title="Partner Id",
    )


class MatchingOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    matching_number: str = Field(
        ...,
        description="Unique 'number' of the matching operation in the accounting system.",
        examples=["<this-matching-number>"],
        title="Matching Number",
    )
    balance: float = Field(
        ...,
        description="Balance of the matching operation. This is the sum of all entries in the matching operation. It should be zero if the matching is complete.",
        examples=[0.0],
        title="Balance",
    )


class MiscellaneousOperationLineType(Enum):
    customer_account = "customer_account"
    supplier_account = "supplier_account"
    general_account = "general_account"


class MiscellaneousOperationStatusIn(Enum):
    draft = "draft"
    posted = "posted"


class MiscellaneousOperationStatusOut(Enum):
    cancelled = "cancelled"
    draft = "draft"
    posted = "posted"
    matched = "matched"


class Mode(Enum):
    EAT_IN = "EAT_IN"
    TAKEAWAY = "TAKEAWAY"
    DELIVERY = "DELIVERY"
    UNKNOWN = "UNKNOWN"


class ModifierLine(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    modifier_id: Optional[str] = Field(
        None,
        description="Unique identifier of the modifier line item",
        examples=["<pos-modifier-id>"],
        title="Modifier Id",
    )
    name: str = Field(
        ..., description="Name of the modifier", examples=["Ketchup"], title="Name"
    )
    modifier_group_id: Optional[str] = Field(
        None,
        description="Unique identifier of the modifier group line item",
        examples=["<pos-modifier-group-id>"],
        title="Modifier Group Id",
    )
    quantity: Optional[float] = Field(
        "1",
        description="Quantity of the modifier line item",
        examples=[1],
        title="Quantity",
    )
    unit_price: Optional[float] = Field(
        "0",
        description="Unit price (without tax) of the modifier line item",
        examples=[10],
        title="Unit Price",
    )
    total: Optional[float] = Field(
        "0",
        description="Total amount including tax of the modifier line item",
        examples=[11],
        title="Total",
    )


class ModifierOptionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the modifier option",
        examples=["<this-modifier-option-id>"],
        title="Id",
    )
    name: Optional[str] = Field(
        None,
        description="Name of the modifier option",
        examples=["Extra Cheese"],
        title="Name",
    )
    price: float = Field(
        ..., description="Price of the modifier option", examples=[2.0], title="Price"
    )


class ModifiersItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the modifier",
        examples=["<this-modifier-id>"],
        title="Id",
    )
    name: Optional[str] = Field(
        None, description="Name of the modifier", examples=["Toppings"], title="Name"
    )
    options: List[ModifierOptionItem] = Field(
        ..., description="List of options of the modifier", title="Options"
    )


class MultipleMatchingIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    matchings: List[MatchingIn] = Field(..., title="Matchings")


class MultipleMatchingOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    matching_number: Optional[str] = Field(
        None, examples=["<this-matching-number>"], title="Matching Number"
    )
    processed: bool = Field(..., title="Processed")
    error_msg: Optional[Dict[str, Any]] = Field(None, title="Error Msg")


class NextDocumentNumber(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    bookyear_name: Optional[str] = Field(None, title="Bookyear Name")
    next_document_number: Optional[str] = Field(None, title="Next Document Number")
    start_date: Optional[Date] = Field(None, title="Start Date")
    end_date: Optional[Date] = Field(None, title="End Date")


class ObjectivesItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    total: float = Field(
        ...,
        description="Total amount including tax of the sales",
        examples=[10],
        title="Total",
    )
    tax_amount: float = Field(
        ..., description="Total amount of the taxes", examples=[1], title="Tax Amount"
    )
    date: Date = Field(
        ..., description="Date of the objective", examples=["2025-01-01"], title="Date"
    )


class OperationType(Enum):
    incoming_transfer = "incoming_transfer"
    outgoing_transfer = "outgoing_transfer"
    card = "card"
    direct_debit = "direct_debit"
    direct_debit_collection = "direct_debit_collection"
    direct_debit_hold = "direct_debit_hold"
    fee = "fee"
    cheque = "cheque"
    recall = "recall"
    swift_income = "swift_income"
    pay_later = "pay_later"
    financing_installment = "financing_installment"
    cashback = "cashback"
    interest = "interest"
    other = "other"


class OpportunityStatus(Enum):
    open = "open"
    won = "won"
    lost = "lost"
    cancelled = "cancelled"
    closed = "closed"


class OrderCustomerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    first_name: Optional[str] = Field(None, examples=["John"], title="First Name")
    last_name: Optional[str] = Field(None, examples=["Doe"], title="Last Name")
    phone: Optional[str] = Field(None, examples=["+32 2 000 00 00"], title="Phone")
    internal_notes: Optional[str] = Field(
        None,
        examples=["Preferred contact: accounts payable team."],
        title="Internal Notes",
    )
    email: str = Field(..., examples=["john.doe@example.com"], title="Email")


class OrderCustomerItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    first_name: Optional[str] = Field(None, examples=["John"], title="First Name")
    last_name: Optional[str] = Field(None, examples=["Doe"], title="Last Name")
    phone: Optional[str] = Field(None, examples=["+32 2 000 00 00"], title="Phone")
    internal_notes: Optional[str] = Field(
        None,
        examples=["Preferred contact: accounts payable team."],
        title="Internal Notes",
    )
    email: Optional[str] = Field(None, examples=["john.doe@example.com"], title="Email")
    id: str = Field(
        ...,
        description="Technical id of the customer in Chift",
        examples=["<this-customer-id>"],
        title="Id",
    )


class OrderLineItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    variant_id: str = Field(
        ...,
        description="Technical id of the product variant in Chift",
        examples=["<commerce-variant-id>"],
        title="Variant Id",
    )
    quantity: int = Field(..., examples=[2], title="Quantity")
    tax_rate: float = Field(
        ...,
        description="Indicates the tax rate used for the line (e.g. 21.0).",
        examples=[21.0],
        title="Tax Rate",
    )
    unit_price: float = Field(
        ...,
        description="Unit price of the product without taxes.",
        examples=[100.0],
        title="Unit Price",
    )


class OrderLineProductVariantItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the product variant in Chift",
        examples=["<commerce-variant-id>"],
        title="Id",
    )
    sku: Optional[str] = Field(None, examples=["TSHIRT-M"], title="Sku")
    name: str = Field(..., examples=["T-Shirt - M"], title="Name")
    categories: Optional[List[CommerceProductCategoryItem]] = Field(
        [], title="Categories"
    )


class OrderPaymentMethods(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the payment method in the eCommerce",
        examples=["<commerce-payment-method-id>"],
        title="Id",
    )
    name: Optional[str] = Field(
        None,
        description="Name of the payment method",
        examples=["Credit card"],
        title="Name",
    )


class OrderStatus(Enum):
    cancelled_unpaid = "cancelled_unpaid"
    cancelled = "cancelled"
    draft = "draft"
    confirmed = "confirmed"
    shipped = "shipped"
    refunded = "refunded"


class OrderTransactions(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the transaction in the eCommerce",
        examples=["<commerce-transaction-id>"],
        title="Id",
    )
    created_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Created On"
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical id of the payment method in the eCommerce",
        examples=["<commerce-payment-method-id>"],
        title="Payment Method Id",
    )
    payment_method_name: Optional[str] = Field(
        None,
        description="Name of the payment method",
        examples=["Credit card"],
        title="Payment Method Name",
    )
    amount: float = Field(..., examples=[242.0], title="Amount")
    status: CommerceTransactionStatus


class OriginalOutstandingItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: Optional[str] = Field(
        None,
        description="Unique id of the original outstanding item in the accounting system.",
        examples=["<accounting-outstanding-item-id>"],
        title="Id",
    )
    number: Optional[str] = Field(
        None,
        description="Number of the original outstanding item.",
        examples=["INV-2026-0001"],
        title="Number",
    )
    journal_id: Optional[str] = Field(
        None,
        description="Id of the journal associated with the original outstanding item.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    journal_type: Optional[JournalType] = Field(
        None,
        description="Type of the journal associated with the original outstanding item.",
    )
    date: Optional[Date] = Field(
        None,
        description="Date of the original outstanding item.",
        examples=["2026-01-15"],
        title="Date",
    )
    due_date: Optional[Date] = Field(
        None,
        description="Due date of the original outstanding item.",
        examples=["2026-02-14"],
        title="Due Date",
    )
    reference: Optional[str] = Field(
        None,
        description="Reference of the original outstanding item.",
        examples=["REF-0001"],
        title="Reference",
    )


class OutstandingItemDueDatesOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    due_date: Date = Field(
        ...,
        description="Due date of the outstanding item.",
        examples=["2026-02-14"],
        title="Due Date",
    )
    payment_method: Optional[str] = Field(
        None,
        description="Payment method used to pay the outstanding on that due date.",
        examples=["bank_transfer"],
        title="Payment Method",
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical ID of the payment method used to pay the outstanding on that due date.",
        examples=["<accounting-payment-method-id>"],
        title="Payment Method Id",
    )
    amount: float = Field(
        ...,
        description="Amount due for the outstanding on that due date.",
        examples=[242.0],
        title="Amount",
    )


class OutstandingType(Enum):
    client = "client"
    supplier = "supplier"


class PMSAccountingTransactionOriginType(Enum):
    payment = "payment"
    invoice = "invoice"
    order = "order"


class PMSAccountingTransactionStatus(Enum):
    draft = "draft"
    posted = "posted"


class PMSAccountingTransactionType(Enum):
    payment = "payment"
    room = "room"
    addon = "addon"
    invoice = "invoice"
    pos = "pos"
    fee = "fee"
    tax = "tax"
    reservation = "reservation"
    other = "other"


class PMSLocationItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the location",
        examples=["<this-location-id>"],
        title="Id",
    )
    name: str = Field(
        ...,
        description="Name given to the location",
        examples=["Restaurant de la Paix"],
        title="Name",
    )
    timezone: Optional[str] = Field(
        None,
        description="Indicates the timezone of the location. TZ notation, e.g. Europe/Brussels .",
        examples=["Europe/Paris"],
        title="Timezone",
    )
    address: Optional[AddressItem] = Field(None, description="Address of the location")


class PMSStates(Enum):
    consumed = "consumed"
    closed = "closed"


class POSCustomerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the customer",
        examples=["<this-customer-id>"],
        title="Id",
    )
    first_name: Optional[str] = Field(
        None,
        description="First name of the customer",
        examples=["John"],
        title="First Name",
    )
    last_name: Optional[str] = Field(
        None,
        description="Last name of the customer",
        examples=["Doe"],
        title="Last Name",
    )
    name: str = Field(
        ...,
        description="Name (first name + last name) of the customer",
        examples=["John Doe"],
        title="Name",
    )
    phone: Optional[str] = Field(
        None,
        description="Phone number (mobile, home, work, ...) of the customer",
        examples=["+32 4 000 00 00"],
        title="Phone",
    )
    email: Optional[str] = Field(
        None,
        description="Email address of the customer",
        examples=["john.doe@example.com"],
        title="Email",
    )
    created_on: Optional[DateTime] = Field(
        None,
        description="Date and time when the customer was created",
        examples=["2025-01-01T00:00:00Z"],
        title="Created On",
    )
    addresses: Optional[List[AddressItem]] = Field(
        [], description="List of addresses of the customer", title="Addresses"
    )
    loyalty: Optional[int] = Field(
        None,
        description="Loyalty points of the customer",
        examples=[100],
        title="Loyalty",
    )
    birthdate: Optional[Date] = Field(
        None,
        description="Birthdate of the customer",
        examples=["1990-01-01"],
        title="Birthdate",
    )


class POSLineItemType(Enum):
    menu = "menu"
    product = "product"
    service = "service"
    general_discount = "general_discount"


class POSLocationItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the location",
        examples=["<this-location-id>"],
        title="Id",
    )
    name: str = Field(
        ...,
        description="Name given to the location",
        examples=["Restaurant de la Paix"],
        title="Name",
    )
    timezone: Optional[str] = Field(
        None,
        description="Indicates the timezone of the location. TZ notation, e.g. Europe/Brussels .",
        examples=["Europe/Paris"],
        title="Timezone",
    )
    address: Optional[AddressItem] = Field(None, description="Address of the location")


class POSPaymentStatus(Enum):
    Pending = "Pending"
    Completed = "Completed"
    Canceled = "Canceled"
    Failed = "Failed"
    Unknown = "Unknown"
    Authorised = "Authorised"


class POSTaxRateItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the tax rate",
        examples=["<this-tax-rate-id>"],
        title="Id",
    )
    label: Optional[str] = Field(
        None, description="Label of the tax rate", examples=["VAT 21%"], title="Label"
    )
    rate: Optional[float] = Field(
        None, description="Percentage of the tax rate", examples=[21.0], title="Rate"
    )
    tax_code: Optional[str] = Field(
        None,
        description="VAT code of the tax rate",
        examples=["VAT21"],
        title="Tax Code",
    )


class PartnerTypeInput(Enum):
    client = "client"
    supplier = "supplier"


class PartnerTypeOutput(Enum):
    owner = "owner"
    account = "account"


class PatchConnectionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    redirect: Optional[bool] = Field(
        False,
        description="Indicates whether you want to return to the consumer's redirectUrl after update (true) or whether you want to return on the connection page (false)",
        title="Redirect",
    )
    name: Optional[constr(max_length=80)] = Field(
        None,
        description="Can be used to update the name of an existing connection",
        title="Name",
    )
    credentials: Optional[List[CredentialItem]] = Field(
        None,
        description="Can be used to update the credentials of an existing connection. Please use the getIntegrations route to see the available credentials for each integration. Note: The preferred approach to let a consumer change their credentials is to call this endpoint without passing credentials in the body of the request, which will return a link that can be reshared with the consumer to update their credentials through the Chift UI.",
        title="Credentials",
    )


class Payment(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., examples=["<this-payment-id>"], title="Id")
    name: str = Field(..., examples=["Payment PAY-0001"], title="Name")
    currency: str = Field(..., examples=["EUR"], title="Currency")
    amount: float = Field(
        ...,
        description="Total amount of the payment. A part of this amount could be reconciled with another invoice.",
        examples=[242.0],
        title="Amount",
    )
    dedicated_amount: Optional[float] = Field(
        0,
        description="Amount of the payment dedicated to the invoice. It is 0 when the source cannot attribute the payment to individual invoices (e.g. a lump payment covering several), so a 0 here does not mean nothing was paid.",
        examples=[242.0],
        title="Dedicated Amount",
    )
    payment_date: Date = Field(..., examples=["2026-02-14"], title="Payment Date")
    journal_type: JournalType
    journal_id: str = Field(
        ..., examples=["<accounting-journal-id>"], title="Journal Id"
    )
    journal_name: str = Field(..., examples=["Bank journal"], title="Journal Name")
    reconciled: Optional[bool] = Field(False, title="Reconciled")
    communication: Optional[str] = Field(
        None, examples=["+++000/0000/00000+++"], title="Communication"
    )
    matching_number: Optional[str] = Field(
        None, examples=["<accounting-matching-number>"], title="Matching Number"
    )


class PaymentLocationItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the location",
        examples=["<this-location-id>"],
        title="Id",
    )
    name: str = Field(
        ...,
        description="Name given to the location",
        examples=["Restaurant de la Paix"],
        title="Name",
    )


class PaymentMethods(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the payment method",
        examples=["<this-payment-method-id>"],
        title="Id",
    )
    name: str = Field(
        ...,
        description="Chift's name of the payment method (e.g. 'Cash', 'Card', 'Transfer', ...)",
        examples=["Cash"],
        title="Name",
    )
    extra: Optional[str] = Field(
        None,
        description="Original name of the payment method",
        examples=["Espèces"],
        title="Extra",
    )


class PaymentPaymentStatus(Enum):
    pending = "pending"
    completed = "completed"
    partially_completed = "partially_completed"
    canceled = "canceled"
    failed = "failed"
    unknown = "unknown"
    authorized = "authorized"


class PaymentReportingConditions(Enum):
    TP01 = "TP01"
    TP02 = "TP02"
    TP03 = "TP03"


class PaymentReportingMethod(Enum):
    MP01 = "MP01"
    MP02 = "MP02"
    MP03 = "MP03"
    MP04 = "MP04"
    MP05 = "MP05"
    MP06 = "MP06"
    MP07 = "MP07"
    MP08 = "MP08"
    MP09 = "MP09"
    MP10 = "MP10"
    MP11 = "MP11"
    MP12 = "MP12"
    MP13 = "MP13"
    MP14 = "MP14"
    MP15 = "MP15"
    MP16 = "MP16"
    MP17 = "MP17"
    MP18 = "MP18"
    MP19 = "MP19"
    MP20 = "MP20"
    MP21 = "MP21"
    MP22 = "MP22"
    MP23 = "MP23"


class PaymentStatus(Enum):
    all = "all"
    unpaid = "unpaid"
    paid = "paid"


class PaymentTermAccountInfo(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    amount: float = Field(
        ...,
        description="Amount to be paid to the G or N account",
        examples=[200.0],
        title="Amount",
    )
    iban: str = Field(
        ...,
        description="IBAN of the G or R account",
        examples=["NL00INGB0000000000"],
        title="Iban",
    )


class PayoutStatus(Enum):
    scheduled = "scheduled"
    in_transit = "in_transit"
    paid = "paid"
    failed = "failed"
    canceled = "canceled"


class PayoutType(Enum):
    payout = "payout"
    bank_account = "bank_account"
    other = "other"


class PostAddressItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: constr(min_length=1) = Field(
        ...,
        description="Name given to the address (e.g. 'home')",
        examples=["home"],
        title="Name",
    )
    street: Optional[str] = Field(
        None, description="Street name", examples=["Example Street"], title="Street"
    )
    number: Optional[str] = Field(
        None, description="Number of the address", examples=["123"], title="Number"
    )
    box: Optional[str] = Field(
        None, description="Box of the address", examples=["A"], title="Box"
    )
    city: constr(min_length=1) = Field(
        ..., description="City name", examples=["Paris"], title="City"
    )
    postal_code: constr(min_length=1) = Field(
        ...,
        description="Postal code of the address",
        examples=["75000"],
        title="Postal Code",
    )
    country: Optional[str] = Field(
        None,
        description="Country, format: ISO 3166-1 codes.",
        examples=["FR"],
        title="Country",
    )


class Api1(Enum):
    Point_of_Sale = "Point of Sale"
    eCommerce = "eCommerce"
    Accounting = "Accounting"
    Invoicing = "Invoicing"
    Banking = "Banking"
    Custom = "Custom"
    Payment = "Payment"
    Property_Management_System = "Property Management System"


class PostConnectionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    integrationid: Optional[int] = Field(
        None,
        description="Can be used to specify the integration code of a specific connector. If specified, the url will point directly to the connection page of the connector and will redirect on save to the redirect url of the consumer if specified.",
        title="Integrationid",
    )
    name: Optional[constr(max_length=80)] = Field(
        None,
        description="Can be used to specify the name of the connection. Must be used in combination with an integrationid.",
        title="Name",
    )
    credentials: Optional[List[CredentialItem]] = Field(
        None,
        description="Can be used to specify the credentials of your connection. Must be used in combination with an integrationid and a name. Please use the getIntegrations route to see the available credentials for each integration",
        title="Credentials",
    )
    country: Optional[str] = Field(
        None,
        description="ISO 3166-1 alpha-2 country code to filter connectors by country. Ignored if integrationid is provided.",
        title="Country",
    )
    redirect: Optional[bool] = Field(
        False,
        description="Indicates whether you want to return to the consumer's redirectUrl after creation (true) or whether you want to stay on the connection selection page (false)(This field is ignored if integrationid is provided)",
        title="Redirect",
    )
    apis: Optional[List[Api1]] = Field(
        None,
        description="Can be used to filter connectors by APIs. Ignored if integrationid is provided.",
        title="Apis",
    )


class PostConsumerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: constr(min_length=1, max_length=255) = Field(
        ...,
        description="Name of the consumer as it will appear in your Chift platform. This name may be used for automated communications, such as automatic mailings if you enable this feature.",
        title="Name",
    )
    email: Optional[constr(max_length=80)] = Field(
        None,
        description="Optional - Email address of the consumer. It can be used for sending automated reminders (if feature enabled) and deployment by mail of local agent (when on-premise connector).",
        title="Email",
    )
    internal_reference: Optional[constr(max_length=255)] = Field(
        None,
        description="Your internal reference for the client (e.g., customer ID or unique identifier in your system).",
        title="Internal Reference",
    )
    redirect_url: Optional[str] = Field(
        None,
        description="URL to redirect the user after completing the activation via our link. Useful for directing customers to a dynamic or personalized url. You can also set up a default static URL in Chift's back office for all consumers.",
        title="Redirect Url",
    )


class PostSyncFlowEventData(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    is_preview: Optional[bool] = Field(
        False, description="Whether to preview the sync", title="Is Preview"
    )
    force_restart: Optional[bool] = Field(
        False,
        description="Whether to force restart the sync from the specified dates",
        title="Force Restart",
    )
    start_date: Optional[Date] = Field(
        None, description="Start date for sync range", title="Start Date"
    )
    end_date: Optional[Date] = Field(
        None, description="End date for sync range", title="End Date"
    )
    force_from_date: Optional[Date] = Field(
        None,
        description="Force sync from this specific date, overriding flow configuration",
        title="Force From Date",
    )
    import_mode: Optional[ImportMode] = Field(
        None, description="Import mode: reimport all data or only missing days"
    )


class ProductCategoryItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the category",
        examples=["<this-product-category-id>"],
        title="Id",
    )
    name: str = Field(
        ..., description="Name given to the category", examples=["Pizza"], title="Name"
    )
    description: Optional[str] = Field(
        None,
        description="Description of the category",
        examples=["Delicious pizza"],
        title="Description",
    )
    id_parent: Optional[str] = Field(
        None,
        description="Indicates if the category belongs to a parent category",
        examples=["<pos-product-category-id>"],
        title="Id Parent",
    )


class ProductCostItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    amount: float = Field(..., examples=[100.0], title="Amount")
    currency: str = Field(..., examples=["EUR"], title="Currency")


class ProductItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(
        ..., description="Name", examples=["Consulting services"], title="Name"
    )
    unit_price: Optional[float] = Field(
        None, description="Unit price", examples=[100.0], title="Unit Price"
    )
    tax_id: Optional[str] = Field(
        None,
        description="Technical id of the tax in Chift",
        examples=["<invoicing-tax-id>"],
        title="Tax Id",
    )
    code: Optional[str] = Field(
        None, description="Reference/code", examples=["SKU-0001"], title="Code"
    )
    unit: Optional[str] = Field(
        None,
        description="Unit of measure matching target software name",
        examples=["unit"],
        title="Unit",
    )
    category: Optional[str] = Field(
        None,
        description="Category matching target software name",
        examples=["Services"],
        title="Category",
    )
    currency: Optional[str] = Field(
        None,
        description="Currency matching target software name",
        examples=["EUR"],
        title="Currency",
    )
    description: Optional[str] = Field(
        None,
        description="Description",
        examples=["Consulting services"],
        title="Description",
    )
    cost: Optional[float] = Field(
        0, description="Cost of the product", examples=[0.0], title="Cost"
    )


class ProductPriceItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    unit_price: float = Field(
        ...,
        description="Unit price (without tax) of the product",
        examples=[10],
        title="Unit Price",
    )
    tax_rate: Optional[float] = Field(
        None,
        description="Tax rate applied to the product",
        examples=[10],
        title="Tax Rate",
    )


class ProductStatus(Enum):
    unknown = "unknown"
    archived = "archived"
    unpublished = "unpublished"
    published = "published"


class Type(Enum):
    cube = "cube"
    view = "view"


class PublicCubeMetaMember(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    title: Optional[str] = Field(None, title="Title")
    shortTitle: Optional[str] = Field(None, title="Shorttitle")
    aliasName: Optional[str] = Field(None, title="Aliasname")
    type: Optional[str] = Field(None, title="Type")
    description: Optional[str] = Field(None, title="Description")
    aggType: Optional[str] = Field(None, title="Aggtype")
    drillMembers: Optional[List[str]] = Field(None, title="Drillmembers")
    format: Optional[str] = Field(None, title="Format")
    primaryKey: Optional[bool] = Field(None, title="Primarykey")
    suggestFilterValues: Optional[bool] = Field(None, title="Suggestfiltervalues")
    nullable: Optional[bool] = Field(None, title="Nullable")
    values: Optional[List[str]] = Field(None, title="Values")
    allowedGranularities: Optional[List[str]] = Field(
        None, title="Allowedgranularities"
    )
    jsonSchema: Optional[Dict[str, Any]] = Field(None, title="Jsonschema")
    monetary: Optional[bool] = Field(None, title="Monetary")
    currencyDimension: Optional[str] = Field(None, title="Currencydimension")
    currencySensitive: Optional[bool] = Field(None, title="Currencysensitive")
    alignAxis: Optional[str] = Field(None, title="Alignaxis")


class Relationship(Enum):
    one_to_one = "one_to_one"
    one_to_many = "one_to_many"
    many_to_one = "many_to_one"


class PublicCubeRelationship(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    target: str = Field(..., title="Target")
    relationship: Relationship = Field(..., title="Relationship")


class QueryMeta(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    currency: Optional[List[Optional[str]]] = Field(None, title="Currency")
    monetary_measures: Optional[List[str]] = Field(None, title="Monetary Measures")


class QueryResponse(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    results: List[ExecutionResult] = Field(..., title="Results")
    meta: Optional[QueryMeta] = None


class ReadMappingItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    source_id: Optional[str] = Field(None, title="Source Id")
    target_id: Optional[str] = Field(None, title="Target Id")


class ReadSyncMappingItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    description: Optional[str] = Field(None, title="Description")
    display_order: Optional[int] = Field(0, title="Display Order")
    challenge_question: Optional[str] = Field(None, title="Challenge Question")
    conditions: Optional[Dict[str, Dict[str, Any]]] = Field(None, title="Conditions")
    values: List[ReadMappingItem] = Field(..., title="Values")
    hidden_source_ids: List[str] = Field(..., title="Hidden Source Ids")
    sub_mapping_name: str = Field(..., title="Sub Mapping Name")
    sub_mapping_description: Optional[str] = Field(
        None, title="Sub Mapping Description"
    )


class Ref(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: Optional[str] = Field(
        None,
        description="Technical id in the target software",
        examples=["<target-software-id>"],
        title="Id",
    )
    model: Optional[str] = Field(
        None,
        description="Name of the model/entity in the target software",
        examples=["invoice"],
        title="Model",
    )


class RefundFeesItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the fee in Chift",
        examples=["<commerce-fee-id>"],
        title="Id",
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    type: FeesType
    untaxed_amount: float = Field(..., examples=[5.0], title="Untaxed Amount")
    tax_amount: float = Field(..., examples=[1.05], title="Tax Amount")
    total: float = Field(..., examples=[6.05], title="Total")


class RefundItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    status: PaymentPaymentStatus = Field(..., description="Payment status")
    description: str = Field(
        ...,
        description="Description",
        examples=["Refund for invoice #INV-2026-0001"],
        title="Description",
    )
    amount: float = Field(..., description="Amount", examples=[242.0], title="Amount")
    currency: str = Field(
        ..., description="Currency", examples=["EUR"], title="Currency"
    )
    refund_date: DateTime = Field(
        ...,
        description="Date of creation",
        examples=["2026-01-15T10:30:00Z"],
        title="Refund Date",
    )
    payment_id: Optional[str] = Field(
        None,
        description="ID of the payment being refunded",
        examples=["<payment-payment-id>"],
        title="Payment Id",
    )


class RefundOrderLineItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the order line in Chift",
        examples=["<commerce-order-line-id>"],
        title="Id",
    )
    variant: Optional[OrderLineProductVariantItem] = Field(
        None, description="Product variant"
    )
    quantity: int = Field(..., examples=[1], title="Quantity")
    untaxed_amount: float = Field(
        ...,
        description="Untaxed amount refunded (after discount).",
        examples=[100.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ...,
        description="Total taxes refunded (after discount).",
        examples=[21.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total refunded (after discount).",
        examples=[121.0],
        title="Total",
    )


class ReleaseItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    fileName: str = Field(..., title="Filename")
    version: str = Field(..., title="Version")
    downloadUrl: str = Field(..., title="Downloadurl")


class ReportCategory(Enum):
    Supply_of_goods_subject_to_value_added_tax_ = (
        "Supply of goods subject to value-added tax."
    )
    Services_subject_to_value_added_tax_ = "Services subject to value-added tax."
    Supply_of_goods_and_provision_of_services_not_subject_to_value_added_tax_in_France__including_intra_Community_distance_sales_mentioned_in_1__of_I_of_Article_258_A_and_in_Article_259_B_of_the_French_General_Tax_Code_ = "Supply of goods and provision of services not subject to value-added tax in France, including intra-Community distance sales mentioned in 1° of I of Article 258 A and in Article 259 B of the French General Tax Code."
    Transactions_subject_to_the_regimes_provided_for_in_e__of_1_of_Article_266_and_in_Articles_268_and_297_A_of_the_French_General_Tax_Code__VAT_margin_scheme__ = "Transactions subject to the regimes provided for in e) of 1 of Article 266 and in Articles 268 and 297 A of the French General Tax Code (VAT margin scheme)."


class ReportPayments(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    tax_id: Optional[str] = Field(
        None,
        description="Unique identifier of the tax",
        examples=["<pos-tax-id>"],
        title="Tax Id",
    )
    tax_rate: float = Field(
        ..., description="Tax rate", examples=["10"], title="Tax Rate"
    )
    tax_amount: float = Field(
        ..., description="Tax amount", examples=["1"], title="Tax Amount"
    )
    total: float = Field(
        ..., description="Total amount including tax", examples=["11"], title="Total"
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Unique identifier of the payment method used for the payment",
        examples=["<pos-payment-method-id>"],
        title="Payment Method Id",
    )
    payment_method_name: Optional[str] = Field(
        None,
        description="Chift's name of the payment method used for the payment",
        examples=["Cash"],
        title="Payment Method Name",
    )
    currency: str = Field(
        ...,
        description="Currency of the payment (ISO 4217)",
        examples=["EUR"],
        title="Currency",
    )


class ReturnFeesItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the fee in Chift",
        examples=["<commerce-fee-id>"],
        title="Id",
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    type: FeesType
    untaxed_amount: float = Field(..., examples=[5.0], title="Untaxed Amount")
    tax_amount: float = Field(..., examples=[1.05], title="Tax Amount")
    total: float = Field(..., examples=[6.05], title="Total")


class ReturnOrderLineItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the order line in Chift",
        examples=["<commerce-order-line-id>"],
        title="Id",
    )
    variant: Optional[OrderLineProductVariantItem] = Field(
        None, description="Product variant"
    )
    quantity: int = Field(..., examples=[1], title="Quantity")
    untaxed_amount: float = Field(
        ...,
        description="Untaxed amount refunded (after discount).",
        examples=[100.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ...,
        description="Total taxes refunded (after discount).",
        examples=[21.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total refunded (after discount).",
        examples=[121.0],
        title="Total",
    )


class RoutingCodeItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    type: Optional[BankAccountRoutingType] = Field(
        "unknown",
        description="The routing format. Possible values: ach (US), sort_code (UK), bsb (AU), transit (CA), ifsc (IN), clabe (MX).",
    )
    value: Optional[str] = Field(
        None,
        description="The raw routing code value, without spaces or dashes.",
        title="Value",
    )


class SchemeItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique id of the scheme instance in the accounting system.",
        title="Id",
    )
    code: str = Field(
        ...,
        description="Short code or identifier of the scheme (e.g., 'RGS', 'SBR').",
        title="Code",
    )
    name: str = Field(
        ...,
        description="Name or description of the scheme as it appears in the accounting system.",
        title="Name",
    )


class SettingGroup(Enum):
    period = "period"
    write = "write"
    state = "state"
    observability = "observability"
    scope = "scope"


class SettingGroupItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    key: SettingGroup
    title: str = Field(..., title="Title")


class SettingOption(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    value: str = Field(..., title="Value")
    title: str = Field(..., title="Title")


class SettingType(Enum):
    Date = "date"
    bool = "bool"
    int = "int"
    enum = "enum"
    str_list = "str_list"
    dict = "dict"


class ShippingRefund(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    untaxed_amount: float = Field(
        ...,
        description="Untaxed amount refunded (after discount).",
        examples=[10.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ...,
        description="Total taxes refunded (after discount).",
        examples=[2.1],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total refunded (after discount).",
        examples=[12.1],
        title="Total",
    )


class Source(Enum):
    WEB = "WEB"
    MOBILE = "MOBILE"
    KIOSK = "KIOSK"
    POS = "POS"
    REMOTE_POS = "REMOTE_POS"
    JUSTEAT = "JUSTEAT"
    FOODORA = "FOODORA"
    UBEREATS = "UBEREATS"
    GLOVO = "GLOVO"
    DELIVEROO = "DELIVEROO"
    SMILEIN = "SMILEIN"
    UNKNOWN = "UNKNOWN"


class States(Enum):
    open = "open"
    closed = "closed"
    all = "all"


class Status(Enum):
    active = "active"
    inactive = "inactive"


class SyncConsumerStatus(Enum):
    active = "active"
    inactive = "inactive"


class SyncExecutionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., description="Execution id", title="Id")
    parentexecutionid: Optional[str] = Field(
        ...,
        description="Parent execution id. An execution is linked to multiple child executions linked to consumers.",
        title="Parentexecutionid",
    )
    start: DateTime = Field(..., title="Start")
    end: Optional[DateTime] = Field(..., title="End")
    status: str = Field(..., title="Status")
    consumer_id: str = Field(..., title="Consumer Id")
    flow_id: str = Field(..., title="Flow Id")


class SyncSkipReason(Enum):
    presync = "presync"
    connector_feature = "connector_feature"
    challenge = "challenge"


class SyncsConnectionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    one_api: Optional[int] = Field(None, title="One Api")
    connection_type: Optional[int] = Field(None, title="Connection Type")
    display_order: Optional[int] = Field(0, title="Display Order")
    display_hidden: Optional[bool] = Field(False, title="Display Hidden")


class TaxInfo(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    tax_code: str = Field(
        ...,
        description="VAT code of the tax line. This is the Id of the Tax code instance in the accounting software.",
        examples=["<accounting-tax-code>"],
        title="Tax Code",
    )
    tax_amount: float = Field(
        ...,
        description="Amount of the tax line. The amount must be positive and is required even with reversed VAT.",
        examples=[42.0],
        title="Tax Amount",
    )
    vat_account: Optional[str] = Field(
        None,
        description="VAT ledger account of the tax line. This is the ledger account number used to book the tax amount. The account is mandatory if tax amount is different from 0. The ledger account will ONLY be used when it is mandatory to pass this information to the accounting software.",
        examples=["<accounting-account-number>"],
        title="Vat Account",
    )
    reversed_vat_account: Optional[str] = Field(
        None,
        description="Reversed VAT ledger account of the tax line. This is the ledger account number used to book the reversed tax amount. The account is mandatory if you are using a reversed VAT code. The ledger account will ONLY be used when it is mandatory to pass this information to the accounting software.",
        examples=["<accounting-account-number>"],
        title="Reversed Vat Account",
    )
    description: Optional[str] = Field(
        None,
        description="Optional extra description of the tax line.",
        examples=["VAT 21%"],
        title="Description",
    )


class TaxRateItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    label: str = Field(..., examples=["Standard VAT"], title="Label")
    rate: float = Field(..., examples=[21.0], title="Rate")
    country: Optional[str] = Field(None, examples=["BE"], title="Country")


class Token(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    access_token: str = Field(..., title="Access Token")
    token_type: str = Field(..., title="Token Type")
    expires_in: int = Field(..., title="Expires In")
    expires_on: int = Field(..., title="Expires On")


class TotalTaxItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    tax_id: Optional[str] = Field(
        None,
        description="Unique identifier of the tax",
        examples=["<pos-tax-id>"],
        title="Tax Id",
    )
    tax_rate: float = Field(
        ..., description="Tax rate", examples=["10"], title="Tax Rate"
    )
    tax_amount: float = Field(
        ..., description="Tax amount", examples=["1"], title="Tax Amount"
    )
    total: float = Field(
        ..., description="Total amount including tax", examples=["11"], title="Total"
    )


class TransactionAccountingCategory(Enum):
    all = "all"
    unknown = "unknown"
    payout = "payout"
    payout_cancel = "payout_cancel"
    payment = "payment"
    payment_cancel = "payment_cancel"
    fee = "fee"
    fee_cancel = "fee_cancel"
    invoice = "invoice"
    internal_move = "internal_move"


class TransactionFilterDateType(Enum):
    value_date = "value_date"
    execution_date = "execution_date"


class TransactionItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    amount: float = Field(
        ..., description="Total amount incl. fee", examples=[200.0], title="Amount"
    )
    fee: float = Field(..., description="Total fee", examples=[5.0], title="Fee")
    currency: str = Field(
        ..., description="Currency", examples=["EUR"], title="Currency"
    )
    exchange_rate: float = Field(
        ..., description="Exchange rate", examples=[1.0], title="Exchange Rate"
    )
    create_date: DateTime = Field(
        ...,
        description="Create Date",
        examples=["2026-01-15T10:30:00Z"],
        title="Create Date",
    )
    application_type: str = Field(
        ...,
        description="Type of the transaction as recorded in the target software",
        examples=["payment"],
        title="Application Type",
    )
    accounting_category: TransactionAccountingCategory = Field(
        ..., description="Accounting category"
    )
    refund_id: Optional[str] = Field(
        None, examples=["<payment-refund-id>"], title="Refund Id"
    )
    payment_id: Optional[str] = Field(
        None, examples=["<payment-payment-id>"], title="Payment Id"
    )
    description: Optional[str] = Field(
        None,
        description="Description linked to the transaction",
        examples=["Card payment"],
        title="Description",
    )


class TriggerPriority(Enum):
    NORMAL = "NORMAL"
    MEDIUM = "MEDIUM"
    HIGH = "HIGH"


class TriggerResponse(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    status: str = Field(..., title="Status")
    message: str = Field(..., title="Message")
    data: Optional[Dict[str, Any]] = Field(None, title="Data")


class TriggerType(Enum):
    timer = "timer"
    event = "event"


class UpdateConsumerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: Optional[constr(max_length=255)] = Field(
        None,
        description="Name of the consumer as it will appear in your Chift platform. This name may be used for automated communications, such as automatic mailings if you enable this feature.",
        title="Name",
    )
    email: Optional[constr(max_length=80)] = Field(
        None,
        description="Optional - Email address of the consumer. It can be used for sending automated reminders (if feature enabled) and deployment by mail of local agent (when on-premise connector).",
        title="Email",
    )
    internal_reference: Optional[constr(max_length=255)] = Field(
        None,
        description="Your internal reference for the client (e.g., customer ID or unique identifier in your system).",
        title="Internal Reference",
    )
    redirect_url: Optional[str] = Field(
        None,
        description="URL to redirect the user after completing the activation via our link. Useful for directing customers to a dynamic or personalized url. You can also set up a default static URL in Chift's back office for all consumers.",
        title="Redirect Url",
    )


class UpdateFlowConsumer(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    triggerid: Optional[str] = Field("trigger-1", title="Triggerid")
    data: Optional[Dict[str, Any]] = Field(
        {},
        description="Object containing the configuration of the flow for the consumer",
        title="Data",
    )


class UpdateOrderItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    customer_id: Optional[str] = Field(
        None,
        description="Unique identifier of the customer",
        examples=["<pos-customer-id>"],
        title="Customer Id",
    )


class ValidationError(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    loc: List[Union[str, int]] = Field(..., title="Location")
    msg: str = Field(..., title="Message")
    type: str = Field(..., title="Error Type")
    input: Optional[Any] = Field(None, title="Input")
    ctx: Optional[Dict[str, Any]] = Field(None, title="Context")


class VariantAttributeItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., examples=["Size"], title="Name")
    value: str = Field(..., examples=["M"], title="Value")


class VariantAttributeOptionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., examples=["Size"], title="Name")
    values: List[str] = Field(..., examples=[["S", "M", "L"]], title="Values")


class VariantItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    parent_id: str = Field(
        ...,
        description="Technical id of the parent product in Chift",
        examples=["<commerce-product-id>"],
        title="Parent Id",
    )
    name: str = Field(..., examples=["T-Shirt - M"], title="Name")
    description: Optional[str] = Field(
        None,
        examples=["Classic cotton T-Shirt, available in several sizes."],
        title="Description",
    )
    description_html: Optional[str] = Field(
        None,
        examples=["<p>Classic cotton T-Shirt, available in several sizes.</p>"],
        title="Description Html",
    )
    categories: Optional[List[CommerceProductCategoryItem]] = Field(
        [], title="Categories"
    )
    created_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Created On"
    )
    sku: Optional[str] = Field(None, examples=["TSHIRT-M"], title="Sku")
    taxable: Optional[bool] = Field(None, title="Taxable")
    barcode: Optional[str] = Field(None, examples=["5410000000000"], title="Barcode")
    available_quantity: Optional[float] = Field(
        0, examples=[10.0], title="Available Quantity"
    )
    prices: Optional[List[CommerceProductPriceItem]] = Field([], title="Prices")
    unit_cost: Optional[ProductCostItem] = None
    unit_of_measure: Optional[str] = Field(
        None, examples=["piece"], title="Unit Of Measure"
    )
    weight: Optional[float] = Field(0, examples=[0.2], title="Weight")
    weight_unit: Optional[str] = Field(None, examples=["kg"], title="Weight Unit")
    status: Optional[ProductStatus] = None
    inventory_details: Optional[List[InventoryDetailsItem]] = Field(
        [], title="Inventory Details"
    )
    common_attributes: Optional[List[CommonAttributeItem]] = Field(
        [],
        description="List of attributes that are shared by all variants of the product.",
        title="Common Attributes",
    )
    variant_attributes: Optional[List[VariantAttributeItem]] = Field(
        [],
        description="List of attributes that are specific to the variant of the product.",
        title="Variant Attributes",
    )
    common_images: Optional[List[ImageItem]] = Field(
        [],
        description="List of images that are shared by all variants of the product.",
        title="Common Images",
    )
    variant_images: Optional[List[ImageItem]] = Field(
        [],
        description="List of images that are specific to the variant of the product.",
        title="Variant Images",
    )


class VatCodeScope(Enum):
    nat = "nat"
    eu = "eu"
    int = "int"
    unknown = "unknown"


class VatCodeType(Enum):
    sale = "sale"
    purchase = "purchase"
    both = "both"
    unknown = "unknown"


class Visibility(Enum):
    consumer = "consumer"
    connector = "connector"
    both = "both"


class WebhookInstanceGetItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    webhookid: UUID = Field(..., title="Webhookid")
    accountid: UUID = Field(..., title="Accountid")
    createdby: Optional[UUID] = Field(None, title="Createdby")
    createdon: DateTime = Field(..., title="Createdon")
    event: str = Field(..., title="Event")
    url: str = Field(..., title="Url")
    status: Status
    integrationid: Optional[int] = Field(None, title="Integrationid")


class WebhookInstancePatchItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    status: Optional[Status] = None
    url: Optional[constr(max_length=500)] = Field(None, title="Url")
    signingsecret: Optional[constr(max_length=100)] = Field(None, title="Signingsecret")


class WebhookInstancePostItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    event: constr(max_length=80) = Field(..., title="Event")
    url: constr(max_length=500) = Field(..., title="Url")
    signingsecret: Optional[constr(max_length=100)] = Field(None, title="Signingsecret")
    integrationid: Optional[int] = Field(
        None,
        description="Can be used to specify the integration code of a specific connector. This is mandatory in case you create a webhook related to a specific One API.",
        title="Integrationid",
    )


class WebhookItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    event: str = Field(..., title="Event")
    api: Optional[str] = Field(None, title="Api")


class WebhookLogItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    webhooklogid: UUID = Field(..., title="Webhooklogid")
    webhookid: UUID = Field(..., title="Webhookid")
    event: str = Field(..., title="Event")
    url: str = Field(..., title="Url")
    accountid: str = Field(..., title="Accountid")
    createdon: DateTime = Field(..., title="Createdon")
    httpstatus: int = Field(..., title="Httpstatus")
    integrationid: Optional[int] = Field(None, title="Integrationid")


class WelfareFundType(Enum):
    TC01 = "TC01"
    TC02 = "TC02"
    TC03 = "TC03"
    TC04 = "TC04"
    TC05 = "TC05"
    TC06 = "TC06"
    TC07 = "TC07"
    TC08 = "TC08"
    TC09 = "TC09"
    TC10 = "TC10"
    TC11 = "TC11"
    TC12 = "TC12"
    TC13 = "TC13"
    TC14 = "TC14"
    TC15 = "TC15"
    TC16 = "TC16"
    TC17 = "TC17"
    TC18 = "TC18"
    TC19 = "TC19"
    TC20 = "TC20"
    TC21 = "TC21"
    TC22 = "TC22"


class WithholdingTax(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    tax_rate: float = Field(
        ...,
        description="Rate of the withholding tax. Normal withholding tax should have a negative tax rate.",
        examples=[-19.0],
        title="Tax Rate",
    )
    tax_code: str = Field(
        ...,
        description="Tax code of the withholding tax. This is the Id of the Tax Code in the accounting software.",
        examples=["<accounting-tax-code>"],
        title="Tax Code",
    )
    tax_amount: float = Field(
        ...,
        description="Amount of the withholding tax. This is the amount of the withholding tax that will be applied to the invoice. It should be negative for normal withholding tax.",
        examples=[-38.0],
        title="Tax Amount",
    )


class WithholdingTaxPaymentReason(Enum):
    A = "A"
    U = "U"
    R = "R"
    Q = "Q"
    H = "H"
    V = "V"
    V2 = "V2"
    I = "I"
    J = "J"
    K = "K"
    P = "P"
    S = "S"
    T = "T"
    W = "W"
    X = "X"
    Y = "Y"
    B = "B"
    C = "C"
    D = "D"
    E = "E"
    F = "F"
    G = "G"
    L = "L"
    L1 = "L1"
    M = "M"
    M1 = "M1"
    M2 = "M2"
    IN = "IN"
    O = "O"
    O1 = "O1"
    V1 = "V1"


class WithholdingTaxReason(Enum):
    RT01 = "RT01"
    RT02 = "RT02"
    RT03 = "RT03"
    RT04 = "RT04"
    RT05 = "RT05"
    RT06 = "RT06"


class WriteMode(Enum):
    normal = "normal"
    debug = "debug"
    preview = "preview"


class AccountTaxInformation(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    tax_codes: List[AccountTaxCode] = Field(
        ...,
        description="Tax codes configured on the ledger account in the accounting system.",
        title="Tax Codes",
    )
    mode: AccountTaxMode = Field(
        ...,
        description="How strictly the accounting system applies the tax codes of the ledger account. 'free': the list is informative (mainly to define the default one) and other tax codes can be used. 'restricted': only the tax codes of the list (or none) can be used. 'forced': one of the tax codes of the list must be used when creating entries on the ledger account.",
    )


class AccountingInvoicePaymentIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    date: Date = Field(
        ...,
        description="The date of the payment",
        examples=["2026-02-14"],
        title="Date",
    )
    payment_method_id: str = Field(
        ...,
        description="Technical ID of the payment method in the accounting system.",
        examples=["<accounting-payment-method-id>"],
        title="Payment Method Id",
    )
    currency: str = Field(
        ..., description="Payment currency", examples=["EUR"], title="Currency"
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, applicable at the payment date. Required when the currency is different from the folder's base currency.",
        title="Currency Exchange Rate",
    )
    reference: Optional[str] = Field(
        None,
        description="External reference of the payment.",
        examples=["REF-0001"],
        title="Reference",
    )
    number: Optional[str] = Field(
        None,
        description="Unique Payment number (if managed by the software).",
        examples=["PAY-2026-0001"],
        title="Number",
    )
    items: List[AccountingInvoicePaymentItemIn] = Field(
        ..., description="Payment's repartition", title="Items"
    )


class AccountingVatCode(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique id of the VAT Code instance in the accounting system.",
        examples=["<this-vat-code-id>"],
        title="Id",
    )
    code: Optional[str] = Field(
        None,
        description="Short code (sometimes also used as id) for the VAT code, as defined in the accounting system. This code is typically used for internal referencing, mapping, or selection of VAT codes",
        examples=["VAT21"],
        title="Code",
    )
    active: Optional[bool] = Field(
        True,
        description="Flag indicating whether the vat code is active. If True, the vat code is active and can be used in entries/invoices. If False, the vat code is inactive and cannot be used in entries/invoices.",
        title="Active",
    )
    label: str = Field(
        ...,
        description="Label of the VAT code as it appears in the accounting system. This is the human-readable name used to identify the VAT code.",
        examples=["VAT 21%"],
        title="Label",
    )
    scope: Optional[VatCodeScope] = Field(
        "unknown",
        description="Scope of the VAT code, indicating its applicability. Use 'nat' for VAT codes that apply to local/domestic sales or purchases, 'eu' for cross-border transactions within the EU, and 'int' for transactions with countries outside the EU.",
    )
    rate: float = Field(
        ...,
        description="VAT rate associated with the VAT code. This is the percentage rate applied to the amount when this VAT code is used.",
        examples=[21.0],
        title="Rate",
    )
    type: VatCodeType = Field(
        ...,
        description="Type of the VAT code, indicating whether it is a sales tax or purchase tax. This helps in understanding how the VAT code is applied in transactions.",
    )
    deductible_account: Optional[str] = Field(
        None,
        description="Ledger account number used for the deductible part of the VAT code. This is typically used for purchase transactions where VAT can be deducted.",
        examples=["<accounting-account-number>"],
        title="Deductible Account",
    )
    payable_account: Optional[str] = Field(
        None,
        description="Ledger account number used for the payable part of the VAT code. This is typically used for sales transactions where VAT is collected from customers.",
        examples=["<accounting-account-number>"],
        title="Payable Account",
    )
    reversed: Optional[bool] = Field(
        False,
        description="Whether the VAT code is a reverse-charge code, where VAT-reporting responsibility shifts from seller to buyer. When true, the invoice line's tax_amount is 0: two VAT lines are booked that cancel each other, one on the deductible account and one on the payable account.",
        title="Reversed",
    )
    withholding_tax: Optional[bool] = Field(
        False,
        description="Whether this VAT code can be used for withholding taxes.",
        title="Withholding Tax",
    )
    country: Optional[str] = Field(
        None,
        description="ISO 3166-1 country code the VAT code is restricted to, set only when the code can be used for one specific country.",
        title="Country",
    )


class AddressItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    address_type: AddressType
    name: Optional[str] = Field(None, title="Name")
    number: Optional[str] = Field(None, title="Number")
    box: Optional[str] = Field(None, title="Box")
    phone: Optional[str] = Field(None, title="Phone")
    mobile: Optional[str] = Field(None, title="Mobile")
    email: Optional[str] = Field(None, title="Email")
    street: str = Field(..., examples=["Example Street"], title="Street")
    city: str = Field(..., examples=["Brussels"], title="City")
    postal_code: str = Field(..., examples=["1000"], title="Postal Code")
    country: str = Field(
        ..., description="Format: ISO 3166-1 codes.", examples=["BE"], title="Country"
    )


class AddressItemInInvoicing(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    address_type: AddressTypeInvoicing
    name: Optional[str] = Field(None, title="Name")
    number: Optional[str] = Field(None, title="Number")
    box: Optional[str] = Field(None, title="Box")
    phone: Optional[str] = Field(None, title="Phone")
    mobile: Optional[str] = Field(None, title="Mobile")
    email: Optional[str] = Field(None, title="Email")
    street: str = Field(..., examples=["Example Street"], title="Street")
    city: str = Field(..., examples=["Brussels"], title="City")
    postal_code: str = Field(..., examples=["1000"], title="Postal Code")
    country: str = Field(
        ..., description="Format: ISO 3166-1 codes.", examples=["BE"], title="Country"
    )


class AddressItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    address_type: AddressType
    name: Optional[str] = Field(None, title="Name")
    number: Optional[str] = Field(None, title="Number")
    box: Optional[str] = Field(None, title="Box")
    phone: Optional[str] = Field(None, title="Phone")
    mobile: Optional[str] = Field(None, title="Mobile")
    email: Optional[str] = Field(None, title="Email")
    street: Optional[str] = Field(None, title="Street")
    city: Optional[str] = Field(None, title="City")
    postal_code: Optional[str] = Field(None, title="Postal Code")
    country: Optional[str] = Field(
        None, description="Format: ISO 3166-1 codes.", title="Country"
    )


class AddressItemOutInvoicing(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    address_type: AddressTypeInvoicing
    name: Optional[str] = Field(None, title="Name")
    number: Optional[str] = Field(None, title="Number")
    box: Optional[str] = Field(None, title="Box")
    phone: Optional[str] = Field(None, title="Phone")
    mobile: Optional[str] = Field(None, title="Mobile")
    email: Optional[str] = Field(None, title="Email")
    street: Optional[str] = Field(None, title="Street")
    city: Optional[str] = Field(None, title="City")
    postal_code: Optional[str] = Field(None, title="Postal Code")
    country: Optional[str] = Field(
        None, description="Format: ISO 3166-1 codes.", title="Country"
    )


class AnalyticDistribution(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    analytic_plan: str = Field(
        ...,
        description="Technical ID of the analytic plan to which the distribution applies.",
        examples=["<accounting-analytic-plan-id>"],
        title="Analytic Plan",
    )
    analytic_accounts: List[AnalyticDistributionDetail] = Field(
        ...,
        description="List of analytic accounts and their respective percentages for the distribution.",
        title="Analytic Accounts",
    )


class ApiTransactionLookupOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    created_on: DateTime = Field(
        ...,
        description="Date and time when the API transaction started (UTC).",
        examples=["2024-05-15T12:34:56Z"],
        title="Created On",
    )
    method: HTTPMethod = Field(
        ..., description="HTTP method of the original request.", examples=["POST"]
    )
    route: str = Field(
        ...,
        description="Path of the original request.",
        examples=[
            "/consumers/a65f95f4-7188-4866-a3ff-eb0fe17958c3/accounting/suppliers"
        ],
        title="Route",
    )
    status_code: int = Field(
        ...,
        description="HTTP status code returned by the original request.",
        examples=[200],
        title="Status Code",
    )
    created_entity_id: Optional[str] = Field(
        None,
        description="Identifier of the entity created by the request, when applicable (typically for POST).",
        examples=["inv_123"],
        title="Created Entity Id",
    )


class AttachmentItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    base64_string: str = Field(
        ..., examples=["<base64-encoded-file-content>"], title="Base64 String"
    )
    document_type: InvoicingDocumentType


class BalanceItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    available_amount: float = Field(
        ...,
        description="Total amount available",
        examples=[1000.0],
        title="Available Amount",
    )
    currency: str = Field(
        ..., description="Currency", examples=["EUR"], title="Currency"
    )
    create_date: Optional[DateTime] = Field(
        None,
        description="Create Date",
        examples=["2026-01-15T10:30:00Z"],
        title="Create Date",
    )


class BankAccountsItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    is_primary: Optional[bool] = Field(
        True,
        description="Indicates whether this is the default bank account for this third-party. Only one bank account per third-party can have is_primary set to true. Defaults to true.",
        title="Is Primary",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the bank account (e.g., EUR).",
        title="Currency",
    )
    iban: Optional[str] = Field(
        None,
        description="International Bank Account Number (ISO 13616). Present for countries in the IBAN scheme (e.g., France, Germany, United Kingdom). null for countries that do not participate in the IBAN scheme (e.g., US, Canada, Australia).",
        title="Iban",
    )
    bic_swift: Optional[str] = Field(
        None,
        description="Bank Identifier Code (ISO 9362), also known as SWIFT code. 8 or 11 characters. Optional for domestic payments, but required for international wire transfers.",
        title="Bic Swift",
    )
    account_number: Optional[str] = Field(
        None,
        description="Local bank account number. Used when the country does not participate in the IBAN scheme (e.g., US, CA, AU). Always paired with a routing_code.",
        title="Account Number",
    )
    routing_code: Optional[RoutingCodeItem] = Field(
        None,
        description="Domestic routing identifier for the bank. Required when no iban is present. Contains two subfields: type and value",
    )


class BankTransactionCounterpartItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    counterpart_type: Optional[BankTransactionCounterpartType] = Field(
        None, description="Type of the counterpart"
    )
    id: Optional[str] = Field(
        None, description="ID of the counterpart in the accounting system", title="Id"
    )
    name: Optional[str] = Field(
        None, description="Name of the counterpart", examples=["John Doe"], title="Name"
    )
    iban: Optional[str] = Field(
        None,
        description="IBAN of the counterpart",
        examples=["BE00000000000000"],
        title="Iban",
    )
    bank_account: Optional[str] = Field(
        None,
        description="Bank account number of the counterpart",
        examples=["0000000000"],
        title="Bank Account",
    )


class BankTransactionItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    general_ledger_account: Optional[str] = Field(
        None,
        description="Number of the general ledger account to book the transaction against.",
        examples=["<accounting-account-number>"],
        title="General Ledger Account",
    )
    description: Optional[str] = Field(
        None,
        description="Transaction description",
        examples=["Bank charges"],
        title="Description",
    )
    external_transaction_id: Optional[str] = Field(
        None,
        description="External transaction ID",
        examples=["TXN-2026-0001"],
        title="External Transaction Id",
    )
    amount: float = Field(
        ...,
        description="Total amount including fee and tax",
        examples=[242.0],
        title="Amount",
    )
    fee_amount: Optional[float] = Field(
        0, description="Fee amount", examples=[0.0], title="Fee Amount"
    )
    tax_amount: Optional[float] = Field(
        0, description="Tax amount", examples=[0.0], title="Tax Amount"
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, applicable at the transaction date.",
        title="Currency Exchange Rate",
    )
    counterpart: Optional[BankTransactionCounterpartItem] = Field(
        None, description="Counterpart of the transaction"
    )


class BankTransactionItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    general_ledger_account: Optional[str] = Field(
        None,
        description="Number of the general ledger account to book the transaction against.",
        examples=["<accounting-account-number>"],
        title="General Ledger Account",
    )
    description: Optional[str] = Field(
        None,
        description="Transaction description",
        examples=["Bank charges"],
        title="Description",
    )
    external_transaction_id: Optional[str] = Field(
        None,
        description="External transaction ID",
        examples=["TXN-2026-0001"],
        title="External Transaction Id",
    )
    amount: float = Field(
        ...,
        description="Total amount including fee and tax",
        examples=[242.0],
        title="Amount",
    )
    fee_amount: Optional[float] = Field(
        0, description="Fee amount", examples=[0.0], title="Fee Amount"
    )
    tax_amount: Optional[float] = Field(
        0, description="Tax amount", examples=[0.0], title="Tax Amount"
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, applicable at the transaction date.",
        title="Currency Exchange Rate",
    )
    counterpart: Optional[BankTransactionCounterpartItem] = Field(
        None, description="Counterpart of the transaction"
    )
    id: Optional[str] = Field(
        None,
        description="Internal transaction ID generated by the accounting system",
        examples=["<this-bank-transaction-id>"],
        title="Id",
    )


class CategoryItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    name: str = Field(..., examples=["Beverages"], title="Name")
    parent_id: Optional[str] = Field(
        None,
        description="Technical id of the parent category in the target software",
        examples=["<commerce-category-id>"],
        title="Parent Id",
    )


class ChiftId(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")


class ChiftPageAccountingVatCode(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[AccountingVatCode] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageBalanceItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[BalanceItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageCategoryItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[CategoryItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageContactItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[ContactItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageCountryItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[CountryItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageEmployeeItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[EmployeeItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageModifiersItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[ModifiersItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageObjectivesItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[ObjectivesItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePMSLocationItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PMSLocationItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePOSCustomerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[POSCustomerItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePOSLocationItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[POSLocationItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePOSTaxRateItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[POSTaxRateItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePaymentLocationItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PaymentLocationItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePaymentMethods(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PaymentMethods] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePayment(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[Payment] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageProductCategoryItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[ProductCategoryItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageRefundItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[RefundItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageSchemeItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[SchemeItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageSyncExecutionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[SyncExecutionItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageTaxRateItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[TaxRateItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageTransactionItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[TransactionItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ClientItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    external_reference: Optional[str] = Field(
        None,
        description="External identifier used to link the client in the accounting system with the corresponding client reference in your own system. Normally unique per client/supplier type and can serve as the primary key of the accounting system.",
        examples=["CUST-0001"],
        title="External Reference",
    )
    first_name: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the first name of the client.",
        examples=["John"],
        title="First Name",
    )
    last_name: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the last name of the client.",
        examples=["Doe"],
        title="Last Name",
    )
    name: constr(min_length=1) = Field(
        ...,
        description="Full name or legal name of the client (individual or company). This is the name that will be displayed in the accounting software.",
        examples=["Example Company S.A."],
        title="Name",
    )
    function: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Job function/title of the client.",
        examples=["Accountant"],
        title="Function",
    )
    is_company: Optional[bool] = Field(
        True,
        description="Indicates if the client is an individual or a company.",
        title="Is Company",
    )
    company_id: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the id of the company linked to the client.",
        examples=["<accounting-company-id>"],
        title="Company Id",
    )
    phone: Optional[str] = Field(
        None,
        description="Phone number of the client.",
        examples=["+32 2 000 00 00"],
        title="Phone",
    )
    mobile: Optional[str] = Field(
        None,
        description="Mobile phone number of the client.",
        examples=["+32 4 000 00 00"],
        title="Mobile",
    )
    email: Optional[str] = Field(
        None,
        description="Email address of the client.",
        examples=["john.doe@example.com"],
        title="Email",
    )
    language: Optional[str] = Field(
        None,
        description="Language of the client. Format: ISO 639-1 codes.",
        examples=["en"],
        title="Language",
    )
    internal_notes: Optional[str] = Field(
        None,
        description="Internal notes about the client.",
        examples=["Preferred contact: accounts payable team."],
        title="Internal Notes",
    )
    website: Optional[str] = Field(
        None,
        description="Website of the client.",
        examples=["https://www.example.com"],
        title="Website",
    )
    vat: Optional[str] = Field(
        None,
        description="VAT number of the client, used for tax compliance and invoicing. This value is unique per company and can be used as a reliable identifier to match clients between systems.",
        examples=["BE0000000000"],
        title="Vat",
    )
    company_number: Optional[str] = Field(
        None,
        description="National identification number of the company.",
        examples=["0000000000"],
        title="Company Number",
    )
    iban: Optional[str] = Field(
        None,
        description="IBAN account number of the client.",
        examples=["BE00000000000000"],
        title="Iban",
    )
    bank_account: Optional[str] = Field(
        None,
        description="Bank account number of the client.",
        examples=["0000000000"],
        title="Bank Account",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the client (e.g., EUR).",
        examples=["EUR"],
        title="Currency",
    )
    active: Optional[bool] = Field(
        True, description="Indicates if the client is active.", title="Active"
    )
    addresses: List[AddressItemIn] = Field(
        ...,
        description="List of addresses associated with the client.",
        title="Addresses",
    )
    account_number: Optional[str] = Field(
        None,
        description="Number of ledger account assigned to the client in the accounting system as it will appear in the official accounting export file (FEC, SIE, iXBRL, etc.). This is typically the ledger account used for posting receivables. In some systems, each customer has a dedicated account; in others, a shared control account is used and customer balances are tracked in a subledger.",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )


class ClientItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    external_reference: Optional[str] = Field(
        None,
        description="External identifier used to link the client in the accounting system with the corresponding client reference in your own system. Normally unique per client/supplier type and can serve as the primary key of the accounting system.",
        examples=["CUST-0001"],
        title="External Reference",
    )
    first_name: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the first name of the client.",
        examples=["John"],
        title="First Name",
    )
    last_name: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the last name of the client.",
        examples=["Doe"],
        title="Last Name",
    )
    name: Optional[str] = Field(
        None,
        description="Full name or legal name of the client (individual or company). This is the name displayed in the accounting software.",
        examples=["Example Company S.A."],
        title="Name",
    )
    function: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Job function/title of the client.",
        examples=["Accountant"],
        title="Function",
    )
    is_company: Optional[bool] = Field(
        True,
        description="Indicates if the client is an individual or a company.",
        title="Is Company",
    )
    company_id: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the id of the company linked to the client.",
        examples=["<accounting-company-id>"],
        title="Company Id",
    )
    phone: Optional[str] = Field(
        None,
        description="Phone number of the client.",
        examples=["+32 2 000 00 00"],
        title="Phone",
    )
    mobile: Optional[str] = Field(
        None,
        description="Mobile phone number of the client.",
        examples=["+32 4 000 00 00"],
        title="Mobile",
    )
    email: Optional[str] = Field(
        None,
        description="Email address of the client.",
        examples=["john.doe@example.com"],
        title="Email",
    )
    language: Optional[str] = Field(
        None,
        description="Language of the client. Format: ISO 639-1 codes.",
        examples=["en"],
        title="Language",
    )
    internal_notes: Optional[str] = Field(
        None,
        description="Internal notes about the client.",
        examples=["Preferred contact: accounts payable team."],
        title="Internal Notes",
    )
    website: Optional[str] = Field(
        None,
        description="Website of the client.",
        examples=["https://www.example.com"],
        title="Website",
    )
    vat: Optional[str] = Field(
        None,
        description="VAT number of the client.",
        examples=["BE0000000000"],
        title="Vat",
    )
    company_number: Optional[str] = Field(
        None,
        description="National identification number of the company.",
        examples=["0000000000"],
        title="Company Number",
    )
    iban: Optional[str] = Field(
        None,
        description="IBAN account number of the client. DEPRECATED see bank_accounts",
        examples=["BE00000000000000"],
        title="Iban",
    )
    bank_account: Optional[str] = Field(
        None,
        description="Bank account number of the client. DEPRECATED see bank_accounts",
        examples=["0000000000"],
        title="Bank Account",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the client (e.g., EUR).",
        examples=["EUR"],
        title="Currency",
    )
    active: Optional[bool] = Field(
        True, description="Indicates if the client is active.", title="Active"
    )
    account_number: Optional[str] = Field(
        None,
        description="General ledger account assigned to the customer in the accounting system as it will appear in the official accounting export file (FEC, SIE, iXBRL, etc.). This is the account used on journal entry lines to debit/credit this partner.",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    id: Optional[str] = Field(
        None,
        description="Id of the client in the accounting software. This is the unique identifier used to reference the client in the system.",
        examples=["<this-client-id>"],
        title="Id",
    )
    last_updated_on: Optional[DateTime] = Field(
        None,
        description="The last time the client has been updated.",
        examples=["2026-01-15T10:30:00Z"],
        title="Last Updated On",
    )
    addresses: Optional[List[AddressItemOut]] = Field(
        [],
        description="List of addresses associated with the client.",
        title="Addresses",
    )
    third_party_account: Optional[str] = Field(
        None,
        description="Third party account number/code representing the client in the accounting software.",
        title="Third Party Account",
    )
    bank_accounts: Optional[List[BankAccountsItem]] = Field(
        None,
        description="List of bank accounts information associated with the client.",
        title="Bank Accounts",
    )


class ClientItemUpdate(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    external_reference: Optional[str] = Field(
        None,
        description="External identifier used to link the client in the accounting system with the corresponding client reference in your own system. Normally unique per client/supplier type and can serve as the primary key of the accounting system.",
        examples=["CUST-0001"],
        title="External Reference",
    )
    first_name: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the first name of the client.",
        examples=["John"],
        title="First Name",
    )
    last_name: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the last name of the client.",
        examples=["Doe"],
        title="Last Name",
    )
    name: Optional[str] = Field(
        None,
        description="Full name or legal name of the client (individual or company). This is the name that will be displayed in the accounting software.",
        examples=["Example Company S.A."],
        title="Name",
    )
    function: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Job function/title of the client.",
        examples=["Accountant"],
        title="Function",
    )
    is_company: Optional[bool] = Field(
        True,
        description="Indicates if the client is an individual or a company.",
        title="Is Company",
    )
    company_id: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the id of the company linked to the client.",
        examples=["<accounting-company-id>"],
        title="Company Id",
    )
    phone: Optional[str] = Field(
        None,
        description="Phone number of the client.",
        examples=["+32 2 000 00 00"],
        title="Phone",
    )
    mobile: Optional[str] = Field(
        None,
        description="Mobile phone number of the client.",
        examples=["+32 4 000 00 00"],
        title="Mobile",
    )
    email: Optional[str] = Field(
        None,
        description="Email address of the client.",
        examples=["john.doe@example.com"],
        title="Email",
    )
    language: Optional[str] = Field(
        None,
        description="Language of the client. Format: ISO 639-1 codes.",
        examples=["en"],
        title="Language",
    )
    internal_notes: Optional[str] = Field(
        None,
        description="Internal notes about the client.",
        examples=["Preferred contact: accounts payable team."],
        title="Internal Notes",
    )
    website: Optional[str] = Field(
        None,
        description="Website of the client.",
        examples=["https://www.example.com"],
        title="Website",
    )
    vat: Optional[str] = Field(
        None,
        description="VAT number of the client, used for tax compliance and invoicing. This value is unique per company and can be used as a reliable identifier to match clients between systems.",
        examples=["BE0000000000"],
        title="Vat",
    )
    company_number: Optional[str] = Field(
        None,
        description="National identification number of the company.",
        examples=["0000000000"],
        title="Company Number",
    )
    iban: Optional[str] = Field(
        None,
        description="IBAN account number of the client.",
        examples=["BE00000000000000"],
        title="Iban",
    )
    bank_account: Optional[str] = Field(
        None,
        description="Bank account number of the client.",
        examples=["0000000000"],
        title="Bank Account",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the client (e.g., EUR).",
        examples=["EUR"],
        title="Currency",
    )
    active: Optional[bool] = Field(
        True, description="Indicates if the client is active.", title="Active"
    )
    addresses: Optional[List[AddressItemOut]] = Field(
        [],
        description="List of addresses associated with the client.",
        title="Addresses",
    )


class CommerceCustomerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    first_name: Optional[str] = Field(None, examples=["John"], title="First Name")
    last_name: Optional[str] = Field(None, examples=["Doe"], title="Last Name")
    phone: Optional[str] = Field(None, examples=["+32 2 000 00 00"], title="Phone")
    email: Optional[str] = Field(None, examples=["john.doe@example.com"], title="Email")
    language: Optional[str] = Field(
        None, description="Format: ISO 639-1 codes.", examples=["en"], title="Language"
    )
    internal_notes: Optional[str] = Field(
        None,
        examples=["Preferred contact: accounts payable team."],
        title="Internal Notes",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the client (e.g. EUR).",
        examples=["EUR"],
        title="Currency",
    )
    addresses: Optional[List[CommerceAddressItemOut]] = Field([], title="Addresses")
    created_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Created On"
    )


class CommerceLocationItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the location in Chift",
        examples=["<this-location-id>"],
        title="Id",
    )
    name: str = Field(..., examples=["Main warehouse"], title="Name")
    source_ref: Optional[Ref] = Field(
        None, description="Technical id of the location in the target software"
    )


class ContactItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    is_prospect: Optional[bool] = Field(
        None, description="Is a prospect?", title="Is Prospect"
    )
    is_customer: Optional[bool] = Field(
        None, description="Is a customer?", title="Is Customer"
    )
    is_supplier: Optional[bool] = Field(
        None, description="Is a supplier?", title="Is Supplier"
    )
    is_company: Optional[bool] = Field(
        None, description="Is a company?", title="Is Company"
    )
    company_name: Optional[str] = Field(
        None,
        description="Name of the company",
        examples=["Example Company S.A."],
        title="Company Name",
    )
    first_name: Optional[str] = Field(
        None, description="Firstname", examples=["John"], title="First Name"
    )
    last_name: Optional[str] = Field(
        None, description="Lastname", examples=["Doe"], title="Last Name"
    )
    email: Optional[str] = Field(
        None, description="Email", examples=["john.doe@example.com"], title="Email"
    )
    phone: Optional[str] = Field(
        None, description="Phone", examples=["+32 2 000 00 00"], title="Phone"
    )
    mobile: Optional[str] = Field(
        None, description="Mobile", examples=["+32 4 000 00 00"], title="Mobile"
    )
    company_id: Optional[str] = Field(
        None,
        description="Technical id of the contact' company in Chift",
        examples=["<invoicing-company-id>"],
        title="Company Id",
    )
    vat: Optional[str] = Field(
        None, description="VAT number", examples=["BE0000000000"], title="Vat"
    )
    company_number: Optional[str] = Field(
        None,
        description="Company number (identification number different than the VAT (e.g. siret))",
        examples=["0000000000"],
        title="Company Number",
    )
    currency: Optional[str] = Field(
        None,
        description="Currency matching target sofware name",
        examples=["EUR"],
        title="Currency",
    )
    language: Optional[str] = Field(
        None,
        description="Language matching target sofware name",
        examples=["en"],
        title="Language",
    )
    comment: Optional[str] = Field(
        None,
        description="Comment",
        examples=["Preferred contact: accounts payable team."],
        title="Comment",
    )
    customer_account_number: Optional[str] = Field(
        None,
        description="Number of the accounting account used for sales (e.g. 701000)",
        examples=["<invoicing-account-number>"],
        title="Customer Account Number",
    )
    supplier_account_number: Optional[str] = Field(
        None,
        description="Number of the accounting account used for purchases (e.g. 601000)",
        examples=["<invoicing-account-number>"],
        title="Supplier Account Number",
    )
    birthdate: Optional[Date] = Field(
        None, description="Birthdate", examples=["1990-01-01"], title="Birthdate"
    )
    gender: Optional[ContactGender] = Field(None, description="Gender")
    addresses: Optional[List[AddressItemInInvoicing]] = Field(
        [], description="Addresses", title="Addresses"
    )


class ContactItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    is_prospect: Optional[bool] = Field(
        None, description="Is a prospect?", title="Is Prospect"
    )
    is_customer: Optional[bool] = Field(
        None, description="Is a customer?", title="Is Customer"
    )
    is_supplier: Optional[bool] = Field(
        None, description="Is a supplier?", title="Is Supplier"
    )
    is_company: Optional[bool] = Field(
        None, description="Is a company?", title="Is Company"
    )
    company_name: Optional[str] = Field(
        None,
        description="Name of the company",
        examples=["Example Company S.A."],
        title="Company Name",
    )
    first_name: Optional[str] = Field(
        None, description="Firstname", examples=["John"], title="First Name"
    )
    last_name: Optional[str] = Field(
        None, description="Lastname", examples=["Doe"], title="Last Name"
    )
    email: Optional[str] = Field(
        None, description="Email", examples=["john.doe@example.com"], title="Email"
    )
    phone: Optional[str] = Field(
        None, description="Phone", examples=["+32 2 000 00 00"], title="Phone"
    )
    mobile: Optional[str] = Field(
        None, description="Mobile", examples=["+32 4 000 00 00"], title="Mobile"
    )
    company_id: Optional[str] = Field(
        None,
        description="Technical id of the contact' company in Chift",
        examples=["<invoicing-company-id>"],
        title="Company Id",
    )
    vat: Optional[str] = Field(
        None, description="VAT number", examples=["BE0000000000"], title="Vat"
    )
    company_number: Optional[str] = Field(
        None,
        description="Company number (identification number different than the VAT (e.g. siret))",
        examples=["0000000000"],
        title="Company Number",
    )
    currency: Optional[str] = Field(
        None,
        description="Currency matching target sofware name",
        examples=["EUR"],
        title="Currency",
    )
    language: Optional[str] = Field(
        None,
        description="Language matching target sofware name",
        examples=["en"],
        title="Language",
    )
    comment: Optional[str] = Field(
        None,
        description="Comment",
        examples=["Preferred contact: accounts payable team."],
        title="Comment",
    )
    customer_account_number: Optional[str] = Field(
        None,
        description="Number of the accounting account used for sales (e.g. 701000)",
        examples=["<invoicing-account-number>"],
        title="Customer Account Number",
    )
    supplier_account_number: Optional[str] = Field(
        None,
        description="Number of the accounting account used for purchases (e.g. 601000)",
        examples=["<invoicing-account-number>"],
        title="Supplier Account Number",
    )
    birthdate: Optional[Date] = Field(
        None, description="Birthdate", examples=["1990-01-01"], title="Birthdate"
    )
    gender: Optional[ContactGender] = Field(None, description="Gender")
    addresses: Optional[List[AddressItemOutInvoicing]] = Field(
        [], description="Addresses", title="Addresses"
    )
    external_reference: Optional[str] = Field(
        None,
        description="External reference of the contact in the invoicing system",
        examples=["CONTACT-0001"],
        title="External Reference",
    )


class CoverageItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    operation: str = Field(..., title="Operation")
    status: CoverageStatus


class DataStoreItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., title="Id")
    name: str = Field(..., title="Name")
    status: Optional[DatastoreStatus] = "active"
    definition: DatastoreDef


class DiscountItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: Optional[str] = Field(
        None,
        description="Name of the discount",
        examples=["Percentage discount"],
        title="Name",
    )
    total: float = Field(
        ..., description="Total amount of the discount", examples=[10], title="Total"
    )
    type: Optional[DiscountType] = Field(
        "UNKNOWN", description="Type of the discount", examples=["offered"]
    )


class EcommerceFulfillmentObject(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    fulfillment_type: EcommerceFulfillmentType
    shipping_from_address: Optional[AddressItemOut] = None


class ExpenseItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    employee_id: str = Field(
        ...,
        description="Employee identifier in the accounting system.",
        examples=["<accounting-employee-id>"],
        title="Employee Id",
    )
    total: float = Field(
        ...,
        description="Total amount including taxes for the expense.",
        examples=[242.0],
        title="Total",
    )
    untaxed_amount: float = Field(
        ...,
        description="Total untaxed amount for the expense.",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ...,
        description="Total tax amount for the expense.",
        examples=[42.0],
        title="Tax Amount",
    )
    reference: Optional[str] = Field(
        None,
        description="External reference.",
        examples=["EXP-2026-0001"],
        title="Reference",
    )
    number: Optional[str] = Field(
        None,
        description="Unique number for idempotency in the accounting system.",
        examples=["EXP-2026-0001"],
        title="Number",
    )
    currency: str = Field(
        ...,
        description="Currency of the expense (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, at the expense date. Must be positive if provided.",
        title="Currency Exchange Rate",
    )
    date: Date = Field(
        ..., description="Expense date.", examples=["2026-01-15"], title="Date"
    )
    pdf: Optional[str] = Field(
        None, description="Base64 PDF attachment of the expense document.", title="Pdf"
    )
    pdf_name: Optional[
        constr(
            pattern=r"^[a-zA-Z0-9\-_ ]{1,196}(?:.pdf)?$", min_length=1, max_length=200
        )
    ] = Field(
        None,
        description="A name for the PDF file to be created for accounting software that support it.",
        title="Pdf Name",
    )
    lines: List[ExpenseLineItem] = Field(
        ..., description="Expense lines.", min_length=1, title="Lines"
    )


class ExpenseItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    employee_id: str = Field(
        ...,
        description="Employee identifier in the accounting system.",
        examples=["<accounting-employee-id>"],
        title="Employee Id",
    )
    total: float = Field(
        ...,
        description="Total amount including taxes for the expense.",
        examples=[242.0],
        title="Total",
    )
    untaxed_amount: float = Field(
        ...,
        description="Total untaxed amount for the expense.",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ...,
        description="Total tax amount for the expense.",
        examples=[42.0],
        title="Tax Amount",
    )
    reference: Optional[str] = Field(
        None,
        description="External reference.",
        examples=["EXP-2026-0001"],
        title="Reference",
    )
    number: Optional[str] = Field(
        None,
        description="Unique number for idempotency in the accounting system.",
        examples=["EXP-2026-0001"],
        title="Number",
    )
    currency: str = Field(
        ...,
        description="Currency of the expense (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, at the expense date. Must be positive if provided.",
        title="Currency Exchange Rate",
    )
    date: Date = Field(
        ..., description="Expense date.", examples=["2026-01-15"], title="Date"
    )
    pdf: Optional[str] = Field(
        None, description="Base64 PDF attachment of the expense document.", title="Pdf"
    )
    pdf_name: Optional[
        constr(
            pattern=r"^[a-zA-Z0-9\-_ ]{1,196}(?:.pdf)?$", min_length=1, max_length=200
        )
    ] = Field(
        None,
        description="A name for the PDF file to be created for accounting software that support it.",
        title="Pdf Name",
    )
    lines: List[ExpenseLineItem] = Field(
        ..., description="Expense lines.", min_length=1, title="Lines"
    )
    id: str = Field(
        ...,
        description="Unique id of the expense in the accounting system.",
        examples=["<this-expense-id>"],
        title="Id",
    )


class ExtendedIssueItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., title="Id")
    consumer_id: str = Field(..., title="Consumer Id")
    connection_id: str = Field(..., title="Connection Id")
    integration_id: int = Field(..., title="Integration Id")
    integration_name: str = Field(..., title="Integration Name")
    created_on: DateTime = Field(..., title="Created On")
    updated_on: DateTime = Field(..., title="Updated On")
    last_seen: DateTime = Field(..., title="Last Seen")
    error: ErrorInfo
    occurrences: int = Field(..., title="Occurrences")
    level: IssueLevel
    events: Optional[List[IssueEvent]] = Field([], title="Events")


class FeesItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    created_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Created On"
    )
    type: FeesType
    removed: Optional[bool] = Field(
        False,
        description="Indicates if the fee has been removed from the order",
        title="Removed",
    )
    tax_rate: float = Field(..., examples=[21.0], title="Tax Rate")
    tax_id: Optional[str] = Field(
        None,
        description="Technical id of the tax rate in Chift",
        examples=["<commerce-tax-id>"],
        title="Tax Id",
    )
    discounts: Optional[List[CommerceDiscountItem]] = Field([], title="Discounts")
    untaxed_amount: float = Field(..., examples=[10.0], title="Untaxed Amount")
    tax_amount: float = Field(..., examples=[2.1], title="Tax Amount")
    total: float = Field(..., examples=[12.1], title="Total")


class FieldItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    type: FieldItemType
    display_condition: Optional[Dict[str, Any]] = Field(None, title="Display Condition")
    values: Optional[List[FieldItemValue]] = Field([], title="Values")
    api_route: Optional[str] = Field(None, title="Api Route")
    connection_type: Optional[int] = Field(None, title="Connection Type")


class FinancialEntryLineItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    account_type: EntryLineType = Field(
        ...,
        description="account_type can be either general ('general_account') or thirdparty ('customer', 'supplier' or 'employee')",
    )
    account: constr(min_length=1) = Field(
        ...,
        description="Depending on account_type, if the account_type is 'general_account', this is the general ledger account number used to book the journal item against. If the account_type is 'customer_account', 'supplier_account' or 'employee_account', this is the thirdparty_id (id of the customer, supplier or employee in the accounting system).",
        examples=["<accounting-account-number>"],
        title="Account",
    )
    amount: float = Field(
        ...,
        description="A positive value represents funds received in the bank or cash account (i.e., the bank/cash account is debited and the specified account_number is credited). A negative value represents funds leaving the bank/cash account.",
        examples=[200.0],
        title="Amount",
    )
    description: Optional[str] = Field(
        None,
        description="Line-level text description. Use this to include the most identifying information for the transaction, such as counterparty name, IBAN, or structured communication.",
        examples=["Bank charges"],
        title="Description",
    )
    tax_info: Optional[TaxInfo] = Field(
        None,
        description="Tax information related to the financial entry line item. The provided tax amount is added to the amount of the line item. This is only supported for general accounts.",
    )
    automated_matching_number: Optional[str] = Field(
        None,
        description="Invoice number or document identifier used by the accounting system to perform automated matching (lettering) of open items. When provided, the system will attempt to automatically reconcile the line against the corresponding invoice or document. Not supported on all accounting systems.",
        examples=["<accounting-matching-number>"],
        title="Automated Matching Number",
    )


class FinancialEntryLineItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    account_type: EntryLineType = Field(
        ...,
        description="account_type can be either general ('general_account') or thirdparty ('customer', 'supplier' or 'employee')",
    )
    account: constr(min_length=1) = Field(
        ...,
        description="Depending on account_type, if the account_type is 'general_account', this is the general ledger account number used to book the journal item against. If the account_type is 'customer_account', 'supplier_account' or 'employee_account', this is the thirdparty_id (id of the customer, supplier or employee in the accounting system).",
        examples=["<accounting-account-number>"],
        title="Account",
    )
    amount: float = Field(
        ...,
        description="A positive value represents funds received in the bank or cash account (i.e., the bank/cash account is debited and the specified account_number is credited). A negative value represents funds leaving the bank/cash account.",
        examples=[200.0],
        title="Amount",
    )
    description: Optional[str] = Field(
        None,
        description="Line-level text description. Use this to include the most identifying information for the transaction, such as counterparty name, IBAN, or structured communication.",
        examples=["Bank charges"],
        title="Description",
    )
    tax_info: Optional[TaxInfo] = Field(
        None,
        description="Tax information related to the financial entry line item. The provided tax amount is added to the amount of the line item. This is only supported for general accounts.",
    )
    automated_matching_number: Optional[str] = Field(
        None,
        description="Invoice number or document identifier used by the accounting system to perform automated matching (lettering) of open items. When provided, the system will attempt to automatically reconcile the line against the corresponding invoice or document. Not supported on all accounting systems.",
        examples=["<accounting-matching-number>"],
        title="Automated Matching Number",
    )
    counterpart_account: str = Field(
        ..., examples=["<accounting-account-number>"], title="Counterpart Account"
    )


class FlowConfigInput(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    definitionFields: Optional[List[Dict[str, Any]]] = Field(
        None, title="Definitionfields"
    )
    doorkeyFields: Optional[List[Dict[str, Any]]] = Field(None, title="Doorkeyfields")
    customFields: Optional[List[Dict[str, Any]]] = Field(None, title="Customfields")
    datastores: Optional[List[FlowDataStoreItemInput]] = Field([], title="Datastores")


class FlowConfigOutput(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    definitionFields: Optional[List[Dict[str, Any]]] = Field(
        None, title="Definitionfields"
    )
    doorkeyFields: Optional[List[Dict[str, Any]]] = Field(None, title="Doorkeyfields")
    customFields: Optional[List[Dict[str, Any]]] = Field(None, title="Customfields")
    datastores: Optional[List[FlowDataStoreItemOutput]] = Field([], title="Datastores")


class FlowExecution(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    type: ExecutionType
    data: Optional[Union[FlowExecutionChain, FlowExecutionCode]] = Field(
        ..., title="Data"
    )


class FlowSettings(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    force_from_date: Optional[Date] = Field(None, title="Force From Date")
    force_to_date: Optional[Date] = Field(None, title="Force To Date")
    write_mode: Optional[WriteMode] = "normal"
    import_mode: Optional[ImportMode] = "normal"
    verbose_logs: Optional[bool] = Field(False, title="Verbose Logs")
    extra: Optional[Dict[str, Any]] = Field({}, title="Extra")


class FlowTrigger(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., title="Id")
    type: TriggerType
    cronschedules: Optional[List[str]] = Field(None, title="Cronschedules")
    visible: Optional[bool] = Field(True, title="Visible")
    definitionFields: Optional[List[Dict[str, Any]]] = Field(
        None, title="Definitionfields"
    )
    options: Optional[Dict[str, Any]] = Field(None, title="Options")
    priority: Optional[TriggerPriority] = "NORMAL"


class GenericJournalItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    account_type: EntryLineType = Field(
        ...,
        description="account_type can be either general ('general_account') or thirdparty ('customer', 'supplier' or 'employee')",
    )
    account: Optional[constr(min_length=1)] = Field(
        None,
        description="Depending on account_type, if the account_type is 'general_account', this is the general ledger account number used to book the journal item against. If the account_type is 'customer_account', 'supplier_account' or 'employee_account', this is the thirdparty_id (id of the customer, supplier or employee in the accounting system).",
        examples=["<accounting-account-number>"],
        title="Account",
    )
    force_general_account: Optional[str] = Field(
        None,
        description="Overrides the default general account for customer, supplier, or employee accounts. If provided, this value will be used as the general account instead of the one determined by the accounting system's configuration.",
        examples=["<accounting-account-number>"],
        title="Force General Account",
    )
    prioritise_thirdparty_account: Optional[bool] = Field(
        False,
        description="If True, prioritize using the third-party account information when a general account is forced in accounting software that only allows one account. Recommended to set it to True",
        title="Prioritise Thirdparty Account",
    )
    description: Optional[str] = Field(
        None,
        description="Text description for this journal item. This is typically used to provide additional context or information.",
        examples=["Consulting services"],
        title="Description",
    )
    debit: float = Field(
        ...,
        description="Amount to debit on the account. Must be 0 if credit line is not 0",
        examples=[242.0],
        title="Debit",
    )
    credit: float = Field(
        ...,
        description="Amount to credit on the account. Must be 0 if debit line is not 0",
        examples=[0.0],
        title="Credit",
    )
    analytic_distribution: Optional[List[AnalyticDistribution]] = Field(
        [],
        description="List of analytic distributions for the journal item. Each distribution specifies an analytic plan and the allocation of the journal entry item's amount across analytic accounts using percentages.",
        title="Analytic Distribution",
    )
    tax_code: Optional[str] = Field(
        None,
        description="Indicates the Tax Code used for the entry item. This is the Id of the Tax Code instance in the accounting software.",
        examples=["<accounting-tax-code>"],
        title="Tax Code",
    )
    tax_info: Optional[TaxInfo] = Field(
        None,
        description="Tax information related to the journal item. The provided tax amount is added to the amount (debit or credit) of the journal item. This is only supported for general accounts.",
    )
    account_info: Optional[AccountToCreate] = Field(
        None,
        description="(For certain specific connectors only) Details of the account (third-party or general ledger) to be created if it does not already exist in the accounting system. Some softwares do not support creating accounts via API; in such cases, this information must be provided to allow the accounting software to automatically create the missing account (e.g., for Tiime).",
    )


class HTTPValidationError(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    message: str = Field(..., title="Message")
    status: Optional[str] = Field("error", title="Status")
    detail: Optional[List[ValidationError]] = Field(None, title="Detail")


class IntegrationsCredentialItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    optional: Optional[bool] = Field(False, title="Optional")
    visibility: Optional[Visibility] = Field(
        "both",
        description="Visibility of the credential. If connector is selected, it should be defined when the connector is activated on your environment. If consumer is selected, the end-user will have to provide the credential when creating the connection. If both is selected, the credential can be defined when the connector is activated on your environment or when the end-user creates the connection (it will not be available for the end-user when it's activated at connector-level)",
    )


class IntegrationsPostConnectionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., title="Id")
    title: str = Field(..., title="Title")
    optional: bool = Field(..., title="Optional")
    resource: str = Field(..., title="Resource")
    config: Optional[IntegrationsPostConnectionItemConfig] = Field(
        {}, description="Dict of enabled post-connections on this integration."
    )


class InvoiceLineItemInMultiAnalyticPlans(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    line_number: Optional[int] = Field(
        1,
        description="Optional line sequence number used to preserve the order of invoice items.",
        title="Line Number",
    )
    unit_price: float = Field(
        ...,
        description="Unit price of the item, before tax.",
        examples=[100.0],
        title="Unit Price",
    )
    unit_of_measure: Optional[str] = Field(
        None,
        description="A list of default units can be used for which an advanced mapping has been done (units: 'hour', 'day', 'cm', 'm2', 'm3', 'm', 'km', 'mile', 'g', 'kg', 'ton', 'box', 'pallet', 'roll', 'liter', 'unit'). Custom units of measure must have an identical name in the target software.",
        examples=["unit"],
        title="Unit Of Measure",
    )
    quantity: float = Field(
        ...,
        description="Quantity of items. This is the amount of units sold or purchased.",
        examples=[2.0],
        title="Quantity",
    )
    untaxed_amount: float = Field(
        ...,
        description="Amount excluding tax for the line item (i.e., unit_price × quantity).",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_rate: float = Field(
        ...,
        description="Tax rate used for the line (e.g., 21.0).",
        examples=[21.0],
        title="Tax Rate",
    )
    tax_amount: float = Field(
        ...,
        description="Tax amount calculated for the line (i.e. untaxed_amount * tax_rate).",
        examples=[42.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total amount for the item, including tax (untaxed_amount + tax_amount).",
        examples=[242.0],
        title="Total",
    )
    account_number: constr(min_length=1) = Field(
        ...,
        description="Ledger account number (of type general) used for the line (e.g., 701000).",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    tax_code: constr(min_length=1) = Field(
        ...,
        description="Indicates the tax code used for the line. This is the Id of the Tax Code in the accounting software.",
        examples=["<accounting-tax-code>"],
        title="Tax Code",
    )
    description: Optional[str] = Field(
        "",
        description="Text description for this line item.",
        examples=["Consulting services"],
        title="Description",
    )
    analytic_distribution: Optional[List[AnalyticDistribution]] = Field(
        [], title="Analytic Distribution"
    )


class InvoiceLineItemOutMultiAnalyticPlans(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    line_number: Optional[int] = Field(
        1,
        description="Optional line sequence number used to preserve the order of invoice items.",
        title="Line Number",
    )
    unit_price: float = Field(
        ...,
        description="Unit price of the item, before tax.",
        examples=[100.0],
        title="Unit Price",
    )
    unit_of_measure: Optional[str] = Field(
        None,
        description="A list of default units can be used for which an advanced mapping has been done (units: 'hour', 'day', 'cm', 'm2', 'm3', 'm', 'km', 'mile', 'g', 'kg', 'ton', 'box', 'pallet', 'roll', 'liter', 'unit'). Custom units of measure must have an identical name in the target software.",
        examples=["unit"],
        title="Unit Of Measure",
    )
    quantity: float = Field(
        ...,
        description="Quantity of items. This is the amount of units sold or purchased.",
        examples=[2.0],
        title="Quantity",
    )
    untaxed_amount: float = Field(
        ...,
        description="Amount excluding tax for the line item (i.e., unit_price × quantity).",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_rate: float = Field(
        ...,
        description="Tax rate used for the line (e.g., 21.0).",
        examples=[21.0],
        title="Tax Rate",
    )
    tax_amount: float = Field(
        ...,
        description="Tax amount calculated for the line (i.e. untaxed_amount * tax_rate).",
        examples=[42.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total amount for the item, including tax (untaxed_amount + tax_amount).",
        examples=[242.0],
        title="Total",
    )
    account_number: Optional[str] = Field(
        None,
        description="Ledger account number (of type general) used for the line (e.g., 701000).",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    tax_code: Optional[str] = Field(
        None,
        description="Indicates the tax code used for the line. This is the Id of the Tax Code in the accounting software.",
        examples=["<accounting-tax-code>"],
        title="Tax Code",
    )
    description: Optional[str] = Field(
        "",
        description="Text description for this line.",
        examples=["Consulting services"],
        title="Description",
    )
    analytic_distribution: Optional[List[AnalyticDistribution]] = Field(
        [],
        description="List of analytic distributions for the line item. Each distribution specifies an analytic plan and the associated analytic accounts with their percentages.",
        title="Analytic Distribution",
    )


class InvoicePartnerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    type: PartnerTypeOutput = Field(
        ..., description="Type of the partner", examples=["account"]
    )
    address: Optional[AddressItem] = Field(None, description="Address of the partner")
    first_name: Optional[str] = Field(
        None,
        description="First name of the partner. In case the partner is an individual.",
        examples=["John"],
        title="First Name",
    )
    last_name: Optional[str] = Field(
        None,
        description="Last name of the partner. In case the partner is an individual.",
        examples=["Doe"],
        title="Last Name",
    )
    company_name: Optional[str] = Field(
        None,
        description="Company name of the partner. In case the partner is a company.",
        examples=["Acme Corp"],
        title="Company Name",
    )


class InvoicingBankAccountItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    name: str = Field(
        ...,
        description="Bank account name",
        examples=["Main checking account"],
        title="Name",
    )
    iban: Optional[str] = Field(
        None, description="IBAN", examples=["BE00000000000000"], title="Iban"
    )
    currency: Optional[str] = Field(
        None,
        description="Currency of the bank account",
        examples=["EUR"],
        title="Currency",
    )
    balance: Optional[float] = Field(
        None,
        description="Current balance of the bank account",
        examples=[1000.0],
        title="Balance",
    )


class InvoicingPaymentItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    status: PaymentPaymentStatus = Field(..., description="Payment status")
    description: str = Field(
        ...,
        description="Description",
        examples=["Invoice #INV-2026-0001"],
        title="Description",
    )
    amount: float = Field(..., description="Amount", examples=[242.0], title="Amount")
    currency: str = Field(
        ..., description="Currency", examples=["EUR"], title="Currency"
    )
    payment_date: DateTime = Field(
        ...,
        description="Date of creation",
        examples=["2026-01-15T10:30:00Z"],
        title="Payment Date",
    )
    partner_id: Optional[str] = Field(
        None,
        description="Partner ID",
        examples=["<payment-partner-id>"],
        title="Partner Id",
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical id of the payment method in Chift",
        examples=["<invoicing-payment-method-id>"],
        title="Payment Method Id",
    )
    payment_method_name: Optional[str] = Field(
        None,
        description="Payment method name",
        examples=["Bank transfer"],
        title="Payment Method Name",
    )
    invoice_id: Optional[str] = Field(
        None,
        description="Technical id of the invoice in Chift",
        examples=["<invoicing-invoice-id>"],
        title="Invoice Id",
    )
    invoice_number: Optional[str] = Field(
        None,
        description="Invoice number",
        examples=["INV-2026-0001"],
        title="Invoice Number",
    )


class InvoicingPaymentMethodItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    name: str = Field(
        ..., description="Payment method name", examples=["Bank transfer"], title="Name"
    )


class InvoicingVatCode(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    label: str = Field(..., description="Label", examples=["VAT 21%"], title="Label")
    rate: float = Field(
        ..., description="Tax rate (e.g. 21.0)", examples=[21.0], title="Rate"
    )
    type: VatCodeType = Field(..., description="Type")
    code: Optional[str] = Field(
        None, description="Code", examples=["VAT21"], title="Code"
    )
    scope: Optional[VatCodeScope] = Field("unknown", description="Scope")
    active: Optional[bool] = Field(
        True, description="Is the tax active?", title="Active"
    )


class InvoicingWithholdingTax(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    rate: float = Field(..., description="Rate", examples=[20.0], title="Rate")
    amount: float = Field(..., description="Amount", examples=[40.0], title="Amount")
    reason: Optional[WithholdingTaxReason] = Field(None, description="Reason")
    payment_reason: Optional[WithholdingTaxPaymentReason] = Field(
        None, description="Payment reason"
    )


class IssueItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., title="Id")
    consumer_id: str = Field(..., title="Consumer Id")
    connection_id: str = Field(..., title="Connection Id")
    integration_id: int = Field(..., title="Integration Id")
    integration_name: str = Field(..., title="Integration Name")
    created_on: DateTime = Field(..., title="Created On")
    updated_on: DateTime = Field(..., title="Updated On")
    last_seen: DateTime = Field(..., title="Last Seen")
    error: ErrorInfo
    occurrences: int = Field(..., title="Occurrences")
    level: IssueLevel


class ItemAttachmentInfoOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    status: ItemAttachmentInfoStatus = Field(
        ...,
        description="Indicates whether an attachment (e.g. PDF or image) is available for this entry, and how to retrieve it. 'yes': one or more attachments are available directly, each entry in the attachments array contains a filename and a download URL. 'yes_to_request': an attachment exists but cannot be returned inline. Call GET /accounting/attachments with type and document_id to retrieve the file as a base64-encoded string. The attachments array may be empty in this case. 'no': no attachment is linked to this entry. 'unknown': the connector does not support attachment detection for this provider.",
    )
    attachments: Optional[List[ItemAttachmentInfoAttachment]] = Field(
        [],
        description="List of attachments available directly for this entry. Populated only when status is 'yes'. When status is 'yes_to_request', this list is empty, use GET /accounting/attachments to fetch the file content.",
        title="Attachments",
    )


class Journal(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., examples=["<this-journal-id>"], title="Id")
    code: Optional[str] = Field(None, examples=["BNK"], title="Code")
    name: str = Field(..., examples=["Bank journal"], title="Name")
    journal_type: JournalType = Field(
        ...,
        description="Category of the journal. In retrieved data the values are customer_invoice, customer_refund, supplier_invoice, supplier_refund, financial_operation, miscellaneous_operation, or unknown. 'bank' and 'cash' exist only in the creation model; a journal representing a bank or cash account is retrieved as financial_operation (typically with an iban), so filter on financial_operation, not 'bank'/'cash', to find treasury journals.",
    )
    counterpart_account: Optional[str] = Field(
        None,
        description="Account number of the counterpart account used for the journal, for a treasury journal representing a bank or cash account (retrieved as journal_type financial_operation; bank/cash in the creation model).",
        examples=["<accounting-account-number>"],
        title="Counterpart Account",
    )
    unallocated_account: Optional[str] = Field(
        None,
        description="Unallocated ledger account used to book entries when the final ledger account / client / supplier / employee is not yet known (specific to treasury journals, retrieved as journal_type financial_operation; bank/cash in the creation model).",
        examples=["<accounting-account-number>"],
        title="Unallocated Account",
    )
    next_document_numbers: Optional[List[NextDocumentNumber]] = Field(
        [],
        description="The next document numbers to use when the accounting tool enforces its own numbering sequence on creation (one sequence per journal, per book year, per folder).",
        title="Next Document Numbers",
    )
    iban: Optional[str] = Field(
        None,
        description="IBAN of the bank account linked to the journal, for a treasury journal representing a bank account (retrieved as journal_type financial_operation).",
        examples=["BE00000000000000"],
        title="Iban",
    )
    currency: Optional[str] = Field(
        None,
        description="Currency of the journal (e.g., EUR). If empty, the journal follows the currency of the accounting folder.",
        examples=["EUR"],
        title="Currency",
    )
    other_currencies_allowed: Optional[bool] = Field(
        True,
        description="Indicates if the journal allows other currencies. If it doesn't allow other currencies an exchange rate must be provided for Chift to convert the amounts to the journal currency. This only applicable if the journal currency is the same as the accounting folder currency.",
        title="Other Currencies Allowed",
    )
    blocked: Optional[bool] = Field(
        False,
        description="Indicates if the journal is blocked for creating entries.",
        title="Blocked",
    )


class JournalEntryMonoAnalyticPlan(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    reference: Optional[str] = Field(
        None,
        description="Optional reference field used to store an external or contextual identifier related to the entry. For example, it can be used to store a reference number, the invoice number issued by the suppllier or any other relevant information.",
        examples=["JE-REF-0001"],
        title="Reference",
    )
    due_date: Optional[Date] = Field(
        None,
        description="If the journal entry relates to an invoice, this is the last due date for payment or settlement (format: YYYY-MM-DD), carried from a client/supplier entry line.",
        examples=["2026-02-14"],
        title="Due Date",
    )
    journal_id: str = Field(
        ...,
        description="Id of the journal instance in the accounting system in which the journal entry is recorded.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    name: Optional[str] = Field(
        None,
        description="Mainly the accounting entry number (numéro de pièce comptable) assigned by the source system.",
        examples=["JE-2026-0001"],
        title="Name",
    )
    journal_name: Optional[str] = Field(
        None,
        description="Name of the journal in which the journal entry is recorded.",
        examples=["Bank journal"],
        title="Journal Name",
    )
    date: Optional[Date] = Field(
        "1970-01-01",
        description="Accounting date of the journal entry, which determines the accounting period and therefore the book year the entry falls into. It is automatically set to '1970-01-01' if the value is not available/empty in the accounting system.",
        examples=["2026-01-15"],
        title="Date",
    )
    posted: Optional[bool] = Field(
        False,
        description="Indicates if the journal entry has been posted (finalized) in the accounting system.",
        title="Posted",
    )
    id: str = Field(
        ...,
        description="Unique id of the journal entry instance in the accounting system.",
        examples=["<this-journal-entry-id>"],
        title="Id",
    )
    due_dates: Optional[List[JournalItemDueDatesOut]] = Field(
        [],
        description="List of all due dates of a journal entry. Generally an entry has one client/supplier line per due date, each listed here with its amount.",
        title="Due Dates",
    )
    attachments_info: Optional[ItemAttachmentInfoOut] = Field(
        None,
        description="Indicates whether a file attachment (e.g. PDF or image) is linked to this journal entry and how to retrieve it. Check the status field: 'yes' means a direct download URL is available in the attachments array; 'yes_to_request' means the file exists but must be fetched separately via GET /accounting/attachments?type=entry&document_id={id}, which returns the file as a base64-encoded string. 'no' means no attachment is linked. 'unknown' means the connector does not support this.",
    )
    items: Optional[List[JournalItemMonoAnalyticPlan]] = Field(
        [],
        description="List of journal items that make up the journal entry. Each item represents a line in the journal entry, and the sum of debits must equal the sum of credits to ensure the entry is balanced.",
        title="Items",
    )


class JournalIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    code: constr(min_length=2) = Field(
        ...,
        description="Short code or identifier of the journal (e.g., 'BNK', 'INV').",
        examples=["BNK"],
        title="Code",
    )
    name: constr(min_length=1) = Field(
        ...,
        description="Name or label of the journal as it appears in the accounting system.",
        examples=["Bank journal"],
        title="Name",
    )
    journal_type: JournalInType = Field(..., description="Type of journal. ")
    counterpart_account: Optional[str] = Field(
        None,
        description="When journal is of type bank or cash, this is account number of the counterpart account used for the journal.",
        examples=["<accounting-account-number>"],
        title="Counterpart Account",
    )


class JournalItemMultiAnalyticPlan(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    account_number: constr(min_length=1) = Field(
        ...,
        description="Depending on account_type, if the account_type is 'general_account', this is the general ledger account number used to book the journal item against. If the account_type is 'customer_account', 'supplier_account' or 'employee_account', this is the thirdparty_id (id of the customer, supplier or employee in the accounting system).",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    partner_id: Optional[str] = Field(
        None,
        description="Id of the thirdparty (customer, supplier or employee) in the accounting system if the journal item relates to a thirdparty (e.g., an invoice). The thirdparty line is typically the invoice's total line, i.e. the accounting line tied to a due date / payment term.",
        examples=["<accounting-client-id>"],
        title="Partner Id",
    )
    description: Optional[str] = Field(
        None,
        description="Text description for this journal item. This is typically used to provide additional context or information.",
        examples=["Consulting services"],
        title="Description",
    )
    debit: float = Field(
        ...,
        description="Amount to debit on the account. Must be 0 if credit field is not 0",
        examples=[242.0],
        title="Debit",
    )
    credit: float = Field(
        ...,
        description="Amount to credit on the account. Must be 0 if debit field is not 0",
        examples=[0.0],
        title="Credit",
    )
    currency: constr(min_length=1) = Field(
        ...,
        description="Currency code (e.g., USD, EUR) used for the journal item amounts.",
        examples=["EUR"],
        title="Currency",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, applicable at the date of the journal item. Required when currency is different from the folder's default currency. Amount in folder currency = amount in foreign currency * currency_exchange_rate.",
        title="Currency Exchange Rate",
    )
    id: str = Field(
        ...,
        description="Unique id of the journal item instance in the accounting system.",
        examples=["<this-journal-item-id>"],
        title="Id",
    )
    partner_name: Optional[str] = Field(
        None,
        description="Name of the thirdparty (customer, supplier or employee) linked to the journal item if journal item relates to a thirdparty (e.g., an invoice).",
        examples=["Example Company S.A."],
        title="Partner Name",
    )
    account_name: str = Field(
        ...,
        description="Name or label of the ledger account used to book the journal item against.",
        examples=["Consulting services"],
        title="Account Name",
    )
    matching_numbers: Optional[List[str]] = Field(
        [],
        description="List of matching numbers used to link this journal item to other entries in the accounting system, for reconciliation/lettering. Lines on the same ledger account, in the same book year, carrying the same matching number are lettered (matched) together; when a partner_id is set it is also part of that link. A thirdparty line with debit or credit and no matching number is an open (outstanding) item, and stays outstanding until the matched set balances (sum of debit == sum of credit) within the same book year.",
        examples=[["<accounting-matching-number>"]],
        title="Matching Numbers",
    )
    analytic_distribution: Optional[List[AnalyticDistribution]] = Field(
        [],
        description="List of analytic distributions for the journal item. Each distribution specifies an analytic plan and the allocation of the journal entry item's amount across analytic accounts using percentages.",
        title="Analytic Distribution",
    )


class LinkedDocument(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    amount: float = Field(
        ...,
        description="Amount linked to the document",
        examples=[242.0],
        title="Amount",
    )
    type: LinkedDocumentType = Field(..., description="Type of linked document")


class LocalAgentInfo(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    status: LocalAgentStatus


class MiscellaneousOperationLine(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    line_number: Optional[int] = Field(
        1,
        description="Line number of the operation line. This is used to order the lines in the operation. It is automatically set to 1 if not provided.",
        title="Line Number",
    )
    description: Optional[str] = Field(
        None,
        description="Text description used to provide additional context or information about the operation line.",
        examples=["Bank charges"],
        title="Description",
    )
    amount: float = Field(
        ...,
        description="Positive for Credit and negative for Debit.",
        examples=[200.0],
        title="Amount",
    )
    type: MiscellaneousOperationLineType
    account_number: str = Field(
        ...,
        description="Indicates the account number used in the operation line. This is the ledger account number in the chart of accounts.",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    partner_id: Optional[str] = Field(
        None,
        description="Must be filled in if it is a 'customer_account' or 'supplier_account' line type.",
        examples=["<accounting-client-id>"],
        title="Partner Id",
    )
    analytic_account: Optional[str] = Field(
        None,
        description="Analytic account used in the operation line. This is the id of a analytic account - no distribution possible. It is used to track the operation line in the analytic accounting system.",
        examples=["<accounting-analytic-account-id>"],
        title="Analytic Account",
    )


class MiscellaneousOperationOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    operation_number: Optional[str] = Field(
        None, examples=["OP-2026-0001"], title="Operation Number"
    )
    currency: str = Field(
        ...,
        description="Indicates the currency of the operation (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, at the date of the operation. Must be filled in when creating the operation in another currency from the folder's default currency.",
        title="Currency Exchange Rate",
    )
    lines: List[MiscellaneousOperationLine] = Field(..., title="Lines")
    operation_date: Optional[Date] = Field(
        "1970-01-01",
        description="Accounting date of the miscellaneous operation. It is automatically set to '1970-01-01' if the value is not available/empty in the accounting system.",
        examples=["2026-01-15"],
        title="Operation Date",
    )
    journal_id: str = Field(
        ...,
        description="Indicates the journal used in for the operation.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    status: MiscellaneousOperationStatusOut
    id: str = Field(..., examples=["<this-operation-id>"], title="Id")


class NlPaymentTermsSplit(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    g_account: PaymentTermAccountInfo = Field(
        ...,
        description="This is the 'Geblokkeerde rekening' in Dutch. It is a blocked bank account used only for tax-related payments, such as VAT and payroll tax.",
    )
    n_account: PaymentTermAccountInfo = Field(
        ...,
        description="This is the 'Normale rekening' in Dutch. It is a regular bank account used for standard business operations and unrestricted funds. The non-tax portion of the invoice is paid here.",
    )


class OpportunityItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    name: str = Field(
        ..., description="Name", examples=["New business opportunity"], title="Name"
    )
    total: float = Field(..., description="Total", examples=[1000.0], title="Total")
    probability: float = Field(
        ..., description="Probability of success", examples=[50.0], title="Probability"
    )
    status: OpportunityStatus = Field(..., description="Status")
    partner_id: Optional[str] = Field(
        None,
        description="Technical id of the customer in Chift",
        examples=["<invoicing-partner-id>"],
        title="Partner Id",
    )
    comment: Optional[str] = Field(
        None, description="Comment", examples=["Follow up next week."], title="Comment"
    )
    currency: Optional[str] = Field(
        None,
        description="Currency matching target sofware name",
        examples=["EUR"],
        title="Currency",
    )
    description: Optional[str] = Field(
        None,
        description="Description",
        examples=["Potential new client."],
        title="Description",
    )
    pipe_status: Optional[str] = Field(
        None,
        description="Current stage in pipeline",
        examples=["Qualification"],
        title="Pipe Status",
    )
    pipe_name: Optional[str] = Field(
        None,
        description="Pipeline name",
        examples=["Sales pipeline"],
        title="Pipe Name",
    )
    created_date: Optional[Date] = Field(
        None, description="Date creation", examples=["2026-01-15"], title="Created Date"
    )
    due_date: Optional[Date] = Field(
        None, description="Due Date", examples=["2026-02-14"], title="Due Date"
    )
    end_date: Optional[Date] = Field(
        None, description="End Date", examples=["2026-02-28"], title="End Date"
    )
    is_won: Optional[bool] = Field(None, description="Is won?", title="Is Won")
    owner_ref: Optional[FieldRef] = Field(None, description="Employee/User")


class OrderItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    customer: OrderCustomerItem
    billing_address: CommerceAddressItemIn
    shipping_address: CommerceAddressItemIn
    currency: str = Field(
        ...,
        description="Indicates the currency of the order (e.g. EUR).",
        examples=["EUR"],
        title="Currency",
    )
    note: Optional[str] = Field(
        None, examples=["Please leave at the front door."], title="Note"
    )
    lines: List[OrderLineItemIn] = Field(..., min_length=1, title="Lines")
    payment_method: Optional[str] = Field(
        None,
        description="If filled in the order will be marked as paid and processed by this payment method.",
        examples=["Credit card"],
        title="Payment Method",
    )


class OrderLineItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    created_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Created On"
    )
    variant: Optional[OrderLineProductVariantItem] = Field(
        None, description="Product variant"
    )
    quantity: int = Field(..., examples=[2], title="Quantity")
    current_quantity: int = Field(
        ...,
        description="Quantity without refunds and returns.",
        examples=[2],
        title="Current Quantity",
    )
    unit_price: float = Field(
        ...,
        description="Unit price of the product without taxes.",
        examples=[100.0],
        title="Unit Price",
    )
    description: str = Field(..., examples=["T-Shirt - M"], title="Description")
    tax_id: Optional[str] = Field(
        None,
        description="Technical id of the tax rate in Chift",
        examples=["<commerce-tax-id>"],
        title="Tax Id",
    )
    tax_rate: float = Field(
        ...,
        description="Indicates the tax rate used for the line (e.g. 21.0).",
        examples=[21.0],
        title="Tax Rate",
    )
    untaxed_amount: float = Field(
        ...,
        description="Untaxed amount of the order line with discount.",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ...,
        description="Total taxes applied to the order line.",
        examples=[42.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total of the order line with discount.",
        examples=[242.0],
        title="Total",
    )
    discounts: Optional[List[CommerceDiscountItem]] = Field([], title="Discounts")
    gift_card: Optional[bool] = Field(False, title="Gift Card")
    is_gift: Optional[bool] = Field(False, title="Is Gift")


class OrderRefundItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    created_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Created On"
    )
    total: float = Field(..., examples=[121.0], title="Total")
    reason: Optional[str] = Field(None, examples=["Damaged item"], title="Reason")
    order_lines: Optional[List[RefundOrderLineItem]] = Field([], title="Order Lines")
    other: Optional[float] = Field(0, title="Other")
    shipping_refunds: Optional[List[ShippingRefund]] = Field(
        [], title="Shipping Refunds"
    )
    other_fees: Optional[List[RefundFeesItem]] = Field([], title="Other Fees")
    transactions: Optional[List[OrderTransactions]] = Field([], title="Transactions")


class OrderReturnItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id of the return in Chift",
        examples=["<this-return-id>"],
        title="Id",
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    created_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Created On"
    )
    order_lines: Optional[List[ReturnOrderLineItem]] = Field([], title="Order Lines")
    new_lines: Optional[List[ReturnOrderLineItem]] = Field([], title="New Lines")
    linked_fees: Optional[List[ReturnFeesItem]] = Field([], title="Linked Fees")


class OutstandingItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique id of the outstanding item instance in the accounting system.",
        examples=["<this-outstanding-item-id>"],
        title="Id",
    )
    number: Optional[str] = Field(
        None,
        description="Invoice, Journal entry or Financial entry number of the outstanding item.",
        examples=["INV-2026-0001"],
        title="Number",
    )
    journal_id: str = Field(
        ...,
        description="Id of the journal instance associated with the outstanding item.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    journal_type: JournalType = Field(
        ...,
        description="Type of the journal associated with the outstanding item. This indicates the type of journal in which the outstanding item is recorded, such as 'sales', 'purchases', 'bank', etc. It helps in categorizing the outstanding item based on the nature of the transaction.",
    )
    date: Date = Field(
        ...,
        description="Accounting date of the outstanding item.",
        examples=["2026-01-15"],
        title="Date",
    )
    due_date: Optional[Date] = Field(
        None,
        description="Due date of the outstanding item.",
        examples=["2026-02-14"],
        title="Due Date",
    )
    due_dates: Optional[List[OutstandingItemDueDatesOut]] = Field(
        [], description="List of all due dates of the outstanding.", title="Due Dates"
    )
    currency: str = Field(
        ...,
        description="Currency of the outstanding item (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    currency_exchange_rate: float = Field(
        ...,
        description="Exchange rate from the transaction currency to the folder's base currency, applicable at the date of the outstanding item. Required when currency differs from the folder's default currency.",
        examples=[1.0],
        title="Currency Exchange Rate",
    )
    amount: float = Field(
        ..., description="Total amount", examples=[242.0], title="Amount"
    )
    open_amount: float = Field(
        ..., description="Open amount.", examples=[242.0], title="Open Amount"
    )
    partner_id: str = Field(
        ...,
        description="Id of the thirdparty (customer, supplier or employee) in the accounting system linked to the outstanding item. This is used to identify the entity associated with the outstanding item.",
        examples=["<accounting-client-id>"],
        title="Partner Id",
    )
    account_number: str = Field(
        ...,
        description="Account number in the chart of accounts associated with the outstanding item. This is typically used to book the outstanding item against a specific ledger account.",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    reference: Optional[str] = Field(
        None,
        description="Reference of the outstanding item. This is an optional field that can be used to store additional information or context related to the outstanding item, such as an invoice number or a transaction reference.",
        examples=["REF-0001"],
        title="Reference",
    )
    matching_numbers: Optional[List[str]] = Field(
        [],
        description="List of matching numbers associated with the outstanding item. This is used to track any matching operations that have been performed on the outstanding item, such as payments or adjustments.",
        examples=[["<accounting-matching-number>"]],
        title="Matching Numbers",
    )
    payment_communication: Optional[str] = Field(
        None,
        description="Payment communication associated with the outstanding item. This is an optional field that can be used to store additional information related to the payment",
        examples=["+++000/0000/00000+++"],
        title="Payment Communication",
    )
    posted: bool = Field(
        ...,
        description="Indicates if the outstanding item has been posted (finalized) in the accounting system. If not provided, it defaults to True.",
        title="Posted",
    )
    original_document: Optional[OriginalOutstandingItem] = Field(
        None,
        description="Details of the original document associated with the outstanding item. This is used to link the outstanding item to its original source when oustanding item was initially posted in previous bookyear.",
    )


class PMSAccountingCategoryItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the accounting category",
        examples=["<this-accounting-category-id>"],
        title="Id",
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    name: str = Field(
        ...,
        description="Name given to the accounting category",
        examples=["Beverages"],
        title="Name",
    )
    code: Optional[str] = Field(
        None,
        description="Code assigned to the category",
        examples=["123456"],
        title="Code",
    )
    ledger_account_code: Optional[str] = Field(
        None,
        description="Ledger account code assigned to the category",
        examples=["<pos-ledger-account-code>"],
        title="Ledger Account Code",
    )
    posting_account_code: Optional[str] = Field(
        None,
        description="Posting account code assigned to the category",
        examples=["<pos-posting-account-code>"],
        title="Posting Account Code",
    )


class PMSAccountingTransactionOrigin(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Technical id in the target software",
        examples=["<pms-origin-id>"],
        title="Id",
    )
    type: PMSAccountingTransactionOriginType = Field(
        ..., description="Type of the transaction", examples=["invoice"]
    )


class PMSCustomerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    first_name: Optional[str] = Field(
        None,
        description="First name of the customer. In case the customer is an individual.",
        examples=["John"],
        title="First Name",
    )
    last_name: Optional[str] = Field(
        None,
        description="Last name of the customer. In case the customer is an individual.",
        examples=["Doe"],
        title="Last Name",
    )
    company_name: Optional[str] = Field(
        None,
        description="Company name of the customer. In case the customer is a company.",
        examples=["Acme Corp"],
        title="Company Name",
    )
    phone: Optional[str] = Field(
        None,
        description="Phone number of the customer",
        examples=["+1234567890"],
        title="Phone",
    )
    email: Optional[str] = Field(
        None,
        description="Email address of the customer",
        examples=["john.doe@example.com"],
        title="Email",
    )
    account_number: Optional[str] = Field(
        None,
        description="Number of the accounting account used (e.g. 701000)",
        examples=["<pms-account-number>"],
        title="Account Number",
    )
    created_on: Optional[DateTime] = Field(
        None,
        description="Creation date of the customer",
        examples=["2023-09-25T12:00:00"],
        title="Created On",
    )
    addresses: Optional[List[AddressItem]] = Field(
        None, description="List of addresses related to the customer", title="Addresses"
    )
    is_company: Optional[bool] = Field(
        False,
        description="Indicates if the client is an individual or a company.",
        title="Is Company",
    )


class PMSOrderLineItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the order line item",
        examples=["<this-order-line-id>"],
        title="Id",
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    type: Optional[POSLineItemType] = Field(
        None, description="Type of the order line item", examples=["product"]
    )
    menu_id: Optional[str] = Field(
        None,
        description="Unique identifier of the menu",
        examples=["<pos-menu-id>"],
        title="Menu Id",
    )
    quantity: float = Field(
        ...,
        description="Quantity of the order line item",
        examples=[1],
        title="Quantity",
    )
    unit_price: float = Field(
        ...,
        description="Unit price (without tax) of the order line item",
        examples=[10],
        title="Unit Price",
    )
    total: float = Field(
        ...,
        description="Total amount including tax of the order line item",
        examples=[11],
        title="Total",
    )
    tax_amount: float = Field(
        ...,
        description="Total amount of the taxes of the order line item",
        examples=[1],
        title="Tax Amount",
    )
    tax_rate: Optional[float] = Field(
        None,
        description="Tax rate of the order line item",
        examples=[10],
        title="Tax Rate",
    )
    tax_id: Optional[str] = Field(
        None,
        description="Unique identifier of the tax of the order line item",
        examples=["<pos-tax-id>"],
        title="Tax Id",
    )
    description: Optional[str] = Field(
        None,
        description="Description of the order line item",
        examples=["Pizza"],
        title="Description",
    )
    discounts: Optional[List[DiscountItem]] = Field(
        [],
        description="List of discounts of the order line item. Discounts of items are always part of the total discount of the order",
        examples=[[]],
        title="Discounts",
    )
    product_id: Optional[str] = Field(
        None,
        description="Reference to the product related to this item",
        examples=["<pos-product-id>"],
        title="Product Id",
    )
    accounting_category_id: Optional[str] = Field(
        None,
        description="Sometimes used by a POS to give a specific accounting category to an order item",
        examples=["<pos-accounting-category-id>"],
        title="Accounting Category Id",
    )


class PMSPaymentItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: Optional[str] = Field(
        None,
        description="Unique identifier of the payment",
        examples=["<this-payment-id>"],
        title="Id",
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    payment_method_id: Optional[str] = Field(
        None,
        description="Unique identifier of the payment method used for the payment",
        examples=["<pos-payment-method-id>"],
        title="Payment Method Id",
    )
    payment_method_name: Optional[str] = Field(
        None,
        description="Chift's name of the payment method used for the payment",
        examples=["Cash"],
        title="Payment Method Name",
    )
    total: float = Field(
        ..., description="Total amount of the payment", examples=[11], title="Total"
    )
    tip: Optional[float] = Field(
        "0",
        description="Total amount of the tips of the payment. Tips are not part of the total of the payment",
        examples=[1],
        title="Tip",
    )
    status: Optional[POSPaymentStatus] = Field(
        "Unknown", description="Status of the payment", examples=["Completed"]
    )
    currency: Optional[str] = Field(
        None, description="Currency of the payment", examples=["EUR"], title="Currency"
    )
    date: Optional[DateTime] = Field(
        None,
        description="Date of the payment",
        examples=["2025-01-01T00:00:00Z"],
        title="Date",
    )
    invoice_ref: Optional[str] = Field(
        None,
        description="Reference of the invoice linked to the payment",
        examples=["<pos-invoice-id>"],
        title="Invoice Ref",
    )
    partner_id: Optional[ChiftId] = Field(
        None, description="Reference to the customer related to this payment"
    )
    description: Optional[str] = Field(
        None,
        description="Description of the payment",
        examples=["Payment for invoice INV-12345"],
        title="Description",
    )


class PMSPaymentMethods(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the payment method",
        examples=["<this-payment-method-id>"],
        title="Id",
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    name: str = Field(
        ...,
        description="Chift's name of the payment method (e.g. 'Cash', 'Card', 'Transfer', ...)",
        examples=["Cash"],
        title="Name",
    )
    extra: Optional[str] = Field(
        None,
        description="Original name of the payment method",
        examples=["Espèces"],
        title="Extra",
    )
    ledger_account_code: Optional[str] = Field(
        None,
        description="Ledger account code assigned to the category",
        examples=["<pms-ledger-account-code>"],
        title="Ledger Account Code",
    )


class PMSTaxRateItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    label: Optional[str] = Field(
        None, description="Label of the tax rate", examples=["VAT 21%"], title="Label"
    )
    rate: Optional[float] = Field(
        None, description="Percentage of the tax rate", examples=[21.0], title="Rate"
    )
    is_city_tax: Optional[bool] = Field(
        None,
        description="Indicates if the tax is a city tax (or tourist tax)",
        examples=[False],
        title="Is City Tax",
    )


class POSClosureInformationItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., examples=["<this-closure-id>"], title="Id")
    total: float = Field(
        ...,
        description="Total amount of the payments in current closure",
        examples=[100.0],
        title="Total",
    )
    tax_amount: float = Field(
        ...,
        description="Total tax amount the in current closure",
        examples=[21.0],
        title="Tax Amount",
    )
    payments: Optional[List[ClosurePaymentItem]] = Field(
        [], description="List of payments related to current closure", title="Payments"
    )
    taxes: Optional[List[TotalTaxItem]] = Field(
        [], description="List of taxes related to current closure", title="Taxes"
    )
    operations: Optional[List[ClosureOperationItem]] = Field(
        [],
        description="List of cash register movements related to current closure",
        title="Operations",
    )


class POSCreateCustomerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    first_name: constr(min_length=1) = Field(
        ...,
        description="First name of the customer",
        examples=["John"],
        title="First Name",
    )
    last_name: constr(min_length=1) = Field(
        ...,
        description="Last name of the customer",
        examples=["Doe"],
        title="Last Name",
    )
    phone: Optional[str] = Field(
        None,
        description="Phone number (mobile, home, work, ...) of the customer",
        examples=["+32 4 000 00 00"],
        title="Phone",
    )
    email: Optional[str] = Field(
        None,
        description="Email address of the customer",
        examples=["john.doe@example.com"],
        title="Email",
    )
    address: Optional[PostAddressItem] = Field(
        None, description="Address of the customer"
    )


class POSItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the order line item",
        examples=["<this-order-line-id>"],
        title="Id",
    )
    type: Optional[POSLineItemType] = Field(
        None, description="Type of the order line item", examples=["product"]
    )
    menu_id: Optional[str] = Field(
        None,
        description="Unique identifier of the menu",
        examples=["<pos-menu-id>"],
        title="Menu Id",
    )
    quantity: float = Field(
        ...,
        description="Quantity of the order line item",
        examples=[1],
        title="Quantity",
    )
    unit_price: float = Field(
        ...,
        description="Unit price (without tax) of the order line item",
        examples=[10],
        title="Unit Price",
    )
    total: float = Field(
        ...,
        description="Total amount including tax of the order line item",
        examples=[11],
        title="Total",
    )
    tax_amount: float = Field(
        ...,
        description="Total amount of the taxes of the order line item",
        examples=[1],
        title="Tax Amount",
    )
    tax_rate: Optional[float] = Field(
        None,
        description="Tax rate of the order line item",
        examples=[10],
        title="Tax Rate",
    )
    tax_id: Optional[str] = Field(
        None,
        description="Unique identifier of the tax of the order line item",
        examples=["<pos-tax-id>"],
        title="Tax Id",
    )
    description: Optional[str] = Field(
        None,
        description="Description of the order line item",
        examples=["Pizza"],
        title="Description",
    )
    discounts: Optional[List[DiscountItem]] = Field(
        [],
        description="List of discounts of the order line item. Discounts of items are always part of the total discount of the order",
        examples=[[]],
        title="Discounts",
    )
    product_id: Optional[str] = Field(
        None,
        description="Reference to the product related to this item",
        examples=["<pos-product-id>"],
        title="Product Id",
    )
    accounting_category_id: Optional[str] = Field(
        None,
        description="Sometimes used by a POS to give a specific accounting category to an order item",
        examples=["<pos-accounting-category-id>"],
        title="Accounting Category Id",
    )
    modifiers: Optional[List[ModifierLine]] = Field(
        [], description="List of modifiers", title="Modifiers"
    )


class POSPaymentItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: Optional[str] = Field(
        None,
        description="Unique identifier of the payment",
        examples=["<this-payment-id>"],
        title="Id",
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Unique identifier of the payment method used for the payment",
        examples=["<pos-payment-method-id>"],
        title="Payment Method Id",
    )
    payment_method_name: Optional[str] = Field(
        None,
        description="Chift's name of the payment method used for the payment",
        examples=["Cash"],
        title="Payment Method Name",
    )
    total: float = Field(
        ..., description="Total amount of the payment", examples=[11], title="Total"
    )
    tip: Optional[float] = Field(
        "0",
        description="Total amount of the tips of the payment. Tips are not part of the total of the payment",
        examples=[1],
        title="Tip",
    )
    status: Optional[POSPaymentStatus] = Field(
        "Unknown", description="Status of the payment", examples=["Completed"]
    )
    currency: Optional[str] = Field(
        None, description="Currency of the payment", examples=["EUR"], title="Currency"
    )
    date: Optional[DateTime] = Field(
        None,
        description="Date of the payment",
        examples=["2025-01-01T00:00:00Z"],
        title="Date",
    )
    invoice_ref: Optional[str] = Field(
        None,
        description="Reference of the invoice linked to the payment",
        examples=["<pos-invoice-id>"],
        title="Invoice Ref",
    )
    order_id: Optional[str] = Field(
        None,
        description="Unique identifier of the order",
        examples=["<pos-order-id>"],
        title="Order Id",
    )


class POSProductItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the product",
        examples=["<this-product-id>"],
        title="Id",
    )
    categories: Optional[List[str]] = Field(
        [],
        description="List of categories the product belongs to",
        examples=["<pos-product-category-id>"],
        title="Categories",
    )
    name: str = Field(
        ...,
        description="Name ofthe product",
        examples=["Pizza margheritta"],
        title="Name",
    )
    description: Optional[str] = Field(
        None,
        description="Description of the product",
        examples=["Pizza margheritta"],
        title="Description",
    )
    prices: List[ProductPriceItem] = Field(
        ..., description="List of prices for the product", title="Prices"
    )
    modifier_groups_ids: Optional[List[str]] = Field(
        [],
        description="List of ids of the modifier groups",
        examples=[["<pos-modifier-group-id>"]],
        title="Modifier Groups Ids",
    )
    accounting_category_ids: Optional[List[str]] = Field(
        None,
        description="Used by a POS to give one or more specific accounting categories to a product item. If not available it will use the category ids",
        examples=["<pos-accounting-category-id>"],
        title="Accounting Category Ids",
    )


class Partner(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    external_reference: Optional[str] = Field(
        None,
        description="External identifier used to link the client in the accounting system with the corresponding client reference in your own system. Normally unique per client/supplier type and can serve as the primary key of the accounting system.",
        examples=["CUST-0001"],
        title="External Reference",
    )
    first_name: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the first name of the client.",
        examples=["John"],
        title="First Name",
    )
    last_name: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the last name of the client.",
        examples=["Doe"],
        title="Last Name",
    )
    name: Optional[str] = Field(
        None,
        description="Full name or legal name of the client (individual or company). This is the name displayed in the accounting software.",
        examples=["Example Company S.A."],
        title="Name",
    )
    function: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Job function/title of the client.",
        examples=["Accountant"],
        title="Function",
    )
    is_company: Optional[bool] = Field(
        True,
        description="Indicates if the client is an individual or a company.",
        title="Is Company",
    )
    company_id: Optional[str] = Field(
        None,
        description="Only used when the client is an individual (is_company=false). Indicates the id of the company linked to the client.",
        examples=["<accounting-company-id>"],
        title="Company Id",
    )
    phone: Optional[str] = Field(
        None,
        description="Phone number of the client.",
        examples=["+32 2 000 00 00"],
        title="Phone",
    )
    mobile: Optional[str] = Field(
        None,
        description="Mobile phone number of the client.",
        examples=["+32 4 000 00 00"],
        title="Mobile",
    )
    email: Optional[str] = Field(
        None,
        description="Email address of the client.",
        examples=["john.doe@example.com"],
        title="Email",
    )
    language: Optional[str] = Field(
        None,
        description="Language of the client. Format: ISO 639-1 codes.",
        examples=["en"],
        title="Language",
    )
    internal_notes: Optional[str] = Field(
        None,
        description="Internal notes about the client.",
        examples=["Preferred contact: accounts payable team."],
        title="Internal Notes",
    )
    website: Optional[str] = Field(
        None,
        description="Website of the client.",
        examples=["https://www.example.com"],
        title="Website",
    )
    vat: Optional[str] = Field(
        None,
        description="VAT number of the client.",
        examples=["BE0000000000"],
        title="Vat",
    )
    company_number: Optional[str] = Field(
        None,
        description="National identification number of the company.",
        examples=["0000000000"],
        title="Company Number",
    )
    iban: Optional[str] = Field(
        None,
        description="IBAN account number of the client. DEPRECATED see bank_accounts",
        examples=["BE00000000000000"],
        title="Iban",
    )
    bank_account: Optional[str] = Field(
        None,
        description="Bank account number of the client. DEPRECATED see bank_accounts",
        examples=["0000000000"],
        title="Bank Account",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the client (e.g., EUR).",
        examples=["EUR"],
        title="Currency",
    )
    active: Optional[bool] = Field(
        True, description="Indicates if the client is active.", title="Active"
    )
    account_number: Optional[str] = Field(
        None,
        description="General ledger account assigned to the customer in the accounting system as it will appear in the official accounting export file (FEC, SIE, iXBRL, etc.). This is the account used on journal entry lines to debit/credit this partner.",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    id: Optional[str] = Field(
        None,
        description="Id of the client in the accounting software. This is the unique identifier used to reference the client in the system.",
        examples=["<this-client-id>"],
        title="Id",
    )
    last_updated_on: Optional[DateTime] = Field(
        None,
        description="The last time the client has been updated.",
        examples=["2026-01-15T10:30:00Z"],
        title="Last Updated On",
    )
    addresses: Optional[List[AddressItemOut]] = Field(
        [],
        description="List of addresses associated with the client.",
        title="Addresses",
    )
    third_party_account: Optional[str] = Field(
        None,
        description="Third party account number/code representing the client in the accounting software.",
        title="Third Party Account",
    )
    bank_accounts: Optional[List[BankAccountsItem]] = Field(
        None,
        description="List of bank accounts information associated with the client.",
        title="Bank Accounts",
    )


class PaymentItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    status: PaymentPaymentStatus = Field(..., description="Payment status")
    description: str = Field(
        ...,
        description="Description",
        examples=["Invoice #INV-2026-0001"],
        title="Description",
    )
    amount: float = Field(..., description="Amount", examples=[242.0], title="Amount")
    currency: str = Field(
        ..., description="Currency", examples=["EUR"], title="Currency"
    )
    payment_date: DateTime = Field(
        ...,
        description="Date of creation",
        examples=["2026-01-15T10:30:00Z"],
        title="Payment Date",
    )
    partner_id: Optional[str] = Field(
        None,
        description="Partner ID",
        examples=["<payment-partner-id>"],
        title="Partner Id",
    )


class PaymentMethodItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    name: str = Field(..., examples=["Credit card"], title="Name")
    active: bool = Field(..., title="Active")


class PaymentReporting(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    method: Optional[PaymentReportingMethod] = Field(None, description="Payment method")
    conditions: Optional[PaymentReportingConditions] = Field(
        None, description="Payment conditions"
    )


class PayoutItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    status: PayoutStatus = Field(..., description="Payout status")
    type: PayoutType = Field(..., description="Payout type")
    amount: float = Field(
        ...,
        description="Amount transferred to the merchant",
        examples=[1000.0],
        title="Amount",
    )
    currency: str = Field(
        ..., description="Currency", examples=["EUR"], title="Currency"
    )
    fee: Optional[float] = Field(
        None,
        description="Total fee applied by the processor",
        examples=[5.0],
        title="Fee",
    )
    extra_fee: Optional[float] = Field(
        None,
        description="Extra fee, if exposed by the processor",
        examples=[0.0],
        title="Extra Fee",
    )
    payment_date: Date = Field(
        ...,
        description="Date at which the payout was made",
        examples=["2026-01-15"],
        title="Payment Date",
    )


class PostSyncFlowEvent(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    consumers: Optional[List[Union[UUID, str]]] = Field(None, title="Consumers")
    data: Optional[PostSyncFlowEventData] = None


class ProductItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    name: str = Field(
        ..., description="Name", examples=["Consulting services"], title="Name"
    )
    unit_price: Optional[float] = Field(
        None, description="Unit price", examples=[100.0], title="Unit Price"
    )
    tax_id: Optional[str] = Field(
        None,
        description="Technical id of the tax in Chift",
        examples=["<invoicing-tax-id>"],
        title="Tax Id",
    )
    code: Optional[str] = Field(
        None, description="Reference/code", examples=["SKU-0001"], title="Code"
    )
    unit: Optional[str] = Field(
        None,
        description="Unit of measure matching target software name",
        examples=["unit"],
        title="Unit",
    )
    category: Optional[str] = Field(
        None,
        description="Category matching target software name",
        examples=["Services"],
        title="Category",
    )
    currency: Optional[str] = Field(
        None,
        description="Currency matching target software name",
        examples=["EUR"],
        title="Currency",
    )
    description: Optional[str] = Field(
        None,
        description="Description",
        examples=["Consulting services"],
        title="Description",
    )
    cost: Optional[float] = Field(
        0, description="Cost of the product", examples=[0.0], title="Cost"
    )
    available_quantity: Optional[float] = Field(
        0,
        description="Available quanity of the product in stock. Will only be used for products that have an inventory (services will always be 0)",
        title="Available Quantity",
    )


class ProductVariantItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    parent_id: str = Field(
        ...,
        description="Technical id of the parent product in Chift",
        examples=["<commerce-product-id>"],
        title="Parent Id",
    )
    name: str = Field(..., examples=["T-Shirt - M"], title="Name")
    description: Optional[str] = Field(
        None,
        examples=["Classic cotton T-Shirt, available in several sizes."],
        title="Description",
    )
    description_html: Optional[str] = Field(
        None,
        examples=["<p>Classic cotton T-Shirt, available in several sizes.</p>"],
        title="Description Html",
    )
    categories: Optional[List[CommerceProductCategoryItem]] = Field(
        [], title="Categories"
    )
    created_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Created On"
    )
    sku: Optional[str] = Field(None, examples=["TSHIRT-M"], title="Sku")
    taxable: Optional[bool] = Field(None, title="Taxable")
    barcode: Optional[str] = Field(None, examples=["5410000000000"], title="Barcode")
    available_quantity: Optional[float] = Field(
        0, examples=[10.0], title="Available Quantity"
    )
    prices: Optional[List[CommerceProductPriceItem]] = Field([], title="Prices")
    unit_cost: Optional[ProductCostItem] = None
    unit_of_measure: Optional[str] = Field(
        None, examples=["piece"], title="Unit Of Measure"
    )
    weight: Optional[float] = Field(0, examples=[0.2], title="Weight")
    weight_unit: Optional[str] = Field(None, examples=["kg"], title="Weight Unit")
    status: Optional[ProductStatus] = None
    inventory_details: Optional[List[InventoryDetailsItem]] = Field(
        [], title="Inventory Details"
    )
    variant_attributes: Optional[List[VariantAttributeItem]] = Field(
        [],
        description="List of attributes that are specific to the variant of the product.",
        title="Variant Attributes",
    )
    variant_images: Optional[List[ImageItem]] = Field(
        [],
        description="List of images that are specific to the variant of the product.",
        title="Variant Images",
    )


class PublicCubeMetaCube(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    type: Optional[Type] = Field(None, title="Type")
    title: Optional[str] = Field(None, title="Title")
    description: Optional[str] = Field(None, title="Description")
    connectedComponent: Optional[int] = Field(None, title="Connectedcomponent")
    measures: Optional[List[PublicCubeMetaMember]] = Field(None, title="Measures")
    dimensions: Optional[List[PublicCubeMetaMember]] = Field(None, title="Dimensions")
    hierarchies: Optional[List[Dict[str, Any]]] = Field(None, title="Hierarchies")
    segments: Optional[List[PublicCubeMetaMember]] = Field(None, title="Segments")
    relationships: Optional[List[PublicCubeRelationship]] = Field(
        None, title="Relationships"
    )
    domain: Optional[str] = Field(None, title="Domain")
    queryGroup: Optional[str] = Field(None, title="Querygroup")


class ReadFlowItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    description: Optional[str] = Field(None, title="Description")
    id: str = Field(..., title="Id")
    execution: Optional[FlowExecution] = None
    config: Optional[FlowConfigOutput] = {"datastores": []}
    triggers: Optional[List[FlowTrigger]] = Field(None, title="Triggers")


class ReadFlowTrigger(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(..., title="Id")
    type: TriggerType
    cronschedule: Optional[str] = Field(None, title="Cronschedule")


class ReadSyncSkipMappingItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    skip_reason: SyncSkipReason


class ReportOrders(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    currency: str = Field(
        ...,
        description="Currency of the report (ISO 4217)",
        examples=["EUR"],
        title="Currency",
    )
    total: float = Field(
        ...,
        description="Total amount including tax of the sales",
        examples=[10],
        title="Total",
    )
    tax_amount: float = Field(
        ..., description="Total amount of the taxes", examples=[1], title="Tax Amount"
    )
    tax_duedatetypecode: Optional[constr(max_length=3)] = Field(
        None,
        description="Code giving specific meaning to a date, time or period",
        examples=["4"],
        title="Tax Duedatetypecode",
    )
    category: ReportCategory = Field(
        ..., description="Category of the orders", examples=["TLB1"]
    )
    total_orders: int = Field(
        ...,
        description="Total number of orders for the period",
        examples=[10],
        title="Total Orders",
    )
    taxes: Optional[List[TotalTaxItem]] = Field(
        [], description="List of taxes related to the report", title="Taxes"
    )


class ReservationItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    start_date: Optional[DateTime] = Field(
        None,
        description="Start date of the reservation",
        examples=["2024-10-01T12:00:00"],
        title="Start Date",
    )
    end_date: Optional[DateTime] = Field(
        None,
        description="End date of the reservation",
        examples=["2024-10-10T12:00:00"],
        title="End Date",
    )
    creation_date: Optional[DateTime] = Field(
        None,
        description="Creation date of the reservation",
        examples=["2024-09-25T12:00:00"],
        title="Creation Date",
    )
    resource_id: Optional[ChiftId] = Field(
        None, description="Unique reference to the resource related to the reservation"
    )
    resource_name: Optional[str] = Field(
        None,
        description="Name of the resource related to the reservation",
        examples=["Room 101"],
        title="Resource Name",
    )
    resource_identifier: Optional[str] = Field(
        None,
        description="Identifier of the resource related to the reservation",
        examples=["R101"],
        title="Resource Identifier",
    )


class SalesItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    total: float = Field(
        ...,
        description="Total amount including tax of the sales",
        examples=[10],
        title="Total",
    )
    tax_amount: float = Field(
        ..., description="Total amount of the taxes", examples=[1], title="Tax Amount"
    )
    taxes: Optional[List[TotalTaxItem]] = Field(
        [],
        description="List of taxes grouped by tax rateapplied to the sales",
        title="Taxes",
    )


class SettingField(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    key: str = Field(..., title="Key")
    type: SettingType
    group: SettingGroup
    title: str = Field(..., title="Title")
    tooltip: str = Field(..., title="Tooltip")
    default: Optional[Any] = Field(None, title="Default")
    options: Optional[List[SettingOption]] = Field([], title="Options")
    supported: Optional[bool] = Field(True, title="Supported")
    unsupported_reason: Optional[str] = Field(None, title="Unsupported Reason")


class SpanishSpecificities(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    withholding_tax: Optional[WithholdingTax] = Field(
        None, description="Withholding tax information for the invoice."
    )


class SupplierItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    external_reference: Optional[str] = Field(
        None,
        description="External identifier used to link the supplier in the accounting system with the corresponding supplier reference in your own system.",
        examples=["SUPP-0001"],
        title="External Reference",
    )
    first_name: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the first name of the supplier.",
        examples=["John"],
        title="First Name",
    )
    last_name: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the last name of the supplier.",
        examples=["Doe"],
        title="Last Name",
    )
    name: constr(min_length=1) = Field(
        ...,
        description="Full name or legal name of the supplier (individual or company). This is the name that will be displayed in the accounting software.",
        examples=["Example Company S.A."],
        title="Name",
    )
    function: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the function of the supplier.",
        examples=["Accountant"],
        title="Function",
    )
    is_company: Optional[bool] = Field(
        True,
        description="Indicates if the supplier is an individual or a company.",
        title="Is Company",
    )
    company_id: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the id of the company linked to the supplier.",
        examples=["<accounting-company-id>"],
        title="Company Id",
    )
    phone: Optional[str] = Field(
        None,
        description="Phone number of the supplier.",
        examples=["+32 2 000 00 00"],
        title="Phone",
    )
    mobile: Optional[str] = Field(
        None,
        description="Mobile phone number of the supplier.",
        examples=["+32 4 000 00 00"],
        title="Mobile",
    )
    email: Optional[str] = Field(
        None,
        description="Email address of the supplier.",
        examples=["john.doe@example.com"],
        title="Email",
    )
    language: Optional[str] = Field(
        None,
        description="Language of the supplier. Format: ISO 639-1 codes.",
        examples=["en"],
        title="Language",
    )
    internal_notes: Optional[str] = Field(
        None,
        description="Internal notes about the supplier.",
        examples=["Preferred contact: accounts receivable team."],
        title="Internal Notes",
    )
    website: Optional[str] = Field(
        None,
        description="Website of the supplier.",
        examples=["https://www.example.com"],
        title="Website",
    )
    vat: Optional[str] = Field(
        None,
        description="VAT number of the supplier.",
        examples=["BE0000000000"],
        title="Vat",
    )
    company_number: Optional[str] = Field(
        None,
        description="National identification number of the company.",
        examples=["0000000000"],
        title="Company Number",
    )
    iban: Optional[str] = Field(
        None,
        description="IBAN account number of the supplier.",
        examples=["BE00000000000000"],
        title="Iban",
    )
    bank_account: Optional[str] = Field(
        None,
        description="Bank account number of the supplier.",
        examples=["0000000000"],
        title="Bank Account",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the supplier (e.g., EUR).",
        examples=["EUR"],
        title="Currency",
    )
    active: Optional[bool] = Field(
        True, description="Indicates if the supplier is active.", title="Active"
    )
    addresses: List[AddressItemIn] = Field(
        ...,
        description="List of addresses associated with the supplier.",
        title="Addresses",
    )
    account_number: Optional[str] = Field(
        None,
        description="Number of ledger account assigned to the supplier in the accounting system as it will appear in the official accounting export file (FEC, SIE, iXBRL, etc.). This is typically the ledger account used for posting payables. In some systems, each supplier has a dedicated account; in others, a shared control account is used and supplier balances are tracked in a subledger.",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )


class SupplierItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    external_reference: Optional[str] = Field(
        None,
        description="External identifier used to link the supplier in the accounting system with the corresponding supplier reference in your own system.",
        examples=["SUPP-0001"],
        title="External Reference",
    )
    first_name: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the first name of the supplier.",
        examples=["John"],
        title="First Name",
    )
    last_name: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the last name of the supplier.",
        examples=["Doe"],
        title="Last Name",
    )
    name: Optional[str] = Field(
        None,
        description="Full name or legal name of the supplier (individual or company). This is the name displayed in the accounting software.",
        examples=["Example Company S.A."],
        title="Name",
    )
    function: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the function of the supplier.",
        examples=["Accountant"],
        title="Function",
    )
    is_company: Optional[bool] = Field(
        True,
        description="Indicates if the supplier is an individual or a company.",
        title="Is Company",
    )
    company_id: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the id of the company linked to the supplier.",
        examples=["<accounting-company-id>"],
        title="Company Id",
    )
    phone: Optional[str] = Field(
        None,
        description="Phone number of the supplier.",
        examples=["+32 2 000 00 00"],
        title="Phone",
    )
    mobile: Optional[str] = Field(
        None,
        description="Mobile phone number of the supplier.",
        examples=["+32 4 000 00 00"],
        title="Mobile",
    )
    email: Optional[str] = Field(
        None,
        description="Email address of the supplier.",
        examples=["john.doe@example.com"],
        title="Email",
    )
    language: Optional[str] = Field(
        None,
        description="Language of the supplier. Format: ISO 639-1 codes.",
        examples=["en"],
        title="Language",
    )
    internal_notes: Optional[str] = Field(
        None,
        description="Internal notes about the supplier.",
        examples=["Preferred contact: accounts receivable team."],
        title="Internal Notes",
    )
    website: Optional[str] = Field(
        None,
        description="Website of the supplier.",
        examples=["https://www.example.com"],
        title="Website",
    )
    vat: Optional[str] = Field(
        None,
        description="VAT number of the supplier.",
        examples=["BE0000000000"],
        title="Vat",
    )
    company_number: Optional[str] = Field(
        None,
        description="National identification number of the company.",
        examples=["0000000000"],
        title="Company Number",
    )
    iban: Optional[str] = Field(
        None,
        description="IBAN account number of the supplier. DEPRECATED see bank_accounts",
        title="Iban",
    )
    bank_account: Optional[str] = Field(
        None,
        description="Bank account number of the supplier. DEPRECATED see bank_accounts",
        title="Bank Account",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the supplier (e.g., EUR).",
        examples=["EUR"],
        title="Currency",
    )
    active: Optional[bool] = Field(
        True, description="Indicates if the supplier is active.", title="Active"
    )
    account_number: Optional[str] = Field(
        None,
        description="Ledger account number assigned to the supplier in the accounting system as it will appear in the official accounting export file (FEC, SIE, iXBRL, etc.).",
        examples=["<accounting-account-number>"],
        title="Account Number",
    )
    id: Optional[str] = Field(
        None,
        description="Id of the supplier in the accounting software. This is the unique identifier used to reference the supplier in the system.",
        examples=["<this-supplier-id>"],
        title="Id",
    )
    last_updated_on: Optional[DateTime] = Field(
        None,
        description="The last time the supplier has been updated.",
        examples=["2026-01-15T10:30:00Z"],
        title="Last Updated On",
    )
    addresses: Optional[List[AddressItemOut]] = Field(
        [],
        description="List of addresses associated with the supplier.",
        title="Addresses",
    )
    third_party_account: Optional[str] = Field(
        None,
        description="Third party account number/code representing the client in the accounting software.",
        title="Third Party Account",
    )
    bank_accounts: Optional[List[BankAccountsItem]] = Field(
        None,
        description="List of bank accounts information associated with the suppliers.",
        title="Bank Accounts",
    )


class SupplierItemUpdate(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    external_reference: Optional[str] = Field(
        None,
        description="External identifier used to link the supplier in the accounting system with the corresponding supplier reference in your own system.",
        examples=["SUPP-0001"],
        title="External Reference",
    )
    first_name: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the first name of the supplier.",
        examples=["John"],
        title="First Name",
    )
    last_name: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the last name of the supplier.",
        examples=["Doe"],
        title="Last Name",
    )
    name: Optional[str] = Field(
        None,
        description="Full name or legal name of the supplier (individual or company). This is the name that will be displayed in the accounting software.",
        examples=["Example Company S.A."],
        title="Name",
    )
    function: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the function of the supplier.",
        examples=["Accountant"],
        title="Function",
    )
    is_company: Optional[bool] = Field(
        True,
        description="Indicates if the supplier is an individual or a company.",
        title="Is Company",
    )
    company_id: Optional[str] = Field(
        None,
        description="Only used when the supplier is an individual (is_company=false). Indicates the id of the company linked to the supplier.",
        examples=["<accounting-company-id>"],
        title="Company Id",
    )
    phone: Optional[str] = Field(
        None,
        description="Phone number of the supplier.",
        examples=["+32 2 000 00 00"],
        title="Phone",
    )
    mobile: Optional[str] = Field(
        None,
        description="Mobile phone number of the supplier.",
        examples=["+32 4 000 00 00"],
        title="Mobile",
    )
    email: Optional[str] = Field(
        None,
        description="Email address of the supplier.",
        examples=["john.doe@example.com"],
        title="Email",
    )
    language: Optional[str] = Field(
        None,
        description="Language of the supplier. Format: ISO 639-1 codes.",
        examples=["en"],
        title="Language",
    )
    internal_notes: Optional[str] = Field(
        None,
        description="Internal notes about the supplier.",
        examples=["Preferred contact: accounts receivable team."],
        title="Internal Notes",
    )
    website: Optional[str] = Field(
        None,
        description="Website of the supplier.",
        examples=["https://www.example.com"],
        title="Website",
    )
    vat: Optional[str] = Field(
        None,
        description="VAT number of the supplier.",
        examples=["BE0000000000"],
        title="Vat",
    )
    company_number: Optional[str] = Field(
        None,
        description="National identification number of the company.",
        examples=["0000000000"],
        title="Company Number",
    )
    iban: Optional[str] = Field(
        None,
        description="IBAN account number of the supplier.",
        examples=["BE00000000000000"],
        title="Iban",
    )
    bank_account: Optional[str] = Field(
        None,
        description="Bank account number of the supplier.",
        examples=["0000000000"],
        title="Bank Account",
    )
    currency: Optional[str] = Field(
        None,
        description="Indicates the currency of the supplier (e.g., EUR).",
        examples=["EUR"],
        title="Currency",
    )
    active: Optional[bool] = Field(
        True, description="Indicates if the supplier is active.", title="Active"
    )
    addresses: Optional[List[AddressItemOut]] = Field(
        [],
        description="List of addresses associated with the supplier.",
        title="Addresses",
    )


class WelfareFund(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    rate: float = Field(..., description="Rate", examples=[4.0], title="Rate")
    amount: float = Field(..., description="Amount", examples=[8.0], title="Amount")
    type: Optional[WelfareFundType] = Field(None, description="Type")


class AccountItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    number: str = Field(
        ...,
        description="The account number in the chart of accounts. Must be unique within the accounting folder. In some countries (e.g., France, Belgium, Spain), the number structure is constrained by local accounting rules and must follow official charts of accounts (e.g., Plan Comptable Général).",
        examples=["<this-account-number>"],
        title="Number",
    )
    name: str = Field(
        ...,
        description="Name or label of the account as it appears in the accounting system. This is typically used for internal referencing and identification of the account.",
        examples=["Consulting services"],
        title="Name",
    )
    active: Optional[bool] = Field(
        True,
        description="Flag indicating whether the account is active. If True, the account is active and can be used in transactions. If False, the account is inactive and cannot be used for transactions.",
        title="Active",
    )
    type: Optional[AccountItemType] = Field(
        None,
        description="Represents the functional type of the ledger account, used to support common accounting operations such as payments, invoicing, tax handling, and reporting. The type here is more operational and aligned with how accounts are actually used in day-to-day processes (e.g., tracking receivables, recording VAT, handling bank transactions) ",
    )
    scheme_ids: Optional[List[str]] = Field(
        None,
        description="List of scheme classification IDs linked to this ledger account. A scheme (e.g., RGS, SBR) provides a standardized classification structure for organizing accounts for reporting purposes. Multiple scheme codes can be associated with a single account.",
        title="Scheme Ids",
    )
    accounting_category: Optional[str] = Field(
        None,
        description="Category of the ledger account in the accounting system. This is the category of the account as defined in the accounting system.",
        title="Accounting Category",
    )
    tax_information: Optional[AccountTaxInformation] = Field(
        None,
        description="Tax codes configured on the ledger account and how strictly the accounting system applies them. Null means the connector does not provide this information.",
    )


class BankStatementItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    bank_statement_date: Date = Field(
        ...,
        description="Date of the bank statement",
        examples=["2026-01-15"],
        title="Bank Statement Date",
    )
    bank_account_id: constr(min_length=1) = Field(
        ...,
        description="Unique ID of the bank account in the accounting system",
        examples=["<accounting-bank-account-id>"],
        title="Bank Account Id",
    )
    currency: constr(min_length=1) = Field(
        ...,
        description="Currency of the bank statement. This must be the same as the currency of the bank account.",
        examples=["EUR"],
        title="Currency",
    )
    external_bank_statement_id: constr(min_length=1) = Field(
        ...,
        description="External bank statement ID. To be compatible with all software, we recommend to use only digits with a maximum length of 10.",
        examples=["1234567890"],
        title="External Bank Statement Id",
    )
    opening_balance: Optional[float] = Field(
        None,
        description="Opening balance of the bank account before the statement",
        examples=[1000.0],
        title="Opening Balance",
    )
    pdf: Optional[str] = Field(
        None, description="Base64 PDF attachment of the bank statement.", title="Pdf"
    )
    pdf_name: Optional[
        constr(
            pattern=r"^[a-zA-Z0-9\-_ ]{1,196}(?:.pdf)?$", min_length=1, max_length=200
        )
    ] = Field(
        None,
        description="A name for the PDF file to be created for accounting software that support it.",
        title="Pdf Name",
    )
    items: List[BankTransactionItemIn] = Field(
        ..., description="List of transaction items", min_length=1, title="Items"
    )


class BankStatementItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    bank_statement_date: Date = Field(
        ...,
        description="Date of the bank statement",
        examples=["2026-01-15"],
        title="Bank Statement Date",
    )
    bank_account_id: constr(min_length=1) = Field(
        ...,
        description="Unique ID of the bank account in the accounting system",
        examples=["<accounting-bank-account-id>"],
        title="Bank Account Id",
    )
    currency: constr(min_length=1) = Field(
        ...,
        description="Currency of the bank statement. This must be the same as the currency of the bank account.",
        examples=["EUR"],
        title="Currency",
    )
    id: Optional[str] = Field(
        None,
        description="Internal bank statement ID generated by the accounting system",
        examples=["<this-bank-statement-id>"],
        title="Id",
    )
    external_bank_statement_id: Optional[str] = Field(
        None,
        description="External bank statement ID",
        examples=["1234567890"],
        title="External Bank Statement Id",
    )
    items: List[BankTransactionItemOut] = Field(
        ..., description="List of transaction items with their IDs", title="Items"
    )


class BankingTransactionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the transaction",
        examples=["<this-transaction-id>"],
        title="Id",
    )
    bank_account_id: Optional[str] = Field(
        None,
        description="Identifier of the bank account linked to the transaction",
        examples=["<banking-account-id>"],
        title="Bank Account Id",
    )
    amount: float = Field(
        ...,
        description="Signed amount of the transaction in the account currency (inflows positive, outflows negative). Tax-inclusive: it includes any tax, while tax_amount reports the VAT portion separately.",
        examples=[1000],
        title="Amount",
    )
    tax_amount: Optional[float] = Field(
        None,
        description="VAT amount of the transaction",
        examples=[200],
        title="Tax Amount",
    )
    currency: str = Field(
        ...,
        description="Currency of the transaction",
        examples=["EUR"],
        title="Currency",
    )
    description: Optional[str] = Field(
        None,
        description="Description of the transaction",
        examples=["Card payment"],
        title="Description",
    )
    additional_information: Optional[str] = Field(
        None,
        description="Additional information of the transaction",
        examples=["Card payment"],
        title="Additional Information",
    )
    counterpart_name: Optional[str] = Field(
        None,
        description="Name of the counterpart",
        examples=["John Doe"],
        title="Counterpart Name",
    )
    counterpart_reference: Optional[str] = Field(
        None,
        description="Reference of the counterpart",
        examples=["BE00000000000000"],
        title="Counterpart Reference",
    )
    remittance_information: Optional[str] = Field(
        None,
        description="Remittance information of the transaction",
        examples=["Card payment"],
        title="Remittance Information",
    )
    creation_date: DateTime = Field(
        ...,
        description="Creation date of the transaction",
        examples=["2025-01-01T00:00:00Z"],
        title="Creation Date",
    )
    value_date: DateTime = Field(
        ...,
        description="Value date of the transaction",
        examples=["2025-01-01T00:00:00Z"],
        title="Value Date",
    )
    execution_date: DateTime = Field(
        ...,
        description="Execution date of the transaction",
        examples=["2025-01-01T00:00:00Z"],
        title="Execution Date",
    )
    internal_transaction: Optional[bool] = Field(
        False,
        description="Whether the transaction is internal, i.e. a transfer between two of the consumer's own bank accounts.",
        examples=[False],
        title="Internal Transaction",
    )
    operation_type: Optional[OperationType] = Field(
        "other", description="Type of the operation", examples=["incoming_transfer"]
    )
    last_update_on: Optional[DateTime] = Field(
        None,
        description="Last update date of the transaction",
        examples=["2025-01-01T00:00:00Z"],
        title="Last Update On",
    )
    status: Optional[BankingTransactionStatus] = Field(
        None, description="Status of the transaction", examples=["pending"]
    )
    opening_balance: Optional[float] = Field(
        None,
        description="Opening balance of the account at the time of the transaction",
        examples=[1000.0],
        title="Opening Balance",
    )
    attachments_info: Optional[ItemAttachmentInfoOut] = Field(
        None,
        description="Extra information about the attachments linked to the invoice.",
    )


class ChiftPageAccountItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[AccountItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageBankingTransactionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[BankingTransactionItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageClientItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[ClientItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageCommerceCustomerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[CommerceCustomerItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageCommerceLocationItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[CommerceLocationItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageContactItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[ContactItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageInvoicingBankAccountItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[InvoicingBankAccountItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageInvoicingPaymentItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[InvoicingPaymentItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageInvoicingPaymentMethodItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[InvoicingPaymentMethodItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageInvoicingVatCode(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[InvoicingVatCode] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageJournalEntryMonoAnalyticPlan(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[JournalEntryMonoAnalyticPlan] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageJournal(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[Journal] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageMiscellaneousOperationOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[MiscellaneousOperationOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageOpportunityItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[OpportunityItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageOutstandingItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[OutstandingItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePMSAccountingCategoryItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PMSAccountingCategoryItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePMSCustomerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PMSCustomerItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePMSPaymentItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PMSPaymentItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePMSPaymentMethods(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PMSPaymentMethods] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePMSTaxRateItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PMSTaxRateItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePOSPaymentItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[POSPaymentItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePOSProductItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[POSProductItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePaymentItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PaymentItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePaymentMethodItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PaymentMethodItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePayoutItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PayoutItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageProductItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[ProductItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageSupplierItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[SupplierItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ClosureItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    date: Date = Field(
        ..., description="Date of the closure", examples=["2025-01-01"], title="Date"
    )
    status: ClosureStates = Field(
        ..., description="Status of the closure", examples=["closed"]
    )
    closures: Optional[List[POSClosureInformationItem]] = Field(
        [], description="Info about closures related to specific day", title="Closures"
    )


class CommerceProductItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    name: str = Field(..., examples=["T-Shirt"], title="Name")
    description: Optional[str] = Field(
        None,
        examples=["Classic cotton T-Shirt, available in several sizes."],
        title="Description",
    )
    description_html: Optional[str] = Field(
        None,
        examples=["<p>Classic cotton T-Shirt, available in several sizes.</p>"],
        title="Description Html",
    )
    categories: Optional[List[CommerceProductCategoryItem]] = Field(
        [], title="Categories"
    )
    created_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Created On"
    )
    last_updated_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Last Updated On"
    )
    variants: Optional[List[ProductVariantItem]] = Field([], title="Variants")
    status: Optional[ProductStatus] = None
    sku: Optional[str] = Field(None, examples=["TSHIRT"], title="Sku")
    common_attributes: Optional[List[CommonAttributeItem]] = Field(
        [],
        description="List of attributes that are shared by all variants of the product.",
        title="Common Attributes",
    )
    variant_attributes_options: Optional[List[VariantAttributeOptionItem]] = Field(
        [], title="Variant Attributes Options"
    )
    common_images: Optional[List[ImageItem]] = Field(
        [],
        description="List of images that are shared by all variants of the product.",
        title="Common Images",
    )


class ConnectionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    connectionid: UUID = Field(..., title="Connectionid")
    name: str = Field(..., title="Name")
    integration: str = Field(..., title="Integration")
    integrationid: int = Field(..., title="Integrationid")
    api: str = Field(..., title="Api")
    data: Optional[Dict[str, Any]] = Field(None, title="Data")
    status: ConnectionStatus
    agent: Optional[LocalAgentInfo] = Field(
        None,
        description="For local agent only. Indicates whether the local agent is up and running",
    )
    datalayer: Optional[DatalayerStatus] = Field(
        None,
        description="State of the datalayer for this connection. Null when the datalayer does not apply: the connection is inactive, its API has no datalayer, or the datalayer is not set up for that API on your account. Resolved per API, so every connection of the same API on a consumer reports the same value. `ready` when reads can be served from the datalayer (`x-chift-datalayer` header). `syncing` when the datalayer is enabled but its first sync has not finished, so there is nothing to query yet. `not_enabled` when the datalayer is available for this API but not turned on for this consumer.",
    )


class CreateFlowItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    description: Optional[str] = Field(None, title="Description")
    execution: FlowExecution
    config: Optional[FlowConfigInput] = {"datastores": []}
    triggers: List[FlowTrigger] = Field(..., title="Triggers")


class CreateSyncMappingToFieldItemInput(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    description: Optional[str] = Field(None, title="Description")
    display_order: Optional[int] = Field(0, title="Display Order")
    challenge_question: Optional[str] = Field(None, title="Challenge Question")
    conditions: Optional[Dict[str, Dict[str, Any]]] = Field(None, title="Conditions")
    source_field: FieldItem
    target_field: FieldItem
    display_delete: Optional[bool] = Field(False, title="Display Delete")
    logic: Optional[Dict[str, Any]] = Field(None, title="Logic")
    manual_mapping: Optional[bool] = Field(False, title="Manual Mapping")


class CreateSyncMappingToFieldItemOutput(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    description: Optional[str] = Field(None, title="Description")
    display_order: Optional[int] = Field(0, title="Display Order")
    challenge_question: Optional[str] = Field(None, title="Challenge Question")
    conditions: Optional[Dict[str, Dict[str, Any]]] = Field(None, title="Conditions")
    source_field: FieldItem
    target_field: FieldItem
    display_delete: Optional[bool] = Field(False, title="Display Delete")
    logic: Optional[Dict[str, Any]] = Field(None, title="Logic")
    manual_mapping: Optional[bool] = Field(False, title="Manual Mapping")


class ExecuteFlowRequest(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    consumers: Optional[List[Union[UUID, str]]] = Field(
        None,
        description="Consumer ids, or the single value 'all' for every consumer of the sync.",
        title="Consumers",
    )
    settings: Optional[FlowSettings] = Field(
        default_factory=lambda: FlowSettings.model_validate(
            {
                "write_mode": "normal",
                "import_mode": "normal",
                "verbose_logs": False,
                "extra": {},
            }
        )
    )
    preflight: Optional[bool] = Field(
        False,
        description="Check the request without starting anything. Everything that decides whether the run may start is evaluated, and a refusal comes back as a blocking warning rather than an error, so you can warn someone before they confirm.",
        title="Preflight",
    )


class ExecuteFlowResponse(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    resolved_settings: FlowSettings
    chainexecutionid: Optional[str] = Field(
        None,
        description="Unset for a preflight: nothing was started.",
        title="Chainexecutionid",
    )
    consumers: Optional[List[ExecutionConsumerResult]] = Field(None, title="Consumers")
    preview_executionid: Optional[str] = Field(None, title="Preview Executionid")
    warnings: Optional[List[ExecutionWarning]] = Field(None, title="Warnings")
    blocked: Optional[bool] = Field(
        False,
        description="Only a preflight reports this: the same request sent for real would be refused.",
        title="Blocked",
    )


class FinancialEntryItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    date: Date = Field(
        ...,
        description="Accounting date of the financial entry (format: YYYY-MM-DD). This is the date when the financial entry is recorded in the accounting system.",
        examples=["2026-01-15"],
        title="Date",
    )
    journal_id: str = Field(
        ...,
        description="Id of the journal instance in the accounting system in which the financial entry is recorded.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    currency: str = Field(
        ...,
        description="Indicates the currency of the operation (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, at the date of the operation. Must be filled in when creating the operation in another currency from the folder's default currency.",
        title="Currency Exchange Rate",
    )
    reference: Optional[str] = Field(
        None,
        description="Entry-level reference, shared across all line items of the financial entry. Typically used to store an external identifier such as a bank statement reference or payment batch reference. Leave empty when creating one financial entry per individual transaction, as the relevant identifier should then be placed in the line-level description or automated_matching_number instead.",
        examples=["REF-0001"],
        title="Reference",
    )
    number: Optional[str] = Field(
        None,
        description="Unique 'number' of the financial entry in the accounting software. This is an internal number or identifier assigned to the financial entry. It is recommended to use this number for idempotency to prevent duplicate entries. Refer to the idempotency documentation in the Developer Guides for more details.",
        examples=["FE-2026-0001"],
        title="Number",
    )
    items: List[FinancialEntryLineItem] = Field(..., min_length=1, title="Items")
    pdf: Optional[str] = Field(
        None,
        description="Base 64 string representing the PDF attached to the sale/purchase entry.",
        title="Pdf",
    )
    pdf_name: Optional[
        constr(
            pattern=r"^[a-zA-Z0-9\-_ ]{1,196}(?:.pdf)?$", min_length=1, max_length=200
        )
    ] = Field(
        None,
        description="A name for the PDF file to be created for accounting software that support it.",
        title="Pdf Name",
    )


class FinancialEntryItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    date: Date = Field(
        ...,
        description="Accounting date of the financial entry (format: YYYY-MM-DD). This is the date when the financial entry is recorded in the accounting system.",
        examples=["2026-01-15"],
        title="Date",
    )
    journal_id: str = Field(
        ...,
        description="Id of the journal instance in the accounting system in which the financial entry is recorded.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    currency: str = Field(
        ...,
        description="Indicates the currency of the operation (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, at the date of the operation. Must be filled in when creating the operation in another currency from the folder's default currency.",
        title="Currency Exchange Rate",
    )
    reference: Optional[str] = Field(
        None,
        description="Entry-level reference, shared across all line items of the financial entry. Typically used to store an external identifier such as a bank statement reference or payment batch reference. Leave empty when creating one financial entry per individual transaction, as the relevant identifier should then be placed in the line-level description or automated_matching_number instead.",
        examples=["REF-0001"],
        title="Reference",
    )
    id: str = Field(
        ...,
        description="Unique id of the financial entry instance in the accounting system.",
        examples=["<this-financial-entry-id>"],
        title="Id",
    )
    number: str = Field(
        ...,
        description="Unique 'number' of the financial entry in the accounting software. This is an internal number or identifier assigned to the financial entry. It is recommended to use this number for idempotency to prevent duplicate entries. Refer to the idempotency documentation in the Developer Guides for more details.",
        examples=["FE-2026-0001"],
        title="Number",
    )
    items: List[FinancialEntryLineItemOut] = Field(..., title="Items")


class FlowSettingsResponse(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    configurable: bool = Field(
        ...,
        description="Whether this flow declares which settings it honours. False means it has not been migrated to the settings vocabulary yet: run it through the deprecated event route instead of offering the settings form.",
        title="Configurable",
    )
    groups: List[SettingGroupItem] = Field(..., title="Groups")
    settings: List[SettingField] = Field(..., title="Settings")


class GenericJournalEntry(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    reference: Optional[str] = Field(
        None,
        description="Optional reference field used to store an external or contextual identifier related to the entry. For example, it can be used to store a reference number, the invoice number issued by the suppllier or any other relevant information.",
        examples=["JE-REF-0001"],
        title="Reference",
    )
    due_date: Optional[Date] = Field(
        None,
        description="If the journal entry relates to an invoice, this is the last due date for payment or settlement (format: YYYY-MM-DD), carried from a client/supplier entry line.",
        examples=["2026-02-14"],
        title="Due Date",
    )
    journal_id: str = Field(
        ...,
        description="Id of the journal instance in the accounting system in which the journal entry is recorded.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    number: constr(min_length=1) = Field(
        ...,
        description="Unique 'number' of the journal entry in the accounting software. This is an internal number or ID assigned to the journal entry, not necessarily the label or number visible on an invoice. It is recommended to use this number for idempotency to prevent duplicate entries. Refer to the idempotency documentation in the Developer Guides for more details.",
        examples=["JE-2026-0001"],
        title="Number",
    )
    currency: constr(min_length=1) = Field(
        ...,
        description="Indicates the currency of the journal entry (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, at the date of the journal entry. Must be filled in when creating the entry in another currency from the folder's default currency.",
        title="Currency Exchange Rate",
    )
    date: Date = Field(
        ...,
        description="Accounting date of the journal entry (format: YYYY-MM-DD). This is the date when the journal entry is recorded in the accounting system.",
        examples=["2026-01-15"],
        title="Date",
    )
    items: List[GenericJournalItem] = Field(
        ...,
        description="List of journal items that make up the journal entry. Each item represents a line in the journal entry, and the sum of debits must equal the sum of credits to ensure the entry is balanced. There must always be at least 2 lines/items.",
        min_length=2,
        title="Items",
    )
    pdf: Optional[str] = Field(
        None,
        description="Base 64 string representing the PDF attached to the item.",
        title="Pdf",
    )
    pdf_name: Optional[
        constr(
            pattern=r"^[a-zA-Z0-9\-_ ]{1,196}(?:.pdf)?$", min_length=1, max_length=200
        )
    ] = Field(
        None,
        description="A name for the PDF file to be created for accounting software that support it.",
        title="Pdf Name",
    )
    posted: Optional[bool] = Field(
        True,
        description="Indicates if the journal entry has been posted (finalized) in the accounting system. If not provided, it defaults to True.",
        title="Posted",
    )
    start_date: Optional[Date] = Field(
        None,
        description="Start date of the period covered by the journal entry (format: YYYY-MM-DD). This field should be used when the entry relates to prepaid income or expenses (accruals and deferrals), indicating when the covered period begins.",
        title="Start Date",
    )
    end_date: Optional[Date] = Field(
        None,
        description="End date of the period covered by the journal entry (format: YYYY-MM-DD). This field should be used when the entry relates to prepaid income or expenses (accruals and deferrals), indicating when the covered period ends.",
        title="End Date",
    )


class InsightsCatalog(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    entities: List[PublicCubeMetaCube] = Field(..., title="Entities")
    coveredOperations: List[str] = Field(..., title="Coveredoperations")


class IntegrationItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    integrationid: int = Field(..., title="Integrationid")
    name: str = Field(..., title="Name")
    description: Optional[str] = Field(None, title="Description")
    status: IntegationStatus
    api: Api
    logo_url: str = Field(..., title="Logo Url")
    icon_url: str = Field(..., title="Icon Url")
    post_connections: Optional[List[IntegrationsPostConnectionItem]] = Field(
        [],
        description="List of post-connections that can be activated for this integration.",
        title="Post Connections",
    )
    operations_coverage: Optional[List[CoverageItem]] = Field(
        [],
        description="List of operations coverage for this integration.",
        title="Operations Coverage",
    )
    credentials: Optional[List[IntegrationsCredentialItem]] = Field(
        [],
        description="List of credentials that must be specified to create a connection. Can be used if you want to pass credentials on connection creation. Not compatible with oAuth2 routes.",
        title="Credentials",
    )
    supported_countries: Optional[List[str]] = Field(
        None,
        description="Country codes (ISO 3166-1 alpha-2) where this integration is supported. If not defined, the integration is supported globally.",
        examples=["BE", "FR", "ES"],
        title="Supported Countries",
    )
    local_agent: Optional[bool] = Field(
        False,
        description="This field indicate if the integration is a local agent.",
        title="Local Agent",
    )


class InvoiceItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    invoice_number: Optional[str] = Field(
        ...,
        description="Number/sequence",
        examples=["INV-12345"],
        title="Invoice Number",
    )
    creation_date: Optional[DateTime] = Field(
        None,
        description="Creation date of the invoice",
        examples=["2023-10-01T12:00:00"],
        title="Creation Date",
    )
    closing_date: Optional[DateTime] = Field(
        None,
        description="Closing date of the invoice",
        examples=["2023-10-10T12:00:00"],
        title="Closing Date",
    )
    partners: Optional[List[InvoicePartnerItem]] = Field(
        None, description="List of partners related to the invoice", title="Partners"
    )


class InvoiceItemInMonoAnalyticPlan(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    invoice_type: InvoiceType = Field(..., description="Specifies the type of invoice")
    invoice_number: Optional[str] = Field(
        None,
        description="Unique 'number' of the invoice instance in the accounting software. This is an internal reference number. if not specified, will be automatically generated according to the system’s numbering rules. It does not necessarily match the number displayed on an invoice.It is recommended to use this number for idempotency to prevent duplicate entries. Refer to the idempotency documentation in the Developer Guides for more details. On the purchase side some tools store the supplier's own invoice number here, so it is not necessarily unique across suppliers.",
        title="Invoice Number",
    )
    currency: str = Field(
        ...,
        description="Code (e.g., USD, EUR) of the currency used for invoice amounts.",
        examples=["EUR"],
        title="Currency",
    )
    untaxed_amount: float = Field(
        ...,
        description="Total amount of the invoice excluding taxes.",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ...,
        description="Total amount of tax applied to the invoice",
        examples=[42.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total amount of the invoice, including taxes (untaxed_amount + tax_amount).",
        examples=[242.0],
        title="Total",
    )
    reference: Optional[str] = Field(
        None,
        description="Optional reference field used to store an external or contextual identifier related to the entry. For purchase invoices, it typically contains the invoice number issued by the supplier. For sales invoices, it may reference a quote number or any other relevant document. Unlike number, this field is not subject to specific format or character restrictions and can hold free-form text.",
        examples=["INV-2026-0001"],
        title="Reference",
    )
    payment_communication: Optional[str] = Field(
        None,
        description="Field containing the payment reference or communication associated with the invoice (e.g., bank transfer reference, SEPA remittance message). Often used for reconciliation purposes.",
        examples=["+++000/0000/00000+++"],
        title="Payment Communication",
    )
    customer_memo: Optional[str] = Field(
        None,
        description="Internal or external note associated with the invoice, typically intended for the customer. Can include additional context, comments, or special instructions related to the transaction.",
        examples=["Thank you for your business."],
        title="Customer Memo",
    )
    payment_term_id: Optional[str] = Field(
        None,
        description="Technical ID of the payment term associated with the invoice.",
        examples=["<accounting-payment-term-id>"],
        title="Payment Term Id",
    )
    withholding_tax: Optional[WithholdingTax] = Field(
        None,
        description="Withholding-tax information for the invoice. A withholding tax is paid to the government directly by the payer rather than by the recipient: the payer deducts it before paying the recipient, reducing the amount received (e.g. an employer remitting salary tax). In Chift use cases this is mainly seen in Spain.",
    )
    invoice_date: Date = Field(
        ...,
        description="Accounting date of the invoice (format: YYYY-MM-DD).",
        examples=["2026-01-15"],
        title="Invoice Date",
    )
    due_date: Date = Field(
        ...,
        description="Due date for payment or settlement (format: YYYY-MM-DD).",
        examples=["2026-02-14"],
        title="Due Date",
    )
    partner_id: Optional[constr(min_length=1)] = Field(
        None,
        description="Id of the related third party (supplier or customer) in the accounting software. Used to link the invoice to the corresponding client or supplier record in the accounting system.",
        examples=["<accounting-client-id>"],
        title="Partner Id",
    )
    journal_id: Optional[str] = Field(
        None,
        description="Identifier of the journal for the invoice. If not provided, the journal will be set automatically if only one exists; otherwise, an error will be thrown.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    status: Optional[InvoiceStatusIn] = Field(
        "posted",
        description="string indicating if invoice has been posted (finalized) in the accounting system.",
    )
    pdf: Optional[str] = Field(
        None,
        description="Base 64 string representing the PDF attached to the invoice.",
        title="Pdf",
    )
    pdf_name: Optional[
        constr(
            pattern=r"^[a-zA-Z0-9\-_ ]{1,196}(?:.pdf)?$", min_length=1, max_length=200
        )
    ] = Field(
        None,
        description="A name for the PDF file to be created for accounting software that support it.",
        title="Pdf Name",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the invoice currency to the folder's base currency, applicable at the date of the invoice. Required when the invoice currency is different from the folder's default currency.",
        title="Currency Exchange Rate",
    )
    invoice_correction: Optional[InvoiceCorrection] = Field(
        None,
        description="Information used to add a correction line when roundings have an impact on the total amount of the invoice.",
    )
    nl_payment_terms_split: Optional[NlPaymentTermsSplit] = Field(
        None,
        description="NL only - For Dutch split payment compliance (G/N-rekening): use to specify how invoice amounts are divided between a blocked (G) and normal (N) account in high-risk sectors (e.g., construction, staffing).",
    )
    shipping_country: Optional[str] = Field(
        None,
        description="Format: ISO 3166-1 codes.",
        examples=["BE"],
        title="Shipping Country",
    )
    partner_info: Optional[AccountToCreate] = Field(
        None,
        description="Information about the client/supplier that must be created. In some tools we cannot create a client/supplier by API. Missing clients/suppliers must be explicitly provided during the invoice creation and will be automatically created by the accounting software. This is for example the case for Tiime.",
    )
    lines: List[InvoiceLineItemInMonoAnalyticPlan] = Field(..., title="Lines")
    start_date: Optional[Date] = Field(
        None,
        description="Start date of the period covered by the invoice (format: YYYY-MM-DD). This field should be used when the invoice relates to prepaid income or expenses (accruals and deferrals), indicating when the covered period begins.",
        examples=["2026-01-01"],
        title="Start Date",
    )
    end_date: Optional[Date] = Field(
        None,
        description="End date of the period covered by the invoice (format: YYYY-MM-DD). This field should be used when the invoice relates to prepaid income or expenses (accruals and deferrals), indicating when the covered period ends.",
        examples=["2026-01-31"],
        title="End Date",
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical ID of the payment method in the accounting system. This is an indication of the payment method that will be used to pay the invoice. It is not necessarily the payment method that will eventually be used to pay the invoice.",
        examples=["<accounting-payment-method-id>"],
        title="Payment Method Id",
    )
    delivery_date: Optional[Date] = Field(
        None,
        description="Date on which the goods were delivered or the service was provided (format: YYYY-MM-DD). Used to assign the invoice and its tax to the correct period when it differs from the invoice date.",
        examples=["2026-01-15"],
        title="Delivery Date",
    )


class InvoiceItemInMultiAnalyticPlans(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    invoice_type: InvoiceType = Field(..., description="Specifies the type of invoice")
    invoice_number: Optional[str] = Field(
        None,
        description="Unique 'number' of the invoice instance in the accounting software. This is an internal reference number. if not specified, will be automatically generated according to the system’s numbering rules. It does not necessarily match the number displayed on an invoice.It is recommended to use this number for idempotency to prevent duplicate entries. Refer to the idempotency documentation in the Developer Guides for more details. On the purchase side some tools store the supplier's own invoice number here, so it is not necessarily unique across suppliers.",
        title="Invoice Number",
    )
    currency: str = Field(
        ...,
        description="Code (e.g., USD, EUR) of the currency used for invoice amounts.",
        examples=["EUR"],
        title="Currency",
    )
    untaxed_amount: float = Field(
        ...,
        description="Total amount of the invoice excluding taxes.",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ...,
        description="Total amount of tax applied to the invoice",
        examples=[42.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total amount of the invoice, including taxes (untaxed_amount + tax_amount).",
        examples=[242.0],
        title="Total",
    )
    reference: Optional[str] = Field(
        None,
        description="Optional reference field used to store an external or contextual identifier related to the entry. For purchase invoices, it typically contains the invoice number issued by the supplier. For sales invoices, it may reference a quote number or any other relevant document. Unlike number, this field is not subject to specific format or character restrictions and can hold free-form text.",
        examples=["INV-2026-0001"],
        title="Reference",
    )
    payment_communication: Optional[str] = Field(
        None,
        description="Field containing the payment reference or communication associated with the invoice (e.g., bank transfer reference, SEPA remittance message). Often used for reconciliation purposes.",
        examples=["+++000/0000/00000+++"],
        title="Payment Communication",
    )
    customer_memo: Optional[str] = Field(
        None,
        description="Internal or external note associated with the invoice, typically intended for the customer. Can include additional context, comments, or special instructions related to the transaction.",
        examples=["Thank you for your business."],
        title="Customer Memo",
    )
    payment_term_id: Optional[str] = Field(
        None,
        description="Technical ID of the payment term associated with the invoice.",
        examples=["<accounting-payment-term-id>"],
        title="Payment Term Id",
    )
    withholding_tax: Optional[WithholdingTax] = Field(
        None,
        description="Withholding-tax information for the invoice. A withholding tax is paid to the government directly by the payer rather than by the recipient: the payer deducts it before paying the recipient, reducing the amount received (e.g. an employer remitting salary tax). In Chift use cases this is mainly seen in Spain.",
    )
    invoice_date: Date = Field(
        ...,
        description="Accounting date of the invoice (format: YYYY-MM-DD).",
        examples=["2026-01-15"],
        title="Invoice Date",
    )
    due_date: Date = Field(
        ...,
        description="Due date for payment or settlement (format: YYYY-MM-DD).",
        examples=["2026-02-14"],
        title="Due Date",
    )
    partner_id: Optional[constr(min_length=1)] = Field(
        None,
        description="Id of the related third party (supplier or customer) in the accounting software. Used to link the invoice to the corresponding client or supplier record in the accounting system.",
        examples=["<accounting-client-id>"],
        title="Partner Id",
    )
    journal_id: Optional[str] = Field(
        None,
        description="Identifier of the journal for the invoice. If not provided, the journal will be set automatically if only one exists; otherwise, an error will be thrown.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    status: Optional[InvoiceStatusIn] = Field(
        "posted",
        description="string indicating if invoice has been posted (finalized) in the accounting system.",
    )
    pdf: Optional[str] = Field(
        None,
        description="Base 64 string representing the PDF attached to the invoice.",
        title="Pdf",
    )
    pdf_name: Optional[
        constr(
            pattern=r"^[a-zA-Z0-9\-_ ]{1,196}(?:.pdf)?$", min_length=1, max_length=200
        )
    ] = Field(
        None,
        description="A name for the PDF file to be created for accounting software that support it.",
        title="Pdf Name",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the invoice currency to the folder's base currency, applicable at the date of the invoice. Required when the invoice currency is different from the folder's default currency.",
        title="Currency Exchange Rate",
    )
    invoice_correction: Optional[InvoiceCorrection] = Field(
        None,
        description="Information used to add a correction line when roundings have an impact on the total amount of the invoice.",
    )
    nl_payment_terms_split: Optional[NlPaymentTermsSplit] = Field(
        None,
        description="NL only - For Dutch split payment compliance (G/N-rekening): use to specify how invoice amounts are divided between a blocked (G) and normal (N) account in high-risk sectors (e.g., construction, staffing).",
    )
    shipping_country: Optional[str] = Field(
        None,
        description="Format: ISO 3166-1 codes.",
        examples=["BE"],
        title="Shipping Country",
    )
    partner_info: Optional[AccountToCreate] = Field(
        None,
        description="Information about the client/supplier that must be created. In some tools we cannot create a client/supplier by API. Missing clients/suppliers must be explicitly provided during the invoice creation and will be automatically created by the accounting software. This is for example the case for Tiime.",
    )
    lines: List[InvoiceLineItemInMultiAnalyticPlans] = Field(..., title="Lines")
    start_date: Optional[Date] = Field(
        None,
        description="Start date of the period covered by the invoice (format: YYYY-MM-DD). This field should be used when the invoice relates to prepaid income or expenses (accruals and deferrals), indicating when the covered period begins.",
        examples=["2026-01-01"],
        title="Start Date",
    )
    end_date: Optional[Date] = Field(
        None,
        description="End date of the period covered by the invoice (format: YYYY-MM-DD). This field should be used when the invoice relates to prepaid income or expenses (accruals and deferrals), indicating when the covered period ends.",
        examples=["2026-01-31"],
        title="End Date",
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical ID of the payment method in the accounting system. This is an indication of the payment method that will be used to pay the invoice. It is not necessarily the payment method that will eventually be used to pay the invoice.",
        examples=["<accounting-payment-method-id>"],
        title="Payment Method Id",
    )
    delivery_date: Optional[Date] = Field(
        None,
        description="Date on which the goods were delivered or the service was provided (format: YYYY-MM-DD). Used to assign the invoice and its tax to the correct period when it differs from the invoice date.",
        examples=["2026-01-15"],
        title="Delivery Date",
    )


class InvoiceItemOutMonoAnalyticPlan(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    invoice_type: InvoiceType = Field(..., description="Specifies the type of invoice")
    invoice_number: Optional[str] = Field(
        None,
        description="Unique 'number' of the invoice instance in the accounting software. This is an internal reference number. if not specified, will be automatically generated according to the system’s numbering rules. It does not necessarily match the number displayed on an invoice.It is recommended to use this number for idempotency to prevent duplicate entries. Refer to the idempotency documentation in the Developer Guides for more details. On the purchase side some tools store the supplier's own invoice number here, so it is not necessarily unique across suppliers.",
        title="Invoice Number",
    )
    currency: str = Field(
        ...,
        description="Code (e.g., USD, EUR) of the currency used for invoice amounts.",
        examples=["EUR"],
        title="Currency",
    )
    untaxed_amount: float = Field(
        ...,
        description="Total amount of the invoice excluding taxes.",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ...,
        description="Total amount of tax applied to the invoice",
        examples=[42.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total amount of the invoice, including taxes (untaxed_amount + tax_amount).",
        examples=[242.0],
        title="Total",
    )
    reference: Optional[str] = Field(
        None,
        description="Optional reference field used to store an external or contextual identifier related to the entry. For purchase invoices, it typically contains the invoice number issued by the supplier. For sales invoices, it may reference a quote number or any other relevant document. Unlike number, this field is not subject to specific format or character restrictions and can hold free-form text.",
        examples=["INV-2026-0001"],
        title="Reference",
    )
    payment_communication: Optional[str] = Field(
        None,
        description="Field containing the payment reference or communication associated with the invoice (e.g., bank transfer reference, SEPA remittance message). Often used for reconciliation purposes.",
        examples=["+++000/0000/00000+++"],
        title="Payment Communication",
    )
    customer_memo: Optional[str] = Field(
        None,
        description="Internal or external note associated with the invoice, typically intended for the customer. Can include additional context, comments, or special instructions related to the transaction.",
        examples=["Thank you for your business."],
        title="Customer Memo",
    )
    payment_term_id: Optional[str] = Field(
        None,
        description="Technical ID of the payment term associated with the invoice.",
        examples=["<accounting-payment-term-id>"],
        title="Payment Term Id",
    )
    withholding_tax: Optional[WithholdingTax] = Field(
        None,
        description="Withholding-tax information for the invoice. A withholding tax is paid to the government directly by the payer rather than by the recipient: the payer deducts it before paying the recipient, reducing the amount received (e.g. an employer remitting salary tax). In Chift use cases this is mainly seen in Spain.",
    )
    id: Optional[str] = Field(None, examples=["<this-invoice-id>"], title="Id")
    invoice_date: Date = Field(
        ...,
        description="Document date of the invoice. It can differ from the accounting date/period that determines the book year. It is automatically set to '1970-01-01' if the value is empty in the accounting system.",
        examples=["2026-01-15"],
        title="Invoice Date",
    )
    due_date: Date = Field(
        ...,
        description="Last due date of the invoice, normally on or after the invoice date. The invoice date is used when this information is not given by the software.",
        examples=["2026-02-14"],
        title="Due Date",
    )
    delivery_date: Optional[Date] = Field(
        None,
        description="Date on which the goods were delivered or the service was provided (format: YYYY-MM-DD), when the accounting system exposes it.",
        examples=["2026-01-15"],
        title="Delivery Date",
    )
    partner_id: Optional[str] = Field(
        None, examples=["<accounting-client-id>"], title="Partner Id"
    )
    journal_id: Optional[str] = Field(
        None,
        description="Indicates the journal used in for the invoice. If the journal is not given, the journal will be automatically set if only one journal exists otherwise an error will be thrown.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    payments: Optional[List[Payment]] = Field(None, title="Payments")
    status: Optional[InvoiceStatusOut] = "posted"
    last_updated_on: Optional[DateTime] = Field(
        None,
        description="The last time the invoice has been updated.",
        examples=["2026-01-15T10:30:00Z"],
        title="Last Updated On",
    )
    partner: Optional[Partner] = Field(
        None,
        description="Extra information about the partner (client/supplier) linked to the invoice. To ensure this information is returned, you need to use the include_partner_info query parameter.",
    )
    due_dates: Optional[List[InvoiceItemDueDatesOut]] = Field(
        [], description="List of all due dates of the invoice.", title="Due Dates"
    )
    attachments_info: Optional[ItemAttachmentInfoOut] = Field(
        None,
        description="Indicates whether a file attachment (e.g. PDF or image) is linked to this invoice and how to retrieve it. Check the status field: 'yes' means a direct download URL is available in the attachments array; 'yes_to_request' means the file exists but must be fetched separately via GET /accounting/attachments?type=invoice&document_id={id}, which returns the file as a base64-encoded string. 'no' means no attachment is linked. 'unknown' means the connector does not support this.",
    )
    accounting_info: Optional[AccountingInfoOut] = Field(
        None, description="Additional information about the invoice."
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical ID of the payment method in the accounting system. This is the payment method currently linked to the invoice. It is not necessarily the payment method that will eventually be used to pay the invoice.",
        examples=["<accounting-payment-method-id>"],
        title="Payment Method Id",
    )
    approval_status: Optional[InvoiceApprovalStatus] = Field(
        "unknown", description="Approval status of the invoice."
    )
    lines: List[InvoiceLineItemOutMonoAnalyticPlan] = Field(..., title="Lines")


class InvoiceItemOutMultiAnalyticPlans(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    invoice_type: InvoiceType = Field(..., description="Specifies the type of invoice")
    invoice_number: Optional[str] = Field(
        None,
        description="Unique 'number' of the invoice instance in the accounting software. This is an internal reference number. if not specified, will be automatically generated according to the system’s numbering rules. It does not necessarily match the number displayed on an invoice.It is recommended to use this number for idempotency to prevent duplicate entries. Refer to the idempotency documentation in the Developer Guides for more details. On the purchase side some tools store the supplier's own invoice number here, so it is not necessarily unique across suppliers.",
        title="Invoice Number",
    )
    currency: str = Field(
        ...,
        description="Code (e.g., USD, EUR) of the currency used for invoice amounts.",
        examples=["EUR"],
        title="Currency",
    )
    untaxed_amount: float = Field(
        ...,
        description="Total amount of the invoice excluding taxes.",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ...,
        description="Total amount of tax applied to the invoice",
        examples=[42.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total amount of the invoice, including taxes (untaxed_amount + tax_amount).",
        examples=[242.0],
        title="Total",
    )
    reference: Optional[str] = Field(
        None,
        description="Optional reference field used to store an external or contextual identifier related to the entry. For purchase invoices, it typically contains the invoice number issued by the supplier. For sales invoices, it may reference a quote number or any other relevant document. Unlike number, this field is not subject to specific format or character restrictions and can hold free-form text.",
        examples=["INV-2026-0001"],
        title="Reference",
    )
    payment_communication: Optional[str] = Field(
        None,
        description="Field containing the payment reference or communication associated with the invoice (e.g., bank transfer reference, SEPA remittance message). Often used for reconciliation purposes.",
        examples=["+++000/0000/00000+++"],
        title="Payment Communication",
    )
    customer_memo: Optional[str] = Field(
        None,
        description="Internal or external note associated with the invoice, typically intended for the customer. Can include additional context, comments, or special instructions related to the transaction.",
        examples=["Thank you for your business."],
        title="Customer Memo",
    )
    payment_term_id: Optional[str] = Field(
        None,
        description="Technical ID of the payment term associated with the invoice.",
        examples=["<accounting-payment-term-id>"],
        title="Payment Term Id",
    )
    withholding_tax: Optional[WithholdingTax] = Field(
        None,
        description="Withholding-tax information for the invoice. A withholding tax is paid to the government directly by the payer rather than by the recipient: the payer deducts it before paying the recipient, reducing the amount received (e.g. an employer remitting salary tax). In Chift use cases this is mainly seen in Spain.",
    )
    id: Optional[str] = Field(None, examples=["<this-invoice-id>"], title="Id")
    invoice_date: Date = Field(
        ...,
        description="Document date of the invoice. It can differ from the accounting date/period that determines the book year. It is automatically set to '1970-01-01' if the value is empty in the accounting system.",
        examples=["2026-01-15"],
        title="Invoice Date",
    )
    due_date: Date = Field(
        ...,
        description="Last due date of the invoice, normally on or after the invoice date. The invoice date is used when this information is not given by the software.",
        examples=["2026-02-14"],
        title="Due Date",
    )
    delivery_date: Optional[Date] = Field(
        None,
        description="Date on which the goods were delivered or the service was provided (format: YYYY-MM-DD), when the accounting system exposes it.",
        examples=["2026-01-15"],
        title="Delivery Date",
    )
    partner_id: Optional[str] = Field(
        None, examples=["<accounting-client-id>"], title="Partner Id"
    )
    journal_id: Optional[str] = Field(
        None,
        description="Indicates the journal used in for the invoice. If the journal is not given, the journal will be automatically set if only one journal exists otherwise an error will be thrown.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    payments: Optional[List[Payment]] = Field(None, title="Payments")
    status: Optional[InvoiceStatusOut] = "posted"
    last_updated_on: Optional[DateTime] = Field(
        None,
        description="The last time the invoice has been updated.",
        examples=["2026-01-15T10:30:00Z"],
        title="Last Updated On",
    )
    partner: Optional[Partner] = Field(
        None,
        description="Extra information about the partner (client/supplier) linked to the invoice. To ensure this information is returned, you need to use the include_partner_info query parameter.",
    )
    due_dates: Optional[List[InvoiceItemDueDatesOut]] = Field(
        [], description="List of all due dates of the invoice.", title="Due Dates"
    )
    attachments_info: Optional[ItemAttachmentInfoOut] = Field(
        None,
        description="Indicates whether a file attachment (e.g. PDF or image) is linked to this invoice and how to retrieve it. Check the status field: 'yes' means a direct download URL is available in the attachments array; 'yes_to_request' means the file exists but must be fetched separately via GET /accounting/attachments?type=invoice&document_id={id}, which returns the file as a base64-encoded string. 'no' means no attachment is linked. 'unknown' means the connector does not support this.",
    )
    accounting_info: Optional[AccountingInfoOut] = Field(
        None, description="Additional information about the invoice."
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical ID of the payment method in the accounting system. This is the payment method currently linked to the invoice. It is not necessarily the payment method that will eventually be used to pay the invoice.",
        examples=["<accounting-payment-method-id>"],
        title="Payment Method Id",
    )
    approval_status: Optional[InvoiceApprovalStatus] = Field(
        "unknown", description="Approval status of the invoice."
    )
    lines: List[InvoiceLineItemOutMultiAnalyticPlans] = Field(..., title="Lines")


class InvoicingBankTransactionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    status: BankTransactionStatus = Field(
        ..., description="Status of the bank transaction"
    )
    bank_account_id: str = Field(
        ...,
        description="Technical id of the bank account in Chift",
        examples=["<invoicing-bank-account-id>"],
        title="Bank Account Id",
    )
    amount: float = Field(
        ..., description="Transaction amount", examples=[242.0], title="Amount"
    )
    date: Date = Field(
        ..., description="Transaction date", examples=["2026-01-15"], title="Date"
    )
    description: Optional[str] = Field(
        None,
        description="Transaction description",
        examples=["Bank charges"],
        title="Description",
    )
    currency: str = Field(
        ...,
        description="Currency of the transaction",
        examples=["EUR"],
        title="Currency",
    )
    currency_exchange_rate: Optional[float] = Field(
        1.0,
        description="Exchange rate from the transaction currency to the connector's base currency, at the date of the transaction.",
        title="Currency Exchange Rate",
    )
    accounting_code: Optional[str] = Field(
        None,
        description="Accounting code",
        examples=["<invoicing-account-number>"],
        title="Accounting Code",
    )
    linked_documents: Optional[List[LinkedDocument]] = Field(
        [], description="List of linked documents", title="Linked Documents"
    )


class ItalianSpecificitiesInput(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    stamp_duty_amount: Optional[float] = Field(
        None,
        description="Documentary stamp tax (specific to Italy)",
        examples=[2.0],
        title="Stamp Duty Amount",
    )
    withholding_tax: Optional[InvoicingWithholdingTax] = Field(
        None, description="Withholding tax (specific to Italy)"
    )
    welfare_fund: Optional[WelfareFund] = Field(
        None, description="Welfare fund (specific to Italy)"
    )
    payment_reporting: Optional[PaymentReporting] = Field(
        None, description="Payment reporting (specific to Italy)"
    )


class ItalianSpecificitiesOutput(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    stamp_duty_amount: Optional[float] = Field(
        None,
        description="Documentary stamp tax (specific to Italy)",
        examples=[2.0],
        title="Stamp Duty Amount",
    )
    withholding_tax: Optional[InvoicingWithholdingTax] = Field(
        None, description="Withholding tax (specific to Italy)"
    )
    welfare_fund: Optional[WelfareFund] = Field(
        None, description="Welfare fund (specific to Italy)"
    )
    payment_reporting: Optional[PaymentReporting] = Field(
        None, description="Payment reporting (specific to Italy)"
    )


class JournalEntryMultiAnalyticPlan(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    reference: Optional[str] = Field(
        None,
        description="Optional reference field used to store an external or contextual identifier related to the entry. For example, it can be used to store a reference number, the invoice number issued by the suppllier or any other relevant information.",
        examples=["JE-REF-0001"],
        title="Reference",
    )
    due_date: Optional[Date] = Field(
        None,
        description="If the journal entry relates to an invoice, this is the last due date for payment or settlement (format: YYYY-MM-DD), carried from a client/supplier entry line.",
        examples=["2026-02-14"],
        title="Due Date",
    )
    journal_id: str = Field(
        ...,
        description="Id of the journal instance in the accounting system in which the journal entry is recorded.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    name: Optional[str] = Field(
        None,
        description="Mainly the accounting entry number (numéro de pièce comptable) assigned by the source system.",
        examples=["JE-2026-0001"],
        title="Name",
    )
    journal_name: Optional[str] = Field(
        None,
        description="Name of the journal in which the journal entry is recorded.",
        examples=["Bank journal"],
        title="Journal Name",
    )
    date: Optional[Date] = Field(
        "1970-01-01",
        description="Accounting date of the journal entry, which determines the accounting period and therefore the book year the entry falls into. It is automatically set to '1970-01-01' if the value is not available/empty in the accounting system.",
        examples=["2026-01-15"],
        title="Date",
    )
    posted: Optional[bool] = Field(
        False,
        description="Indicates if the journal entry has been posted (finalized) in the accounting system.",
        title="Posted",
    )
    id: str = Field(
        ...,
        description="Unique id of the journal entry instance in the accounting system.",
        examples=["<this-journal-entry-id>"],
        title="Id",
    )
    due_dates: Optional[List[JournalItemDueDatesOut]] = Field(
        [],
        description="List of all due dates of a journal entry. Generally an entry has one client/supplier line per due date, each listed here with its amount.",
        title="Due Dates",
    )
    attachments_info: Optional[ItemAttachmentInfoOut] = Field(
        None,
        description="Indicates whether a file attachment (e.g. PDF or image) is linked to this journal entry and how to retrieve it. Check the status field: 'yes' means a direct download URL is available in the attachments array; 'yes_to_request' means the file exists but must be fetched separately via GET /accounting/attachments?type=entry&document_id={id}, which returns the file as a base64-encoded string. 'no' means no attachment is linked. 'unknown' means the connector does not support this.",
    )
    items: Optional[List[JournalItemMultiAnalyticPlan]] = Field(
        [],
        description="List of journal items that make up the journal entry. Each item represents a line in the journal entry, and the sum of debits must equal the sum of credits to ensure the entry is balanced.",
        title="Items",
    )


class MiscellaneousOperationIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    operation_number: Optional[str] = Field(
        None, examples=["OP-2026-0001"], title="Operation Number"
    )
    currency: str = Field(
        ...,
        description="Indicates the currency of the operation (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the folder's base currency, at the date of the operation. Must be filled in when creating the operation in another currency from the folder's default currency.",
        title="Currency Exchange Rate",
    )
    lines: List[MiscellaneousOperationLine] = Field(..., title="Lines")
    operation_date: Date = Field(..., examples=["2026-01-15"], title="Operation Date")
    journal_id: Optional[str] = Field(
        None,
        description="Indicates the journal used in for the operation. If the journal is not given, the journal will be automatically set if only one journal exists otherwise an error will be thrown.",
        examples=["<accounting-journal-id>"],
        title="Journal Id",
    )
    status: Optional[MiscellaneousOperationStatusIn] = "posted"


class OrderItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    order_number: Optional[str] = Field(None, examples=["1001"], title="Order Number")
    channel: Optional[str] = Field(None, examples=["Online store"], title="Channel")
    customer: Optional[OrderCustomerItemOut] = None
    billing_address: Optional[CommerceAddressItemOut] = None
    shipping_address: Optional[CommerceAddressItemOut] = None
    created_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Created On"
    )
    last_updated_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Last Updated On"
    )
    confirmed_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Confirmed On"
    )
    delivery_date: Optional[DateTime] = Field(
        None, examples=["2026-02-14T10:30:00Z"], title="Delivery Date"
    )
    cancelled_on: Optional[DateTime] = Field(
        None, examples=["2026-01-20T10:30:00Z"], title="Cancelled On"
    )
    status: OrderStatus
    fulfillment: Optional[EcommerceFulfillmentObject] = None
    discount_amount: float = Field(..., examples=[0.0], title="Discount Amount")
    untaxed_amount_without_fees: float = Field(
        ...,
        description="Untaxed amount without fees (e.g. shipping) but with discout. Refunds and returns are not included.",
        examples=[200.0],
        title="Untaxed Amount Without Fees",
    )
    tax_amount_without_fees: float = Field(
        ...,
        description="Total taxes applied to the order without taxes linked to fees (e.g. shipping).",
        examples=[42.0],
        title="Tax Amount Without Fees",
    )
    total_without_fees: float = Field(
        ...,
        description="Total of the order without fees (e.g. shipping) but with discount. Refunds and returns are not included.",
        examples=[242.0],
        title="Total Without Fees",
    )
    current_untaxed_amount: float = Field(
        ...,
        description="Untaxed amount with discout and any kind of fee (e.g. shipping). After returns, removes and returns.",
        examples=[200.0],
        title="Current Untaxed Amount",
    )
    current_tax_amount: float = Field(
        ...,
        description="Total taxes applied to the order. After returns, removes and returns.",
        examples=[42.0],
        title="Current Tax Amount",
    )
    current_total: float = Field(
        ...,
        description="Current total of the order with discount. After returns, removes and returns.",
        examples=[242.0],
        title="Current Total",
    )
    untaxed_amount: float = Field(
        ...,
        description="Untaxed amount with discout and any kind of fee (e.g. shipping). Refunds and returns are not included.",
        examples=[200.0],
        title="Untaxed Amount",
    )
    tax_amount: float = Field(
        ...,
        description="Total taxes applied to the order. Refunds and returns are not included.",
        examples=[42.0],
        title="Tax Amount",
    )
    total: float = Field(
        ...,
        description="Total of the order with discount. Refunds and returns are not included.",
        examples=[242.0],
        title="Total",
    )
    refunded_amount: Optional[float] = Field(0, examples=[0.0], title="Refunded Amount")
    currency: str = Field(
        ...,
        description="Indicates the currency of the order (e.g. EUR).",
        examples=["EUR"],
        title="Currency",
    )
    note: Optional[str] = Field(
        None, examples=["Please leave at the front door."], title="Note"
    )
    tags: Optional[List[str]] = Field([], examples=[["vip"]], title="Tags")
    lines: List[OrderLineItemOut] = Field(..., title="Lines")
    other_fees: Optional[List[FeesItem]] = Field([], title="Other Fees")
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical id of the payment method in the eCommerce",
        examples=["<commerce-payment-method-id>"],
        title="Payment Method Id",
    )
    transactions: Optional[List[OrderTransactions]] = Field([], title="Transactions")
    payment_methods: Optional[List[OrderPaymentMethods]] = Field(
        [], title="Payment Methods"
    )
    detailed_refunds: Optional[List[OrderRefundItem]] = Field(
        None, title="Detailed Refunds"
    )
    returns: Optional[List[OrderReturnItem]] = Field([], title="Returns")


class PMSAccountingTransactionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    group_id: str = Field(
        ...,
        description="Group id of the transaction",
        examples=["<this-transaction-group-id>"],
        title="Group Id",
    )
    date: DateTime = Field(
        ...,
        description="Datetime of the transaction.This is the date when the transaction is recorded in the pms system.",
        examples=["2026-01-15T10:30:00Z"],
        title="Date",
    )
    amount: float = Field(
        ..., description="Total amount", examples=[121.0], title="Amount"
    )
    is_tax_line: bool = Field(
        ...,
        description="Indicates the transaction is tax line",
        examples=[False],
        title="Is Tax Line",
    )
    sub_group_id: Optional[str] = Field(
        None,
        description="Sub group id of the transaction",
        examples=["<this-transaction-sub-group-id>"],
        title="Sub Group Id",
    )
    tax_code: Optional[str] = Field(
        None,
        description="Indicates the tax code used for the transaction. This is the Id of the Tax Code in the pms software.",
        examples=["<pms-tax-code>"],
        title="Tax Code",
    )
    description: Optional[str] = Field(
        "",
        description="Text description for this transaction.",
        examples=["Room charge"],
        title="Description",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the transaction currency to the connector's base currency, applicable at the transaction date.",
        title="Currency Exchange Rate",
    )
    currency: str = Field(
        ...,
        description="Indicates the currency of the operation (e.g., EUR, USD).",
        examples=["EUR"],
        title="Currency",
    )
    transaction_type: PMSAccountingTransactionType = Field(
        ..., description="Type of transaction", examples=["pos"]
    )
    accounting_category_id: Optional[str] = Field(
        None,
        description="Used by a PMS to give specific accounting categories to a transaction item.",
        examples=["<pms-accounting-category-id>"],
        title="Accounting Category Id",
    )
    ledger_account_code: Optional[str] = Field(
        None,
        description="Ledger account code assigned to the category",
        examples=["<pms-ledger-account-code>"],
        title="Ledger Account Code",
    )
    status: PMSAccountingTransactionStatus = Field(
        ...,
        description="string indicating if invoice has been posted (finalized) in the pms system.",
        examples=["posted"],
    )
    origin: Optional[PMSAccountingTransactionOrigin] = Field(
        ...,
        description="Type of origin of the transaction item",
        examples=[{"id": "<pms-origin-id>", "type": "payment"}],
    )
    partner_id: Optional[ChiftId] = Field(
        None, description="Reference to the partner related to this transaction"
    )
    reference: Optional[str] = Field(
        None,
        description="Optional reference field used to store an external or contextual identifier related to the transaction.",
        examples=["REF-0001"],
        title="Reference",
    )
    location_id: Optional[str] = Field(
        None,
        description="ID of the location this transaction belongs to",
        examples=["<pms-location-id>"],
        title="Location Id",
    )


class PMSClosureItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    date: Date = Field(
        ..., description="Date of the closure", examples=["2025-01-01"], title="Date"
    )
    status: ClosureStates = Field(
        ..., description="Status of the closure", examples=["closed"]
    )
    closures: Optional[List[POSClosureInformationItem]] = Field(
        [], description="Info about closures related to specific day", title="Closures"
    )


class PMSInvoiceFullItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    invoice_number: Optional[str] = Field(
        ...,
        description="Number/sequence",
        examples=["INV-12345"],
        title="Invoice Number",
    )
    creation_date: Optional[DateTime] = Field(
        None,
        description="Creation date of the invoice",
        examples=["2023-10-01T12:00:00"],
        title="Creation Date",
    )
    closing_date: Optional[DateTime] = Field(
        None,
        description="Closing date of the invoice",
        examples=["2023-10-10T12:00:00"],
        title="Closing Date",
    )
    partners: Optional[List[InvoicePartnerItem]] = Field(
        None, description="List of partners related to the invoice", title="Partners"
    )
    items: List[PMSOrderLineItem] = Field(
        ..., description="List of items related to the invoice", title="Items"
    )
    payments: List[PMSPaymentItem] = Field(
        ..., description="List of payments related to the invoice", title="Payments"
    )
    service_id: Optional[str] = Field(
        None,
        description="Reference to the service related to this order",
        title="Service Id",
    )
    reservation: Optional[ReservationItem] = Field(
        None, description="Reference to the reservation related to this order"
    )
    tax_amount: float = Field(
        ..., description="Taxes amount", examples=[21.0], title="Tax Amount"
    )
    untaxed_amount: float = Field(
        ..., description="Untaxed amount", examples=[100.0], title="Untaxed Amount"
    )
    total: float = Field(
        ..., description="Total amount incl. taxes", examples=[121.0], title="Total"
    )


class PMSOrderItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the order",
        examples=["<this-order-id>"],
        title="Id",
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    order_number: Optional[str] = Field(
        None,
        description="Order number (often unique reference during the day)",
        examples=["1"],
        title="Order Number",
    )
    creation_date: DateTime = Field(
        ...,
        description="Creation date of the order",
        examples=["2025-01-01T00:00:00Z"],
        title="Creation Date",
    )
    closing_date: Optional[DateTime] = Field(
        None,
        description="Closing date of the order",
        examples=["2025-01-01T00:00:00Z"],
        title="Closing Date",
    )
    service_date: Optional[DateTime] = Field(
        None,
        description="Indicates the date of the service to which the order belongs (can be used to group orders by closure date)",
        examples=["2025-01-01T00:00:00Z"],
        title="Service Date",
    )
    device_id: Optional[str] = Field(
        None,
        description="ID of device that created the order",
        examples=["<pos-device-id>"],
        title="Device Id",
    )
    total: float = Field(
        ...,
        description="Total amount including tax of the order",
        examples=[11],
        title="Total",
    )
    tax_amount: float = Field(
        ...,
        description="Total amount of the taxes of the order",
        examples=[1],
        title="Tax Amount",
    )
    total_discount: Optional[float] = Field(
        "0",
        description="Total amount of the discounts of the order",
        examples=[10],
        title="Total Discount",
    )
    total_refund: Optional[float] = Field(
        "0",
        description="Total amount of the refunds of the order",
        examples=[5],
        title="Total Refund",
    )
    total_tip: Optional[float] = Field(
        "0",
        description="Total amount of the tips of the order. Tips are not part of the total of the order",
        examples=[1],
        title="Total Tip",
    )
    delivery_fee: Optional[float] = Field(
        "0",
        description="Total amount of the delivery fees of the order",
        examples=[1],
        title="Delivery Fee",
    )
    mode: Optional[Mode] = Field(
        "UNKNOWN", description="Delivery Mode", examples=["eat_in"]
    )
    source: Optional[Source] = Field(
        "UNKNOWN", description="Source of the order", examples=["web"]
    )
    currency: Optional[str] = Field(
        None, description="Currency of the order", examples=["EUR"], title="Currency"
    )
    country: Optional[str] = Field(
        None,
        description="Country of the order, format: ISO 3166-1 codes.",
        examples=["FR"],
        title="Country",
    )
    loyalty: Optional[int] = Field(
        None, description="Loyalty points of the order", examples=[100], title="Loyalty"
    )
    customer_id: Optional[str] = Field(
        None,
        description="Unique identifier of the customer",
        examples=["<pos-customer-id>"],
        title="Customer Id",
    )
    location_id: Optional[str] = Field(
        None,
        description="Unique identifier of the location",
        examples=["<pos-location-id>"],
        title="Location Id",
    )
    taxes: Optional[List[TotalTaxItem]] = Field(
        [], description="List of taxes of the order", title="Taxes"
    )
    guests: Optional[int] = Field(
        None,
        description="Number of guests linked to the order",
        examples=[1],
        title="Guests",
    )
    items: List[PMSOrderLineItem] = Field(
        ..., description="List of items related to the order", title="Items"
    )
    service_id: Optional[str] = Field(
        None,
        description="Reference to the service related to this order",
        title="Service Id",
    )
    reservation: Optional[ReservationItem] = Field(
        None, description="Reference to the reservation related to this order"
    )
    bills: Optional[List[InvoiceItem]] = Field(
        None, description="Reference to the bills related to this order", title="Bills"
    )


class POSOrderItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ...,
        description="Unique identifier of the order",
        examples=["<this-order-id>"],
        title="Id",
    )
    order_number: Optional[str] = Field(
        None,
        description="Order number (often unique reference during the day)",
        examples=["1"],
        title="Order Number",
    )
    creation_date: DateTime = Field(
        ...,
        description="Creation date of the order",
        examples=["2025-01-01T00:00:00Z"],
        title="Creation Date",
    )
    closing_date: Optional[DateTime] = Field(
        None,
        description="Closing date of the order",
        examples=["2025-01-01T00:00:00Z"],
        title="Closing Date",
    )
    service_date: Optional[DateTime] = Field(
        None,
        description="Indicates the date of the service to which the order belongs (can be used to group orders by closure date)",
        examples=["2025-01-01T00:00:00Z"],
        title="Service Date",
    )
    device_id: Optional[str] = Field(
        None,
        description="ID of device that created the order",
        examples=["<pos-device-id>"],
        title="Device Id",
    )
    total: float = Field(
        ...,
        description="Total amount including tax of the order",
        examples=[11],
        title="Total",
    )
    tax_amount: float = Field(
        ...,
        description="Total amount of the taxes of the order",
        examples=[1],
        title="Tax Amount",
    )
    total_discount: Optional[float] = Field(
        "0",
        description="Total amount of the discounts of the order",
        examples=[10],
        title="Total Discount",
    )
    total_refund: Optional[float] = Field(
        "0",
        description="Total amount of the refunds of the order",
        examples=[5],
        title="Total Refund",
    )
    total_tip: Optional[float] = Field(
        "0",
        description="Total amount of the tips of the order. Tips are not part of the total of the order",
        examples=[1],
        title="Total Tip",
    )
    delivery_fee: Optional[float] = Field(
        "0",
        description="Total amount of the delivery fees of the order",
        examples=[1],
        title="Delivery Fee",
    )
    mode: Optional[Mode] = Field(
        "UNKNOWN", description="Delivery Mode", examples=["eat_in"]
    )
    source: Optional[Source] = Field(
        "UNKNOWN", description="Source of the order", examples=["web"]
    )
    currency: Optional[str] = Field(
        None, description="Currency of the order", examples=["EUR"], title="Currency"
    )
    country: Optional[str] = Field(
        None,
        description="Country of the order, format: ISO 3166-1 codes.",
        examples=["FR"],
        title="Country",
    )
    loyalty: Optional[int] = Field(
        None, description="Loyalty points of the order", examples=[100], title="Loyalty"
    )
    customer_id: Optional[str] = Field(
        None,
        description="Unique identifier of the customer",
        examples=["<pos-customer-id>"],
        title="Customer Id",
    )
    location_id: Optional[str] = Field(
        None,
        description="Unique identifier of the location",
        examples=["<pos-location-id>"],
        title="Location Id",
    )
    taxes: Optional[List[TotalTaxItem]] = Field(
        [], description="List of taxes of the order", title="Taxes"
    )
    guests: Optional[int] = Field(
        None,
        description="Number of guests linked to the order",
        examples=[1],
        title="Guests",
    )
    payments: List[POSPaymentItem] = Field(
        ..., description="List of payments made for the order", title="Payments"
    )
    items: List[POSItem] = Field(
        ..., description="List of item lines in the order", title="Items"
    )
    invoice_id: Optional[str] = Field(
        None,
        description="ID of the invoice linked to the order. Only available for specific POS and if the order is linked to an invoice (B2B)",
        examples=["<pos-invoice-id>"],
        title="Invoice Id",
    )


class ReadFlowConsumerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    description: Optional[str] = Field(None, title="Description")
    id: str = Field(..., title="Id")
    config: Optional[FlowConfigOutput] = None
    values: Dict[str, Any] = Field(..., title="Values")
    enabled_on: Optional[DateTime] = Field(
        None,
        description="Date on which the flow was enabled for this consumer",
        title="Enabled On",
    )
    trigger: ReadFlowTrigger


class ReportItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    date: Date = Field(
        ..., description="Date of the report", examples=["2025-01-01"], title="Date"
    )
    location_id: str = Field(
        ...,
        description="Unique identifier of the location",
        examples=["<pos-location-id>"],
        title="Location Id",
    )
    orders: Optional[List[ReportOrders]] = Field(
        [], description="List of orders related to the report", title="Orders"
    )
    payments: Optional[List[ReportPayments]] = Field(
        [], description="List of payments related to the report", title="Payments"
    )


class SyncConsumerItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    syncid: UUID = Field(..., title="Syncid")
    sync_name: str = Field(..., description="Name of the sync", title="Sync Name")
    status: Optional[SyncConsumerStatus] = Field(
        "inactive",
        description="Indicates whether the consumer has setup the sync. If the setup is complete, it will return 'active'. Otherwise 'inactive'.",
    )
    status_details: Optional[str] = Field(
        None,
        description="Gives additional information if the status is inactive",
        title="Status Details",
    )
    link_createdon: DateTime = Field(
        ...,
        description="Date on which the consumer first navigated to the link to setup the sync",
        title="Link Createdon",
    )
    link_mappings: Optional[List[ReadSyncMappingItem]] = Field(
        None,
        description="Values of the mappings requested for the sync for the specific consumer",
        title="Link Mappings",
    )
    skip_mappings: Optional[List[ReadSyncSkipMappingItem]] = Field(
        None,
        description="List of mappings which are ignored due to specific configuration of the consumer",
        title="Skip Mappings",
    )
    link_metadata: Optional[Dict[str, Any]] = Field(
        None, description="Metadata passed during creation", title="Link Metadata"
    )
    link_presync: Optional[Dict[str, Any]] = Field(
        None,
        description="Values of the presync configuration for the sync for the specific consumer",
        title="Link Presync",
    )
    enabled_flows: Optional[List[ReadFlowConsumerItem]] = Field(
        None,
        description="List of flows that the consumer has enabled",
        title="Enabled Flows",
    )


class UploadDocumentItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    currency: Optional[str] = Field(
        None,
        description="Currency of the uploaded document",
        examples=["EUR"],
        title="Currency",
    )
    invoice_type: Optional[InvoicingInvoiceType] = Field(
        None, description="Invoice type of the uploaded document"
    )
    status: Optional[InvoiceStatus] = Field(
        None, description="Status of the uploaded document"
    )
    invoice_date: Optional[Date] = Field(
        None,
        description="Invoicing date",
        examples=["2026-01-15"],
        title="Invoice Date",
    )
    tax_amount: Optional[float] = Field(
        None, description="Taxes amount", examples=[42.0], title="Tax Amount"
    )
    untaxed_amount: Optional[float] = Field(
        None, description="Untaxed amount", examples=[200.0], title="Untaxed Amount"
    )
    total: Optional[float] = Field(
        None, description="Total amount incl. taxes", examples=[242.0], title="Total"
    )
    lines: Optional[List[InvoiceLineItem]] = Field(
        [], description="Invoice lines", title="Lines"
    )
    partner_id: Optional[str] = Field(
        None,
        description="Technical id of the vendor/customer in Chift",
        examples=["<invoicing-partner-id>"],
        title="Partner Id",
    )
    invoice_number: Optional[str] = Field(
        None,
        description="Number/sequence",
        examples=["INV-2026-0001"],
        title="Invoice Number",
    )
    due_date: Optional[Date] = Field(
        None, description="Due date", examples=["2026-02-14"], title="Due Date"
    )
    reference: Optional[str] = Field(
        None, description="Reference", examples=["REF-0001"], title="Reference"
    )
    payment_communication: Optional[str] = Field(
        None,
        description="Payment communication",
        examples=["+++000/0000/00000+++"],
        title="Payment Communication",
    )
    customer_memo: Optional[str] = Field(
        None,
        description="Customer note/memo",
        examples=["Thank you for your business."],
        title="Customer Memo",
    )
    journal_ref: Optional[FieldRef] = Field(None, description="Journal")
    italian_specificities: Optional[ItalianSpecificitiesOutput] = Field(
        None, description="Specificities for Italy"
    )
    spanish_specificities: Optional[SpanishSpecificities] = Field(
        None, description="Specificities for Spain"
    )
    last_updated_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Last Updated On"
    )
    outstanding_amount: Optional[float] = Field(
        None,
        description="Amount left to be paid",
        examples=[0.0],
        title="Outstanding Amount",
    )
    last_payment_date: Optional[Date] = Field(
        None,
        description="Date of the last payment linked to the invoice",
        examples=["2026-02-14"],
        title="Last Payment Date",
    )
    accounting_date: Optional[Date] = Field(
        None,
        description="Accounting date",
        examples=["2026-01-15"],
        title="Accounting Date",
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical id of the payment method in Chift",
        examples=["<invoicing-payment-method-id>"],
        title="Payment Method Id",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the invoice currency to the connector's base currency, at the date of the invoice.",
        title="Currency Exchange Rate",
    )


class ChiftPageCommerceProductItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[CommerceProductItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageInvoiceItemOutMonoAnalyticPlan(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[InvoiceItemOutMonoAnalyticPlan] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageInvoiceItemOutMultiAnalyticPlans(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[InvoiceItemOutMultiAnalyticPlans] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageInvoicingBankTransactionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[InvoicingBankTransactionItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageJournalEntryMultiAnalyticPlan(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[JournalEntryMultiAnalyticPlan] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPageOrderItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[OrderItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePMSAccountingTransactionItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PMSAccountingTransactionItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePMSInvoiceFullItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PMSInvoiceFullItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePMSOrderItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[PMSOrderItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class ChiftPagePOSOrderItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[POSOrderItem] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class CreateSyncMappingItemInput(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    description: Optional[str] = Field(None, title="Description")
    display_order: Optional[int] = Field(0, title="Display Order")
    challenge_question: Optional[str] = Field(None, title="Challenge Question")
    conditions: Optional[Dict[str, Dict[str, Any]]] = Field(None, title="Conditions")
    sub_mappings: Optional[List[CreateSyncMappingToFieldItemInput]] = Field(
        [], title="Sub Mappings"
    )


class CreateSyncMappingItemOutput(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: str = Field(..., title="Name")
    description: Optional[str] = Field(None, title="Description")
    display_order: Optional[int] = Field(0, title="Display Order")
    challenge_question: Optional[str] = Field(None, title="Challenge Question")
    conditions: Optional[Dict[str, Dict[str, Any]]] = Field(None, title="Conditions")
    sub_mappings: Optional[List[CreateSyncMappingToFieldItemOutput]] = Field(
        [], title="Sub Mappings"
    )


class InvoiceItemIn(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    currency: str = Field(
        ...,
        description="Currency matching target sofware name",
        examples=["EUR"],
        title="Currency",
    )
    invoice_type: InvoicingCreateInvoiceType = Field(..., description="Invoice type")
    status: InvoiceStatus = Field(..., description="Status")
    invoice_date: Date = Field(
        ..., description="Invoicing date", examples=["2026-01-15"], title="Invoice Date"
    )
    tax_amount: float = Field(
        ..., description="Taxes amount", examples=[42.0], title="Tax Amount"
    )
    untaxed_amount: float = Field(
        ..., description="Untaxed amount", examples=[200.0], title="Untaxed Amount"
    )
    total: float = Field(
        ..., description="Total amount incl. taxes", examples=[242.0], title="Total"
    )
    lines: Optional[List[InvoiceLineItem]] = Field(
        [], description="Invoice lines", title="Lines"
    )
    partner_id: Optional[str] = Field(
        None,
        description="Technical id of the vendor/customer in Chift",
        examples=["<invoicing-partner-id>"],
        title="Partner Id",
    )
    invoice_number: Optional[str] = Field(
        None,
        description="Number/sequence",
        examples=["INV-2026-0001"],
        title="Invoice Number",
    )
    due_date: Optional[Date] = Field(
        None, description="Due date", examples=["2026-02-14"], title="Due Date"
    )
    reference: Optional[str] = Field(
        None, description="Reference", examples=["REF-0001"], title="Reference"
    )
    payment_communication: Optional[str] = Field(
        None,
        description="Payment communication",
        examples=["+++000/0000/00000+++"],
        title="Payment Communication",
    )
    customer_memo: Optional[str] = Field(
        None,
        description="Customer note/memo",
        examples=["Thank you for your business."],
        title="Customer Memo",
    )
    journal_ref: Optional[FieldRef] = Field(None, description="Journal")
    italian_specificities: Optional[ItalianSpecificitiesInput] = Field(
        None, description="Specificities for Italy"
    )
    spanish_specificities: Optional[SpanishSpecificities] = Field(
        None, description="Specificities for Spain"
    )


class InvoiceItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    currency: str = Field(
        ...,
        description="Currency matching target sofware name",
        examples=["EUR"],
        title="Currency",
    )
    invoice_type: InvoicingInvoiceType = Field(..., description="Invoice type")
    status: InvoiceStatus = Field(..., description="Status")
    invoice_date: Date = Field(
        ..., description="Invoicing date", examples=["2026-01-15"], title="Invoice Date"
    )
    tax_amount: float = Field(
        ..., description="Taxes amount", examples=[42.0], title="Tax Amount"
    )
    untaxed_amount: float = Field(
        ..., description="Untaxed amount", examples=[200.0], title="Untaxed Amount"
    )
    total: float = Field(
        ..., description="Total amount incl. taxes", examples=[242.0], title="Total"
    )
    lines: Optional[List[InvoiceLineItemOut]] = Field(
        [], description="Invoice lines", title="Lines"
    )
    partner_id: Optional[str] = Field(
        None,
        description="Technical id of the vendor/customer in Chift",
        examples=["<invoicing-partner-id>"],
        title="Partner Id",
    )
    invoice_number: Optional[str] = Field(
        None,
        description="Number/sequence",
        examples=["INV-2026-0001"],
        title="Invoice Number",
    )
    due_date: Optional[Date] = Field(
        None, description="Due date", examples=["2026-02-14"], title="Due Date"
    )
    reference: Optional[str] = Field(
        None, description="Reference", examples=["REF-0001"], title="Reference"
    )
    payment_communication: Optional[str] = Field(
        None,
        description="Payment communication",
        examples=["+++000/0000/00000+++"],
        title="Payment Communication",
    )
    customer_memo: Optional[str] = Field(
        None,
        description="Customer note/memo",
        examples=["Thank you for your business."],
        title="Customer Memo",
    )
    journal_ref: Optional[FieldRef] = Field(None, description="Journal")
    italian_specificities: Optional[ItalianSpecificitiesOutput] = Field(
        None, description="Specificities for Italy"
    )
    spanish_specificities: Optional[SpanishSpecificities] = Field(
        None, description="Specificities for Spain"
    )
    last_updated_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Last Updated On"
    )
    outstanding_amount: Optional[float] = Field(
        None,
        description="Amount left to be paid",
        examples=[0.0],
        title="Outstanding Amount",
    )
    last_payment_date: Optional[Date] = Field(
        None,
        description="Date of the last payment linked to the invoice",
        examples=["2026-02-14"],
        title="Last Payment Date",
    )
    accounting_date: Optional[Date] = Field(
        None,
        description="Accounting date",
        examples=["2026-01-15"],
        title="Accounting Date",
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical id of the payment method in Chift",
        examples=["<invoicing-payment-method-id>"],
        title="Payment Method Id",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the invoice currency to the connector's base currency, at the date of the invoice.",
        title="Currency Exchange Rate",
    )


class InvoiceItemOutSingle(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    id: str = Field(
        ..., description="Technical id in Chift", examples=["<this-id>"], title="Id"
    )
    source_ref: Ref = Field(..., description="Technical id in the target software")
    currency: str = Field(
        ...,
        description="Currency matching target sofware name",
        examples=["EUR"],
        title="Currency",
    )
    invoice_type: InvoicingInvoiceType = Field(..., description="Invoice type")
    status: InvoiceStatus = Field(..., description="Status")
    invoice_date: Date = Field(
        ..., description="Invoicing date", examples=["2026-01-15"], title="Invoice Date"
    )
    tax_amount: float = Field(
        ..., description="Taxes amount", examples=[42.0], title="Tax Amount"
    )
    untaxed_amount: float = Field(
        ..., description="Untaxed amount", examples=[200.0], title="Untaxed Amount"
    )
    total: float = Field(
        ..., description="Total amount incl. taxes", examples=[242.0], title="Total"
    )
    lines: Optional[List[InvoiceLineItemOut]] = Field(
        [], description="Invoice lines", title="Lines"
    )
    partner_id: Optional[str] = Field(
        None,
        description="Technical id of the vendor/customer in Chift",
        examples=["<invoicing-partner-id>"],
        title="Partner Id",
    )
    invoice_number: Optional[str] = Field(
        None,
        description="Number/sequence",
        examples=["INV-2026-0001"],
        title="Invoice Number",
    )
    due_date: Optional[Date] = Field(
        None, description="Due date", examples=["2026-02-14"], title="Due Date"
    )
    reference: Optional[str] = Field(
        None, description="Reference", examples=["REF-0001"], title="Reference"
    )
    payment_communication: Optional[str] = Field(
        None,
        description="Payment communication",
        examples=["+++000/0000/00000+++"],
        title="Payment Communication",
    )
    customer_memo: Optional[str] = Field(
        None,
        description="Customer note/memo",
        examples=["Thank you for your business."],
        title="Customer Memo",
    )
    journal_ref: Optional[FieldRef] = Field(None, description="Journal")
    italian_specificities: Optional[ItalianSpecificitiesOutput] = Field(
        None, description="Specificities for Italy"
    )
    spanish_specificities: Optional[SpanishSpecificities] = Field(
        None, description="Specificities for Spain"
    )
    last_updated_on: Optional[DateTime] = Field(
        None, examples=["2026-01-15T10:30:00Z"], title="Last Updated On"
    )
    outstanding_amount: Optional[float] = Field(
        None,
        description="Amount left to be paid",
        examples=[0.0],
        title="Outstanding Amount",
    )
    last_payment_date: Optional[Date] = Field(
        None,
        description="Date of the last payment linked to the invoice",
        examples=["2026-02-14"],
        title="Last Payment Date",
    )
    accounting_date: Optional[Date] = Field(
        None,
        description="Accounting date",
        examples=["2026-01-15"],
        title="Accounting Date",
    )
    payment_method_id: Optional[str] = Field(
        None,
        description="Technical id of the payment method in Chift",
        examples=["<invoicing-payment-method-id>"],
        title="Payment Method Id",
    )
    currency_exchange_rate: Optional[float] = Field(
        1,
        description="Exchange rate from the invoice currency to the connector's base currency, at the date of the invoice.",
        title="Currency Exchange Rate",
    )
    pdf: Optional[str] = Field(
        None,
        description="PDF document in base64",
        examples=["<base64-encoded-file-content>"],
        title="Pdf",
    )


class ReadSyncItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: constr(min_length=1) = Field(..., title="Name")
    config: Optional[Dict[str, Any]] = Field(None, title="Config")
    connections: List[SyncsConnectionItem] = Field(..., title="Connections")
    mappings: Optional[List[CreateSyncMappingItemOutput]] = Field([], title="Mappings")
    syncid: UUID = Field(..., title="Syncid")
    consumers: List[str] = Field(..., title="Consumers")
    flows: List[ReadFlowItem] = Field(..., title="Flows")


class ChiftPageInvoiceItemOut(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    items: List[InvoiceItemOut] = Field(..., title="Items")
    total: conint(ge=0) = Field(..., title="Total")
    page: conint(ge=1) = Field(..., title="Page")
    size: conint(ge=1) = Field(..., title="Size")


class CreateSyncItem(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    name: constr(min_length=1) = Field(..., title="Name")
    config: Optional[Dict[str, Any]] = Field(None, title="Config")
    connections: List[SyncsConnectionItem] = Field(..., title="Connections")
    mappings: Optional[List[CreateSyncMappingItemInput]] = Field([], title="Mappings")
    flows: Optional[List[CreateFlowItem]] = Field([], title="Flows")


class CubeAndFilter(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    and_: List[Union[CubeLeafFilter, CubeAndFilter, CubeOrFilter]] = Field(
        ..., alias="and", title="And"
    )


class CubeJoinQuery(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    queries: List[CubeLoadQuery] = Field(
        ..., max_length=5, min_length=1, title="Queries"
    )
    align: Optional[List[List[Optional[str]]]] = Field(None, title="Align")


class CubeLoadQuery(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    measures: Optional[List[str]] = Field(None, title="Measures")
    dimensions: Optional[List[str]] = Field(None, title="Dimensions")
    segments: Optional[List[str]] = Field(None, title="Segments")
    filters: Optional[List[Union[CubeLeafFilter, CubeAndFilter, CubeOrFilter]]] = Field(
        None, title="Filters"
    )
    timeDimensions: Optional[List[CubeTimeDimension]] = Field(
        None, title="Timedimensions"
    )
    order: Optional[Union[Dict[str, Order], List[CubeOrderItem], List[List[str]]]] = (
        Field(None, title="Order")
    )
    limit: Optional[int] = Field(None, title="Limit")
    offset: Optional[int] = Field(None, title="Offset")
    timezone: Optional[str] = Field(None, title="Timezone")
    total: Optional[bool] = Field(None, title="Total")
    renewQuery: Optional[bool] = Field(None, title="Renewquery")
    ungrouped: Optional[bool] = Field(None, title="Ungrouped")


class CubeOrFilter(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    or_: List[Union[CubeLeafFilter, CubeAndFilter, CubeOrFilter]] = Field(
        ..., alias="or", title="Or"
    )


CubeAndFilter.model_rebuild()
CubeJoinQuery.model_rebuild()
CubeLoadQuery.model_rebuild()
